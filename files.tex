...
\end{lisp}
  ...
  \cdf{merge-pathnames}.  If \cd{(pathname-directory {\it pathname\/})} is a list whose
...
wild-pathname-p pathname &optional field-key
    ...
    If no {\it field-key} is provided, or the {\it field-key} is \cdf{nil}, the result is
...
pathname-match-p pathname wildname
    ...
    \cdf{directory} function.  Missing components of {\it wildname} default to \cd{:wild}.

    If either argument is not a pathname, string, or file stream, an error
    of type \cdf{type-error} is signaled.  It is valid for {\it pathname} to be a
    wild pathname; a wildcard field in {\it pathname} will match only a
    wildcard field in {\it wildname}; that is, \cdf{pathname-match-p} is not commutative.
...
translate-pathname source from-wildname to-wildname &key
    ...
    {\it result} is \cd{"honk"}; however, the exact behavior of \cdf{translate-pathname}
    ...
    If the piece in {\it to-wildname} is \cd{:wild} or \cdf{nil}, the corresponding
...
{\it word\/} ::= \Mplus{letter\/ {\Mor} digit\/ {\Mor} \cdf{-}} \\
{\it wildcard-word\/} ::= \Mopt{word} \cdf{*} \Mstar{word\/ \cdf{*}} \Mopt{word} \\*
{\it wildcard-inferiors\/} ::= \cdf{**}
\end{tabbing}
  ...
  Each {\it directory} is a word, a wildcard word, or \cdf{**} (which is parsed as \cd{:wild-inferiors}).
  ...
  The {\it version} is a positive decimal integer or the word \cdf{NEWEST} (which is parsed
...
Given some components, \cdf{make-pathname}
...
the merging rules used by \cdf{merge-pathnames} are used to fill
...
Whenever a pathname is constructed, whether by \cdf{make-pathname} or some
...
\cdf{namestring} returns the full form of the {\it pathname} as a string.
\cdf{file-namestring} returns a string representing just the {\it name},
{\it type}, and {\it version} components of the {\it pathname};
the result of \cdf{directory-namestring}
represents just the {\it directory-name} portion; and \cdf{host-namestring}
...
\cdf{enough-namestring} takes another argument, {\it defaults}.
...
an error is signaled if \cdf{wild-pathname-p} would be true of {\it filename}.
\end{newer}
...
The functions \cdf{read-char} and/or \cdf{write-char} may be
...
\begin{newer}
X3J13 voted in June 1989 \issue{MORE-CHARACTER-PROPOSAL}
to eliminate the type \cdf{string-char}, add the type \cdf{base-character},
and redefine \cdf{open} to use the type \cdf{character} as the default
\cd{:element-type}.
...
\cdf{open} directly should use the special form \cdf{unwind-protect} to
...
\cdf{with-open-file}
evaluates the {\it forms} of the body (an implicit \cdf{progn}) with the variable
...
\cd{:if-does-not-exist~nil}, {\it stream} will be bound to \cdf{nil}
...
a \cdf{merge-pathnames} operation using {\it file} as the defaults.
...
as the {\it file} argument to \cdf{rename-file} has implementation-dependent consequences;
\cdf{rename-file} might signal an error, for example,
...
called \cdf{renamef} in MacLisp and Lisp Machine Lisp.
The name \cdf{renamef} is not used in Common Lisp
because the convention that a trailing \cdf{f} means ``file''
conflicts with the use of a trailing \cdf{f}
for forms related to \cdf{setf}.
\end{incompatibility}
\afternoterule
\end{defun}
...
as the {\it file} argument to \cdf{delete-file} has implementation-dependent consequences;
\cdf{delete-file} might signal an error, for example,
...
\begin{incompatibility}
This corresponds to the function
called \cdf{deletef} in MacLisp and Lisp Machine Lisp.
\end{incompatibility}
\afternoterule
\end{defun}
...
\begin{incompatibility}
This corresponds to the function
called \cdf{probef} in MacLisp and Lisp Machine Lisp.
\end{incompatibility}
\afternoterule
...
\begin{incompatibility}
This corresponds to the function
called \cdf{filepos} in MacLisp and Lisp Machine Lisp.
\end{incompatibility}
\afternoterule
\end{defun}
...
\begin{incompatibility}
This corresponds to the function
called \cdf{lengthf} in MacLisp and Lisp Machine Lisp.
\end{incompatibility}
\afternoterule
\end{defun}
...
to the {\it file-stream}, or \cdf{nil} if this
...
X3J13 voted in June 1989 \issue{PATHNAME-WILD}
to clarify that supplying a wild pathname
as the {\it filename} argument to \cdf{load} has implementation-dependent consequences;
