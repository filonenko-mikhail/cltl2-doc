...
\cdf{special} declarations {\it do} affect the interpretation of variable
...
A \cdf{declare} form is known as a {\it declaration}.
...
it appears explicitly, as a list whose {\it car} is the symbol \cdf{declare},
...
declaration applies to {\it both} bindings of \cdf{x}.
...
\cdf{notinline} would {\it not} apply to that particular call to \cdf{floor}.
...
to specify that \cdf{locally} executes the {\it form\/}s as an implicit
\cdf{progn} and returns the value(s) of the last {\it form}.
\end{new}
...
The function \cdf{proclaim} takes a {\it decl-spec} as its
...
{\it declaration-form}.''  That was incorrect; \cdf{proclaim} accepts
...
anywhere \cdf{proclaim} may be called.  However, each {\it decl-spec}
is not evaluated.
...
A \cdf{special} declaration does {\it not} affect bindings pervasively.
Inner bindings of a variable implicitly shadow
a \cdf{special} declaration and must be explicitly re-declared to
be special.
(However, a \cdf{special} proclamation {\it does} pervasively affect bindings;
...
{\it exp} assigned to \cdf{bodoni} within the scope of the declaration
...
\end{lisp}
The variable \cdf{maxwell} is declared to be an integer over the {\it scope}
of the type declaration, not over its {\it extent}.  Indeed \cdf{maxwell}
...
\cdf{spy-swap}, which is {\it not} in the scope of the type declaration!
...
object {\it x} is an {\it otherwise inaccessible part}
  ...
  Now suppose that construct {\it c} contains a \cdf{dynamic-extent} declaration for
...
{\it value-type} is a \cdf{values} type specifier.
\end{newer}
...
and \cdf{flonum-identity} constructs can be expressed as \cd{(the fixnum {\it x})}
