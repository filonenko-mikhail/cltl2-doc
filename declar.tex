...
Декларации указывают Lisp системе дополнительную информацию о вашей
...
декларации \cdf{special}, которые \emph{влияют} на интерпретацию
...
\cdf{declaim}, он в отличии от \cdf{proclaim} не вычисляет свои
аргументы.
...
\cdf{dolist}\>\cdf{with-open-stream} \\*
\cdf{dotimes}\>\cdf{with-output-to-string} \\*
\cdf{flet}\>\cdf{with-condition-restarts} \\
\cdf{print-unreadable-object}\>\cdf{with-standard-io-syntax}
\end{lisp}
...
\emph{value-type} не принадлежит спецификатору \cdf{values},
конструкцию \cdf{the} можно заменить следующей:
\begin{lisp}
(let ((\#1=\#:temp \emph{form})) (declare (type \emph{value-type} \#1\#)) \#1\#)
\end{lisp}
В случае возврата нескольких значений, конструкция может быть записана
более точно, используя, например, \cdf{multiple-value-bind}.
\end{defspec}
