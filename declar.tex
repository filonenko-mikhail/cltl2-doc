...
~~(declare (dynamic-extent \#'gd))~~~~~;\textrm{\cdf{mapcar} won't hang on to \cdf{gd}}\\*
...
~~(setq foo 3)~~~~~~~~~~~~~~~~~~~~~;\textrm{Correct} \\*
~~(setq foo j))~~~~~~~~~~~~~~~~~~~~;\textrm{Erroneous---but why? (see text)}
\end{lisp}
...
\begin{newer}
X3J13 проголосова в июне 1989
\issue{PROCLAIM-ETC-IN-COMPILE-FILE}
ввести новый макрос \cdf{declaim}, который гарантированно 
...
~~...)
\end{lisp}
Ссылка на \cd{*print-circle*} в первой строке примера является специальной, так
...
Функция \cdf{proclaim} в качестве аргумета принимает \emph{decl-spec}, и
применяет указания в глобальном пространстве. (Такие глобальные декларации
называются \emph{прокламации}.)
Так как \cdf{proclaim} является функцией, то ее аргумент вычисляется всегда.
...
(proclaim '(inline floor))
\end{lisp}
рекомендует то, что \cdf{floor} должна быть встроена в места ее вызова. Но в
ситуации 
\begin{lisp}
(defun foo (x y) (declare (notinline floor)) ...)
\end{lisp}
...
~~~~(let ((y 4)) (declare (special y)) (foo x))))
\end{lisp}
В вышеприведенном коде, внешние и внутренние связывания переменной \cd{y}
...
переменной \cd{y}.
Все связывания и переменной \cd{x} и ссылки на нее является специальными,
...
\cdf{labels}), то декларация примениться для локального определения, а не для глобального.
...
Помните, компилятор может игнорирвать эту декларацию.

Следует отметить, что соблюдаются правила лексической области видимости. Если
одна из указанных функций лексически ограничена (с помощью \cdf{flet} или
\cdf{labels}), то декларация примениться для локального определения, а не для
глобального.
...
Компилятор \emph{не} может ее игнорировать.
