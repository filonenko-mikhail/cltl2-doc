...
тип \textit{type1} точно является () подтипом типа \textit{type2}.
Если результат {\false}, тогда тип \textit{type1} может быть, а может и не быть
подтипом типа \textit{type2} (иногда это невозможно оределить, особенно когда
...
\begin{tabbing}
~~~~~~~~\=~~~~~~~~\=\kill
{\true}\>{\true}\>\textit{type1} точно является подтипом \textit{type2} \\
{\false}\>{\true}\>\textit{type1} точно не является подтипом \textit{type2} \\
...
If \textit{A} and \textit{B} are type specifiers (other than \cdf{*}, which technically
...
manner.
If \textit{A} and \textit{B} are two type specifiers (but not \cdf{*}, which technically
...
\textit{integerp} является истиной, если аргумент целое число, в противном
случае является ложью.
\begin{lisp}
(integerp x) \EQ\ (typep x 'integer)
\end{lisp}
...
\textit{complexp} является истиной, если аргумент комплексное число, в противном
случае является ложью.
\begin{lisp}
(complexp x) \EQ\ (typep x 'complex)
\end{lisp}
...
\cd{(eq \textit{x} \textit{y})} является истиной тогда и только тогда, когда,
\textit{x} и \textit{y} являются идентичными объектами.
(В реализациях, \textit{x} и \textit{y} обычно равны \cdf{eq} тогда и только
...
the two \textit{car}'s being \cdf{equal} and the two \textit{cdr}'s being
\cdf{equal}.

Для cons-ячеек, \cdf{equal} определен рекурсивно, как сравнение \cd{equal}
сначала \textit{car} элементов, а затем \textit{cdr}.
...
the two \textit{car}'s being \cdf{equalp} and the two \textit{cdr}'s being
\cdf{equalp}.
...
\textit{car} элементы, а затем \textit{cdr}.
...
\cdf{or} являются контролирующими структурами, потому что их аргументы
...
\cdf{not} возвращает {\true}, если \textit{x} является {\false}, иначе
возвращает {\false}.
...
\cd{(and \textit{form1} \textit{form2} ... )} последовательно слева направо
вычисляет формы. Если какая-либо форма \textit{formN} вычислилась в {\false},
...
{\flase} обозначает ложь и не-{\false} значения истину, так и для условных
выражений.
Например:
\begin{lisp}
(if (and (>= n 0) \\
...
\cd{(and \textit{x})} \EQ\ \textit{x}. Также \cd{(and)} выполняется в {\true},
...
вектора (element 0 \textit{and} element 1 \textit{and}
element 2 \textit{and} $\ldots$), можно использовать функцию \cdf{every}.
\end{defmac}
...
\cd{(or \textit{form1} \textit{form2} ... )} последовательно выполняет каждую
...
Из общего определения, можно сделать дедуктивный вывод о том, что \cd{(or
  \textit{x})} \EQ\ \textit{x}. Также, \cd{(or)} выполняется в {\nil}, который
...
Если необходимо проверить истинность предиката для всех элементов списка или
вектора (element 0 \textit{or} element 1 \textit{or}
element 2 \textit{or} $\ldots$), можно использовать функцию \cdf{some}.
\end{defmac}
