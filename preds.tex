...
\cd{null} is true if its argument is {\empty},
...
and otherwise is false.
Note that \cd{(atom '{\empty})} is true, because {\empty}$\;\equiv\;${\nil}.
\begin{lisp}
...
\cd{listp} is true if its argument is a cons or the empty list {\empty},
