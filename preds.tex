...
\cdf{typep} является предикатом, которые истинен, если объект \emph{object}
...
\cdf{symbolp} является истиной, если её аргумент является символом, в противном
...
X3J13 проголосовал в марте 1989 \issue{REAL-NUMBER-TYPE} добавить функцию  \cdf{realp}.
...
\cdf{bit-vector-p} является истиной, если аргумент битовый вектор, иначе является ложью.
\begin{lisp}
(bit-vector-p x) \EQ\ (typep x 'bit-vector)
\end{lisp}
\end{defun}
...
Специальная форма Lisp'а \cdf{and} отличается тем, что в определенных случаях
...
Специальная форма Lisp'а \cdf{or} отличается тем, что в определенных случаях
вычисляет не все аргументы.
