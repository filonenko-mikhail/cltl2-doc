...
function is used \emph{or} if the condition is of type \cdf{error}.
\end{incompatibility}
\afternoterule
...
bound to \cdf{nil}.  Specifying \cd{(:report \emph{fn})} to
...
*break-on-signals*
  ...
  that \emph{condition}. The \cdf{continue} restart may be used to continue with
...
check-type place typespec [string]
  ...
  \cdf{type-error} if the contents of \emph{place} are not of the
  desired type.
...
use of \cdf{with-condition-restarts}.
  ...
  The \emph{place} must be a generalized variable reference acceptable to \cdf{setf}. The
  \emph{typespec} must be a type specifier; it is not evaluated.  The \cdf{string} should
...
assert test-form [({place}*) [datum {argument}*]]
  ...
  An \cdf{assert} form signals an error if the value of the \emph{test-form} is \cdf{nil}.
  Continuing from this
  error using the \cdf{continue} restart will allow the user to alter the values
  of some variables, and \cdf{assert} will then start over, evaluating the \emph{test-form}
  again.
(The restart is associated with the signaled condition as if by
use of \cdf{with-condition-restarts}.)
\cdf{assert} returns \cdf{nil}.
  ...
  If \emph{datum} is a condition type (a class or class name), then the condition used is effectively the result
  of \cd{(apply \#'make-condition \emph{datum} (list \Mstar{argument}))}.
...
\beforenoterule
  \begin{implementation}
  The debugger need not include the \emph{test-form} in
  the error message, and any \emph{places} should not be included in the message,
...
ctypecase keyplace {(type {form}*)}*
  ...
  The \emph{keyplace} must be a generalized variable reference acceptable to \cdf{setf}.
  ...
  \emph{keyplace} and then \cdf{ctypecase} starts over, making the type tests again. 
  Subforms of \emph{keyplace} may be evaluated multiple times. If the \cdf{store-value}
...
ccase keyplace {({({key}*) | key} {form}*)}*
  ...
  with a message constructed from the clauses. This error may be continued
  using the \cdf{store-value} restart. The argument to \cdf{store-value} is stored in
  \emph{keyplace} and then \cdf{ccase} starts over, making the type tests again. Subforms
  of \emph{keyplace} may be evaluated multiple times. If the \cdf{store-value} restart is
...
handler-case expression {(typespec ([var]) {form}*)}*
  ...
  Each \emph{typespec} may be any type specifier. If during the execution of the \cdf{expression}
  a condition is signaled for which there is an appropriate clause---that is, one
  for which \cd{(typep \emph{condition} '\emph{typespec})} is true---and if there is no intervening
...
\end{lisp}
  if the first clause (containing \emph{form\SU{1}}) has been selected, the handler
...
~~(let (\#2=\#:var-2) \\*
~~~~(tagbody \\*
~~~~~~(handler-bind ((\emph{type\SU{1}} \pushtabs\=\#'(lambda (temp) \\*
\>~~~~(setq \#2\# temp) \\*
\>~~~~(go \#3=\#:tag-3)))\poptabs \\
~~~~~~~~~~~~~~~~~~~~~(\emph{type\SU{2}} \pushtabs\=\#'(lambda (temp) \\*
\>~~~~(setq \#2\# temp) \\*
\>~~~~(go \#4=\#:tag-4)))\poptabs \\*
~~~~~~~~~~~~~~~~~~~~~...) \\*
...
to indicate that the very same gensym appears in multiple places.---GLS]
  ...
  As a special case, the \emph{typespec} can also be the symbol \cd{:no-error} in the last clause.
  If it is, it designates a clause that will take control if the \emph{expression} returns
  ...
  \cdf{multiple-value-call} on the return value of the \emph{expression}.
For example,
...
~~(\emph{type\SU{2}} (\emph{var\SU{2}}) . \emph{body\SU{2}}) \\*
~~... \\*
~~(\emph{type\SU{\hbox{\scriptsize\it n}}} (\emph{var\SU{\hbox{\scriptsize\it n}}}) . \emph{body\SU{\hbox{\scriptsize\it n}}}) \\*
~~(:no-error (\emph{nvar\SU{1}} \emph{nvar\SU{2}} ... \emph{nvar\SU{\hbox{\scriptsize\it m}}}) . \emph{nbody}))
\end{lisp}
is approximately equivalent to
\begin{lisp}
(block \#1=\#:error-return \\*
~~(multiple-value-call \#'(lambda (\emph{nvar\SU{1}} \emph{nvar\SU{2}} ... \emph{nvar\SU{\hbox{\scriptsize\it m}}}) . \emph{nbody}) \\*
...
handler-bind ({(typespec handler)}*) {form}*
  ...
  with \cdf{typecase}). If an appropriate \emph{typespec} is found, the associated handler 
...
\item[\cd{(:documentation \emph{doc-string})}]
     ...
     The \emph{doc-string} should be either \cdf{nil} or
     a string that describes the purpose of the
     condition type. If this option is omitted, \cdf{nil} is assumed.
     Calling \cd{(documentation '\emph{name} 'type)} will retrieve this information.

\item[\cd{(:report \emph{exp})}]

     If \emph{exp} is not a literal string, it must be a suitable argument to the
     \cdf{function} special operator. The expression \cd{(function~\emph{exp})} will be evaluated
     ...
     condition is printed while \cdf{*print-escape*} is \cdf{nil}.
...
\end{lisp}
     ...
     The \cd{:report} option is processed \emph{after} the new condition type has been defined,
...
\end{lisp}
  ...
  \cd{machine-not-available-error},
...
\cd{restart-bind} while offering a more\vadjust{\penalty-10000}
palatable syntax. See also 
\cd{with-simple-restart}.
The function that transfers control to a restart
point established by one of these macros is called \cd{invoke-restart}.
...
with-simple-restart (name format-string {format-argument}*)
  ...
  The \emph{name} may be \cdf{nil}, in which case an anonymous restart
  is established.
...
restart-case expression {(case-name arglist
  ...
  returned by the \cdf{restart-case} form. While the \emph{expression} is running, any code may
  ...
  returned by the last such \emph{form} will be returned by the \cdf{restart-case} form.

As a special case,
if the \emph{expression} is a list whose \emph{car} is \cdf{signal}, \cdf{error},
...
\end{lisp}
  ...
  If there are no \emph{form\/}s in a selected clause, \cdf{restart-case} returns \cdf{nil}.
...
\item[\cd{:test \emph{fn}}]
    ...
    The \emph{fn} must be a suitable argument for the \cdf{function} special operator. The
    expression \cd{(function~\emph{fn})} will be evaluated in the current lexical
...
to the stream.---GLS]
    ...
    When \cdf{*print-escape*} is \cdf{nil}, the printer will use the report information for
...
~~(let ((\#2=\#:var-2 nil)) \\*
~~~~(tagbody \\*
~~~~~~(restart-bind ((\emph{name\SU{1}} \pushtabs\=\#'(lambda (\&rest temp) \\*
\>~~~~(setq \#2\# temp) \\*
\>~~~~(go \#3=\#:tag-3)) \\*
\>\textrm{$\langle$slightly transformed \emph{options\SU{1}}$\rangle$})\poptabs \\
~~~~~~~~~~~~~~~~~~~~~(\emph{name\SU{2}} \pushtabs\=\#'(lambda (\&rest temp) \\*
\>~~~~(setq \#2\# temp) \\*
\>~~~~(go \#4=\#:tag-4)) \\*
\>\textrm{$\langle$slightly transformed \emph{options\SU{2}}$\rangle$})\poptabs \\*
~~~~~~~~~~~~~~~~~~~~~...) \\*
...
restart-bind ({(name function {keyword value}*)}*) {form}*
  ...
  Each \emph{name} may be \cdf{nil} to indicate an anonymous restart, or some other symbol
...
\item[\cd{:interactive-function \emph{form}}]
     ...
     this restart. The function may prompt interactively using \cdf{*query-io*}
     if necessary.
...
\item[\cd{:report-function \emph{form}}]
     ...
     function is called whenever the restart is printed while \cdf{*print-escape*}
     is \cdf{nil}.
\end{flushdesc}
\end{defmac}
...
the value of \emph{restarts-form} should be a list of restarts \cd{(\emph{R1} \emph{R2} ...)}.
      ...
      consider the restarts \emph{R1}, \emph{R2}, $\ldots$ if $\emph{C}'$ is \cdf{eq} to \emph{C}.
...
with that condition are considered.
  ...
  If the \emph{restart-identifier} is a non-\cdf{nil}
  ...
  the symbol \cdf{nil} to be given as the \emph{restart-identifier}.  Applications that 
...
invoke-restart-interactively restart-identifier
  ...
  Calls the function associated with the given \emph{restart-identifier}, prompting for any
  necessary arguments. The \emph{restart-identifier} must be a restart or the non-null name
  ...
  performs
  \cd{(apply~\#'invoke-restart \emph{restart-identifier} \emph{arguments})}.
...
warn datum &rest arguments
  ...
  If \emph{datum} is a condition type (a class or class name), then the condition used is effectively the result
  of \cd{(apply \#'make-condition \emph{datum} \emph{arguments})}. This result
...
invoke-debugger condition
  ...
  arguments: the \emph{condition} being handled and the value of \cd{*debugger-hook*}.
...
simple-condition
  ...
  \cdf{simple-condition-format-control} and \cdf{simple-condition-format-arguments}.
...
type-error-datum condition
  ...
  Accesses the datum slot of a given \emph{condition}, which must be of
  type \cdf{type-error}.
\end{defun}
...
type-error-expected-type condition
  ...
  Accesses the expected-type slot of a given \emph{condition}, which must be
...
simple-type-error
  ...
  \cd{simple-condition-format-control} and
  \cd{simple-condition-format-arguments}.
  If \cd{:format-arguments} is not supplied to
  \cd{make-condition}, the 
  format-arguments slot defaults to \cdf{nil}.
...
program-error
  ...
  \emph{not} a subtype of \cdf{control-error}.
\end{defun}
...
control-error
  ...
  from this type. This is a subtype of \cdf{error}. This is \emph{not} a subtype of
  \cdf{program-error}.
...
package-error-package condition
  ...
  manipulated in a \emph{condition} of type \cdf{package-error}.
\end{defun}
