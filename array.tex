...
\cdf{string-char} are called {\it strings}.  Vectors whose elements are
restricted to type \cdf{bit} are called {\it bit-vectors}.
\end{obsolete}
...
\cdf{array-total-size} returns the total number of elements in the {\it array},
...
A call to \cdf{row-major-setf} is suitable for use as a {\it place} for \cdf{setf}.
...
and {\it new-element} may be any object.  \cdf{vector-push} attempts to store
...
place and \cdf{vector-push} returns the {\it former} value of the fill pointer
...
with \cdf{push}, which is a macro that requires a {\it place} suitable
...
\cdf{adjust-array} returns an array of the same type and rank as {\it array},
...
If \cdf{adjust-array} is applied to an {\it array} that is displaced
...
to array {\it Z}, and array {\it Y} is altered by \cdf{adjust-array}, array
