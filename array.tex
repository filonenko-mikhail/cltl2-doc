...
Массив является объектом с элементами, расположенными в соотвествие с
...
ноль.
(Нулемерный массив измерений имеет только один элемент. FIXME)
...
указывает задаёт одиночный элемент. В противном случае, значение должно быть
...
Другие функции доступа, такие как \cdf{svref}, \cdf{char} и \cdf{bit},
используются в частных случаях.
...
\cdf{svref} может быть быстрее чем \cdf{aref}.
...
как способ обработки \cdf{\&rest} аргументов и компиляции \cdf{apply} вызовов.
...
возвращает \cdf{предыдущее} значение указателя. Таким образом, \cdf{vector-push}
