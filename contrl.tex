...
By convention, any function named \cd{(setf \textit{f\/})} should return its first
argument as its only value, in order to preserve the specification that
\cdf{setf} returns its \textit{newvalue}.  See \cdf{setf}.
...
\cd{(quote \textit{x})} simply returns \textit{x}.
...
any form \textit{f} preceded by a single quote (\cd{~'~}) character
is assumed to have \cd{(quote~~)} wrapped around it to
make \cd{(quote \textit{f})}.
For example:
\begin{lisp}
...
of \textit{fn}; \textit{fn} is interpreted as if it had appeared
in the functional position of a function invocation.
In particular,
if \textit{fn} is a symbol, the functional definition associated with
that symbol is returned; see \cdf{symbol-function}.
If \textit{fn} is a lambda-expression, then a
...
of type \cdf{function}.  This implies that a form \cd{(function~\textit{fn\/})}
may be interpreted as \cd{(the (function~\textit{fn\/}))}.
...
of the variable \cdf{x} even after control has returned from the
function \cdf{adder}.

More generally, a lexical closure in effect retains the ability to
refer to lexically visible \textit{bindings}, not just values.
Consider this code:
\begin{lisp}
(defun two-funs (x) \\
...
refers to the \textit{binding} of the variable \cdf{x} created on entry to
...
~~funs)
\end{lisp}
is a list of ten closures that \textit{may} be pairwise \cdf{eq}.  Although
...
but somewhat cumbersome to type, a standard abbreviation is defined for them:
any form \textit{f} preceded by \cd{\#'} (\cd{\#} followed by an apostrophe)
is assumed to have \cd{(function  )} wrapped around it to make
\cd{(function \textit{f})}.  For example,
\begin{lisp}
...
named by \textit{symbol}.  An error is signalled if the symbol has no function
...
Performing this operation causes the symbol to have \textit{only} the
...
definition named by the argument \textit{function-name}.
...
\cdf{boundp} is true if the dynamic (special) variable named by \textit{symbol}
...
It is possible for \textit{both} \cdf{special-form-p} and \cdf{macro-function}
...
The special form \cd{(setq \textit{var1} \textit{form1} \textit{var2} \textit{form2} ...)} is the
``simple variable assignment statement'' of Lisp.
First \textit{form1} is evaluated
and the result is stored in the variable \textit{var1}, then \textit{form2}
is evaluated and the result stored in \textit{var2}, and so forth.
...
\issue{SYMBOL-MACROLET-SEMANTICS} to specify that if any \textit{var}
refers not to an ordinary variable but to a binding made by
\cdf{symbol-macrolet}, then that \textit{var} is handled as
...
\cdf{set} causes the dynamic variable named by \textit{symbol} to take on
\textit{value} as its value.

\begin{new}
X3J13 voted in January 1989
\issue{ARGUMENTS-UNDERSPECIFIED}
to clarify that the \textit{value}
...
\cdf{set} returns \textit{value} as its result.
...
by \textit{symbol} to become unbound (have no value).  \cdf{fmakunbound}
...
\end{lisp}
Both functions return \textit{symbol} as the result value.
...
often called \textit{access} and \textit{update} operations.  The concept of
...
named.  Examples of such storage locations are the \textit{car} and \textit{cdr} of
...
that implement the conceptual \textit{access} and \textit{update} operations.
...
The function \cdf{car} accesses the \textit{car} of a cons,
...
with given arguments as a \textit{name} for the storage location.  Thus, just
as \cdf{x} may be considered a name for a storage location (a variable), so
\cd{(car x)} is a name for the \textit{car} of some cons (which is in turn
...
{\rm Access Function}&{\rm Update Function}&{\rm Update Using \cdf{setf}} \\
\hlinesp
\cdf{x}&\cd{(setq x datum)}&\cd{(setf x datum)} \\
...
\cd{(setf \textit{place} \textit{newvalue})} takes a form \textit{place} that when evaluated
\textit{accesses} a data object in some location and ``inverts''
it to produce a corresponding form to \textit{update} the location.
A call to the \cdf{setf} macro therefore
expands into an update form that stores the result of evaluating
the form \textit{newvalue} into the place referred to by the access form.

If more than one \textit{place}-\textit{newvalue} pair is specified,
the pairs are processed sequentially; that is,
\begin{lisp}
(setf \textit{place1} \textit{newvalue1} \\
~~~~~~\textit{place2} \textit{newvalue2}) \\
~~~~~~... \\
~~~~~~\textit{placen} \textit{newvaluen})
\end{lisp}
is precisely equivalent to
\begin{lisp}
(progn (setf \textit{place1} \textit{newvalue1}) \\
~~~~~~~(setf \textit{place2} \textit{newvalue2}) \\
~~~~~~~... \\
~~~~~~~(setf \textit{placen} \textit{newvaluen}))
\end{lisp}
For consistency, it is legal to write \cd{(setf)}, which simply returns {\nil}.

The form \textit{place} may be any one of the following:
\begin{itemize}
\item
...
%to add \cdf{class-name} to this list.
%\end{newer}
...
%to add \cdf{readtable-case} to this list.
%\end{newer}

%??? \begin{newer}
%X3J13 voted in June 1989 \issue{PATHNAME-LOGICAL}
%to add \cdf{logical-pathname-translations} to this list.
%\end{newer}
...
to that function is in turn a \textit{place} form;
in this case the new \textit{place} has stored back into it the
result of applying the specified ``update'' function
(which is in each of these cases not a true update function):

\begin{flushleft}
\begin{tabular}{@{}lll@{}}
Function Name&Argument That Is a \textit{place}&Update Function Used \\
\hlinesp
...
transferred to the \textit{newvalue} form, and the resulting \cdf{setf} form is
...
\cd{\#'\textit{name}}, that is, \cd{(function \textit{name})}, where \textit{name}
...
\begin{lisp}
(setf (apply \#'\textit{name} \textit{x1} \textit{x2} ... \textit{xn} \textit{rest}) \textit{x0})
\end{lisp}
The \cdf{setf} method for the function \textit{name} must be such that
\begin{lisp}
(setf (\textit{name} \textit{z1} \textit{z2} ... \textit{zm}) \textit{z0})
\end{lisp}
expands into a store form
\begin{lisp}
(\textit{storefn} \textit{zi${}_1$} \textit{zi${}_2$} ... \textit{zi${}_k$} \textit{zm})
\end{lisp}
That is, it must expand into a function call such that all arguments but
the last may be any permutation or subset of the new value \textit{z0} and
the arguments of the access form, but the \textit{last} argument of the storing
...
\begin{lisp}
(apply \#'\textit{storefn} \textit{xi${}_1$} \textit{xi${}_2$} ... \textit{xi${}_k$} \textit{rest})
\end{lisp}
As an example, suppose that the variable \cdf{indexes} contains a list
of subscripts for a multidimensional array \textit{foo} whose rank is not
...
\begin{newer}
X3J13 voted in March 1989 \issue{FUNCTION-NAME} to clarify that this
step uses \cd{macroexpand-1}, not \cdf{macroexpand}.  This allows the chance
...
\begin{itemize}
\item  Any other list whose first element is a symbol (call it \textit{f\/}).
     In this case, the call to \cdf{setf} expands into a call to the function
     named by the
     list \cd{(setf~\textit{f\/})} (see section~\ref{FUNCTION-NAME-SECTION}).
     The first argument is the new value and the
     remaining arguments are the values of the remaining elements of
     \textit{place}.  This expansion occurs regardless of whether either \textit{f\/} or
     \cd{(setf \textit{f\/})} is defined as a function locally, globally, or not at
     all.  For example,
\begin{lisp}
(setf (\textit{f\/} \textit{arg1} \textit{arg2} ...) \textit{newvalue})
\end{lisp}
     expands into a form with the same effect and value as
\begin{lisp}
(let ((\#:temp1 \textit{arg1})~~~~~;{\rm Force correct order of evaluation} \\*
~~~~~~(\#:temp2 \textit{arg2}) \\*
~~~~~~... \\*
~~~~~~(\#:temp0 newvalue)) \\*
~~(funcall (function (setf \textit{f\/})) \\*
~~~~~~~~~~~\#:temp0 \\*
~~~~~~~~~~~\#:temp1 \\*
~~~~~~~~~~~\#:temp2 ...))
\end{lisp}
By convention, any function named \cd{(setf \textit{f\/})} should return its first
argument as its only value, in order to preserve the specification that
\cdf{setf} returns its \textit{newvalue}.
\end{itemize}
\end{newer}
...
of \textit{newvalue}.  Therefore \cd{(setf (car x) y)} does not expand
...
to extend the specification of \cdf{setf} to allow a \textit{place}
whose \cdf{setf} method has more than one store variable (see \cdf{define-setf-method}).
In such a case as many values are accepted from the \textit{newvalue} form
...
\cdf{psetf} is like \cdf{setf} except that if more than one \textit{place}-\textit{newvalue}
...
(The unpredictability matters only if more than one \textit{place} form
refers to the same place.)
\cdf{psetf} always returns {\false}.

\begin{newer}
X3J13 voted in June 1989 \issue{SETF-MULTIPLE-STORE-VARIABLES}
to extend the specification of \cdf{psetf} to allow a \textit{place}
whose \cdf{setf} method has more than one store variable (see \cdf{define-setf-method}).
In such a case as many values are accepted from the \textit{newvalue} form
as there are store variables; extra values are ignored
and missing values default to \cdf{nil},
...
In the form \cd{(shiftf \textit{place1} \textit{place2} ... \textit{placen} \textit{newvalue})},
...
\end{lisp}
The effect of \cd{(shiftf \textit{place1} \textit{place2} ... \textit{placen} \textit{newvalue})}
is equivalent to
\begin{lisp}
...
\end{lisp}
Moreover, for certain \textit{place} forms \cdf{shiftf} may be
significantly more efficient than the \cd{prog1} version.

\begin{newer}
X3J13 voted in June 1989 \issue{SETF-MULTIPLE-STORE-VARIABLES}
to extend the specification of \cdf{shiftf} to allow a \textit{place}
whose \cdf{setf} method has more than one store variable (see \cdf{define-setf-method}).
In such a case as many values are accepted from the \textit{newvalue} form
as there are store variables; extra values are ignored
and missing values default to \cdf{nil},
...
Each \textit{place} form may be any form acceptable
as a generalized variable to \cdf{setf}.
In the form \cd{(rotatef \textit{place1} \textit{place2} ... \textit{placen})},
the values in \textit{place1} through \textit{placen} are accessed and saved.
Values 2 through \textit{n} and value 1 are then stored into \textit{place1} through \textit{placen}.
It is as if all the places form an end-around shift register
that is rotated one place to the left, with the value of \textit{place1}
being shifted around the end to \textit{placen}.
Note that \cd{(rotatef \textit{place1} \textit{place2})} exchanges the contents
of \textit{place1} and \textit{place2}.

The effect of \cd{(rotatef \textit{place1} \textit{place2} ... \textit{placen})}
is roughly equivalent to
\begin{lisp}
(psetf \textit{place1} \textit{place2} \\
~~~~~~~\textit{place2} \textit{place3} \\
~~~~~~~... \\
~~~~~~~\textit{placen} \textit{place1})
\end{lisp}
except that the latter would evaluate any subforms of each \textit{place} twice,
whereas \cdf{rotatef} takes care to evaluate them only once.
Moreover, for certain \textit{place} forms \cdf{rotatef} may be
...
\begin{newer}
X3J13 voted in June 1989 \issue{SETF-MULTIPLE-STORE-VARIABLES}
to extend the specification of \cdf{rotatef} to allow a \textit{place}
whose \cdf{setf} method has more than one store variable (see \cdf{define-setf-method}).
In such a case as many values are accepted from the \textit{newvalue} form
as there are store variables; extra values are ignored
and missing values default to \cdf{nil},
...
reference \cd{(setf \textit{reference} \textit{value})} the \textit{value} form
must be evaluated \textit{after} all the subforms of the reference because
the \textit{value} form appears to the right of them.
...
\textit{value1}, and then the rest of the \cdf{setf} form is processed in a like manner.
...
takes a \textit{place} as an argument, for example, \cdf{ldb}, \cdf{mask-field}, and
...
\item It evaluates the value-producing subforms, in left-to-right order, and 
     binds the temporary variables to them; this is called \textit{binding the temporaries}.

\item It reads the value from the generalized variable, using the supplied 
     accessing form, to get the old value;  this is called \textit{doing the
...
\item It binds the store variable to a new value, and then installs this
     ...
     form; this is called \textit{doing the store}.
\end{itemize}
...
\textit{place} specifiers as arguments. During the \cdf{setf} expansion of these forms, it 
is necessary to call \cdf{get-setf-method} to determine how the inner, 
nested generalized variable must be treated.

    In a form such as
\begin{lisp}
(setf (ldb \textit{byte-spec} \textit{place-form}) \textit{newvalue-form})
\end{lisp}
    the place referred to by the \textit{place-form} must always be both accessed 
    and updated;  note that the update is to the generalized variable 
    specified by \textit{place-form}, not to any object of type \cdf{integer}.

    Thus this call to \cdf{setf} should generate code to do the following:
\begin{itemize}
    \item Evaluate \textit{byte-spec} and bind into a temporary
    \item Bind the temporaries for \textit{place-form}
    \item Evaluate \textit{newvalue-form} and bind into the store variable
    \item Do the access to \textit{place-form}
    \item Do the store into \textit{place-form} with the given bit-field of the
          accessed integer replaced with the value in the store variable
\end{itemize}
    If the evaluation of \textit{newvalue-form} alters what is found in the 
    given \textit{place}---such as setting a different bit-field of the
    integer---then the change of the bit-field denoted by
    \textit{byte-spec} will be to that 
    altered integer, because the access step must be done after the \textit{newvalue-form}
    evaluation.  Nevertheless, the 
    evaluations required for binding the temporaries are done before the
    evaluation of the \textit{newvalue-form}, thereby preserving
...
The treatment of \cdf{mask-field} is similar to that of \cdf{ldb}.
    ...
    In a form such as:
\begin{lisp}
(setf (getf \textit{place-form} \textit{ind-form}) \textit{newvalue-form})
\end{lisp}
    the place referred to by the \textit{place-form} must always be both accessed 
    and updated;  note that the update is to the generalized variable 
    specified by \textit{place-form}, not necessarily to the particular list
    which is the property list in question.

    Thus this call to \cdf{setf} should generate code to do the following:
\begin{itemize}
    \item Bind the temporaries for \textit{place-form} 
    \item Evaluate \textit{ind-form} and bind into a temporary
    \item Evaluate the \textit{newvalue-form} and bind into the store variable
    \item Do the access to \textit{place-form}
    \item Do the store into \textit{place-form} with a possibly new property list
       obtained by combining the results of the evaluations and the access
\end{itemize}

    If the evaluation of \textit{newvalue-form} alters what is found in the 
    given \textit{place}---such as setting a different named property in the
    list---then the change of the property denoted by \textit{ind-form} will be to that 
    altered list, because the access step is done after the \textit{newvalue-form}
    evaluation.   Nevertheless, the 
    evaluations required for binding the temporaries are done before the
    evaluation of the \textit{newvalue-form}, thereby preserving
    the required left-to-right evaluation order.
...
named \textit{name}.  An example of such a macro is \cdf{incf}.  The first
subform of the macro will be a generalized-variable reference.
The \textit{function} is literally the function to apply to the old contents of the
generalized-variable to get the new contents; it is not evaluated.
\textit{lambda-list} describes
the remaining arguments for the \textit{function}; these arguments come from
the remaining subforms of the macro after the generalized-variable reference.
\textit{lambda-list} may contain \cd{\&optional} and \cd{\&rest} markers.
(The \cd{\&key} marker is not permitted here; \cd{\&rest} suffices for the purposes
of \cdf{define-modify-macro}.)
\textit{doc-string} is documentation for the macro \textit{name} being defined.
...
\begin{lisp}
(defmacro \textit{name} (\textit{reference} . \textit{lambda-list}) \\
~~\textit{doc-string} \\
~~{\Xbq}(setf ,\textit{reference} \\
~~~~~~~~~(\textit{function} ,\textit{reference} ,\textit{arg1} ,\textit{arg2} ...)))
\end{lisp}
where \textit{arg1}, \textit{arg2}, ..., are the parameters appearing in \textit{lambda-list};
...
of the form \cd{(\textit{access-fn} ...)}.  The value of a generalized-variable
reference can always be obtained simply by evaluating it, so \textit{access-fn}
...
\cdf{defsetf} requires that \textit{access-fn} be a function or a macro
that evaluates its arguments, behaving like a function.
Furthermore, a \cdf{setf} of a call on \textit{access-fn} will also evaluate
all of \textit{access-fn}'s arguments; it cannot treat any of them specially.
...
The simple form is
\begin{lisp}
(defsetf \textit{access-fn} \textit{update-fn} \Mopt{\textit{doc-string}})
\end{lisp}
The \textit{update-fn} must name a function (or macro) that takes one more argument
than \textit{access-fn} takes.  When \cdf{setf} is given a \textit{place}
that is a call on \textit{access-fn}, it expands into
a call on \textit{update-fn} that is given all the arguments to
\textit{access-fn} and also, as its last argument, the new value
(which must be returned by \textit{update-fn} as its value).
...
\begin{lisp}
(defsetf \textit{access-fn} \textit{lambda-list} (\textit{store-variable}) . \textit{body})
\end{lisp}
and resembles \cdf{defmacro}.  The \textit{body} must
compute the expansion of a \cdf{setf} of a call on \textit{access-fn}.

The \textit{lambda-list} describes the arguments of \textit{access-fn}.  \cd{\&optional},
\cd{\&rest}, and \cd{\&key} markers are permitted in \textit{lambda-list}.
Optional arguments may
have defaults and ``supplied-p'' flags.  The \textit{store-variable} describes the
value to be stored into the generalized-variable reference.

\beforenoterule
\begin{rationale}
The \textit{store-variable} is enclosed
...
The \textit{body} forms can be written as if the variables in the \textit{lambda-list}
were bound to subforms of the call on \textit{access-fn} and the
\textit{store-variable} were bound to the second subform of \cdf{setf}.
However, this is not actually the case.  During the evaluation of the
\textit{body} forms, these variables are bound to names of temporary variables,
...
permits the
\textit{body} forms to be written without regard for order-of-evaluation
...
Note that the code generated by the \textit{body} forms must include provision
for returning the correct value (the value of \textit{store-variable}).  This is
handled by the \textit{body} forms rather than by \cdf{defsetf} because
...
whose name is the same as the \textit{name} of the \textit{access-fn}.
...
\begin{itemize}
\item
A list of \textit{temporary variables}

\item
A list of \textit{value forms} (subforms of the given form)
to whose values the temporary variables are to be bound

\item
A second list of temporary variables, called \textit{store variables}

\item
A \textit{storing form}

\item
An \textit{accessing form}
\end{itemize}
...
The store variables are to be bound to the values of the \textit{newvalue} form,
...
\item
For \cd{(subseq \textit{seq} \textit{s} \textit{e})}:
\begin{lisp}
(g0004 g0005 g0006) \\*
(\textit{seq} \textit{s} \textit{e}) \\*
(g0007) \\
...
\cd{(\textit{access-fn}...)}.  The value of a generalized-variable reference can
always be obtained simply by evaluating it, so \textit{access-fn} should be the
name of a function or a macro.

The \textit{lambda-list} describes the subforms of the generalized-variable
reference, as with \cdf{defmacro}.  The result of evaluating the
\textit{forms} in the body must be five values representing
...
\textit{lambda-list} are bound to parts of the generalized-variable reference,
...
restriction that \textit{access-fn} must be a function or a function-like
...
\textit{lambda-list} in the same manner as for \cdf{defmacro} in order
...
might be applicable to the \textit{place} form that appears as the second
...
five values constituting the \cdf{setf} method for \textit{form}.
The \textit{form} must be a
...
The \textit{form} must be a
generalized-variable reference.
The \textit{env} must be an environment of the sort obtained through
the \cd{\&environment} lambda-list keyword; if \textit{env} is \cdf{nil} or omitted,
...
returns five values constituting the \cdf{setf} method for \textit{form}.
The \textit{form} must be a
...
macros that modify a \cdf{setf} \textit{place} (for example,
...
This applies \textit{function} to a list of arguments.
\begin{obsolete}
The \textit{function} may be a
...
\end{obsolete}
\begin{newer}
X3J13 voted in June 1988 \issue{FUNCTION-TYPE} to allow the \textit{function}
...
\end{newer}
The arguments for the \textit{function} consist of the last argument
to \cdf{apply} appended to the end of a list of all the other
arguments to \cdf{apply} but the \textit{function} itself;
it is as if all the arguments to \cdf{apply} except the \textit{function}
...
\cd{(funcall \textit{fn} \textit{a1} \textit{a2} ... \textit{an})}
applies the function \textit{fn} to the arguments
\textit{a1}, \textit{a2}, ..., \textit{an}.
The \textit{fn} may not
be a special form or a macro; this would not be meaningful.

\begin{newer}
X3J13 voted in June 1988 \issue{FUNCTION-TYPE} to allow the \textit{fn}
to be only of type \cdf{symbol} or \cdf{function}; a lambda-expression
is no longer acceptable as a functional argument.  One must use the
\cdf{function} special form or the abbreviation \cd{\#'} before
a lambda-expression that appears as an  explicit argument form.
...
One says that all the forms but the last are evaluated for \textit{effect},
because their execution is useful only for the side effects caused,
but the last form is executed for \textit{value}.
...
\cd{prog1} is similar to \cdf{progn}, but it returns the value of
its \textit{first} form.  All the argument forms are executed sequentially;
...
effects and to return a value that must be computed \textit{before} the side
effects happen.
For example:
\begin{lisp}
(prog1 (car x) (rplaca x 'foo))
\end{lisp}
alters the \textit{car} of \cdf{x} to be \cdf{foo} and returns the old \textit{car}
of \cdf{x}.

\cd{prog1} always returns a single value, even if the first form
tries to return multiple values.
As a consequence,
\cd{(prog1 \textit{x})} and \cd{(progn \textit{x})} may behave differently
if \textit{x} can produce multiple values.  See \cd{multiple-value-prog1}.
A point of style:
...
its \textit{second} form.  All the argument forms are executed sequentially;
...
\begin{lisp}
(prog2 \textit{a} \textit{b} \textit{c} ... \textit{z}) \EQ\ (progn \textit{a} (prog1 \textit{b} \textit{c} ... \textit{z}))
\end{lisp}
...
\`;{\rm value is that of \cdf{process-the-file}}
\end{lisp}
...
\cd{(prog2 \textit{x} \textit{y})} and \cd{(progn \textit{x} \textit{y})} may behave differently
if \textit{y} can produce multiple values.
\end{defmac}
...
Instead of a list \cd{(\textit{varj} \textit{valuej})}, one may write simply
\textit{varj}.  In this case \textit{varj} is initialized to {\false}.  As a matter
of style, it is recommended that \textit{varj} be written only when that
...
\cd{(\textit{varj} {\false})} if the initial value is intended to mean ``false,'' or
\cd{(\textit{varj} '{\emptylist})} if the initial value is intended to be an empty
...
to regularize the binding formats for \cdf{do}, \cd{do*}, \cdf{let},
\cd{let*}, \cdf{prog}, \cd{prog*}, and \cdf{compiler-let}.
The new syntactic definition for \cdf{let} makes the \textit{value} optional:

\begin{defmac}
let ({var | (var [value])}*) {declaration}* {\,form}*

This changes \cdf{let} to allow a list \cd{(\textit{var})} to appear,
...
\cd{let*} is similar to \cdf{let}, but the bindings of variables
...
in order; the values of all but the last are discarded
(that is, the body of a \cd{let*} form is an implicit \cdf{progn}).
The \cd{let*} form returns the results of evaluating \textit{bodyn} (if the
body is empty, which is fairly useless, \cd{let*} returns {\false} as its value).
The bindings of the variables have lexical scope and indefinite extent.

Instead of a list \cd{(\textit{varj} \textit{valuej})}, one may write simply \textit{varj}.
...
to regularize the binding formats for \cdf{do}, \cd{do*}, \cdf{let},
\cd{let*}, \cdf{prog}, \cd{prog*}, and \cdf{compiler-let}.
The new syntactic definition for \cd{let*} makes the \textit{value} optional:

\begin{defmac}
let* ({var | (var [value])}*) {declaration}* {\,form}*

This changes \cd{let*} to allow a list \cd{(\textit{var})} to appear,
meaning the same as simply \textit{var}.
\end{defmac}
\end{new}
\end{defspec}
...
to regularize the binding formats for \cdf{do}, \cd{do*}, \cdf{let},
\cd{let*}, \cdf{prog}, \cd{prog*}, and \cdf{compiler-let}.
The new syntactic definition for \cdf{compiler-let} makes the \textit{value} optional:

\begin{defmac}
compiler-let ({var | (var [value])}*) {\,form}*

This changes \cdf{compiler-let} to allow a list \cd{(\textit{var})} to appear,
meaning the same as simply \textit{var}.
\end{defmac}
\end{defspec}
\end{obsolete}
...
functions defined by \cdf{macrolet} are defined in the \textit{global} environment;
...
However,
lexically scoped entities \textit{are} visible
within the body of the \cdf{macrolet} form and \textit{are} visible
...
(defun foo (x flag) \\*
~~(macrolet ((fudge (z) \\*
~~~~~~~~~~~~~~~~;;{\rm The parameters \cdf{x} and \cdf{flag} are not accessible} \\*
~~~~~~~~~~~~~~~~;; {\rm at this point; a reference to \cdf{flag} would be to} \\*
~~~~~~~~~~~~~~~~;; {\rm the global variable of that name.} \\*
~~~~~~~~~~~~~~~~{\Xbq}(if flag \\
~~~~~~~~~~~~~~~~~~~~~(* ,z ,z) \\
~~~~~~~~~~~~~~~~~~~~~,z))) \\
~~~~;;{\rm The parameters \cdf{x} and \cdf{flag} are accessible here.} \\*
~~~~(+ x \\*
~~~~~~~(fudge x) \\*
~~~~~~~(fudge (+ x 1)))))
\end{lisp}
...
whose name is the same as the \textit{name} of the function.
Therefore \cdf{return-from} may be used to exit from the function.
\end{newer}
...
to accept any function-name (a symbol or a list
whose \textit{car} is \cdf{setf}---see section~\ref{FUNCTION-NAME-SECTION}) as a \textit{name}
...
the locally defined macro expander functions are \textit{not} included in
the scope of pervasive declarations appearing before the main body.
...
The \textit{forms} are executed as an implicit \cdf{progn} in a lexical
environment that causes every reference to any defined \textit{var}
to be replaced by the corresponding \textit{expansion}.  It is as if
the reference to the \textit{var} were a parameterless macro call;
the \textit{expansion} is evaluated or otherwise processed
...
not substitute for all occurrences of a \textit{var} as a variable
but only for those occurrences that would be construed as
references in the scope of a lexical binding of \textit{var} as
...
The \textit{expansion} for each \textit{var} is not evaluated at binding time
but only after it has replaced a reference to the \textit{var}.
The \cdf{setf} macro allows a symbol macro to be used as a \textit{place},
...
See \cdf{macroexpand} and \cd{macroexpand-1}; they will expand symbol
macros as well as ordinary macros.

Certain \textit{declarations} before the body are handled in a peculiar manner;
...
First the form \textit{test} is evaluated.  If the result is not {\false},
then the form \textit{then} is selected; otherwise the form \textit{else} is selected.
Whichever form is selected is then evaluated, and \cdf{if} returns
whatever is returned by evaluation of the selected form.
\begin{lisp}
(if \textit{test} \textit{then} \textit{else}) \EQ\ (cond (\textit{test} \textit{then}) ({\true} \textit{else}))
\end{lisp}
but \cdf{if} is considered more readable in some situations.

The \textit{else} form may be omitted, in which case if the value of \textit{test}
...
\cd{(when \textit{test} \textit{form1} \textit{form2} ... )}
first evaluates \textit{test}.  If the result is {\false},
then no \textit{form} is evaluated, and {\false} is returned.
Otherwise the \textit{form}s constitute an implicit \cdf{progn}
and are evaluated sequentially from left to right,
and the value of the last one is returned.
\begin{lisp}
(when \textit{p} \textit{a} \textit{b} \textit{c}) \EQ\ (and \textit{p} (progn \textit{a} \textit{b} \textit{c})) \\
(when \textit{p} \textit{a} \textit{b} \textit{c}) \EQ\ (cond (\textit{p} \textit{a} \textit{b} \textit{c})) \\
(when \textit{p} \textit{a} \textit{b} \textit{c}) \EQ\ (if \textit{p} (progn \textit{a} \textit{b} \textit{c}) {\false}) \\
(when \textit{p} \textit{a} \textit{b} \textit{c}) \EQ\ (unless (not \textit{p}) \textit{a} \textit{b} \textit{c})
\end{lisp}
...
\cd{(unless \textit{test} \textit{form1} \textit{form2} ... )}
first evaluates \textit{test}.  If the result is \textit{not} {\false},
then the \textit{form}s are not evaluated, and {\false} is returned.
Otherwise the \textit{form}s constitute an implicit \cdf{progn}
and are evaluated sequentially from left to right,
and the value of the last one is returned.
\begin{lisp}
(unless \textit{p} \textit{a} \textit{b} \textit{c}) \EQ\ (cond ((not \textit{p}) \textit{a} \textit{b} \textit{c})) \\
(unless \textit{p} \textit{a} \textit{b} \textit{c}) \EQ\ (if \textit{p} {\false} (progn \textit{a} \textit{b} \textit{c})) \\
(unless \textit{p} \textit{a} \textit{b} \textit{c}) \EQ\ (when (not \textit{p}) \textit{a} \textit{b} \textit{c})
\end{lisp}
As a matter of style,
...
\textit{clauses}, which are lists of forms.
Each clause consists of a \textit{test} followed
by zero or more \textit{consequents}.
For example:
\begin{lisp}
(cond (\textit{test-1} \textit{consequent-1-1} \textit{consequent-1-2} ...) \\
~~~~~~(\textit{test-2}) \\
~~~~~~(\textit{test-3} \textit{consequent-3-1} ...) \\
~~~~~~... )
\end{lisp}

The first clause whose \textit{test} evaluates to non-{\false}
...
right.  For each clause, the \textit{test} is evaluated.  If the result is
{\false}, \cdf{cond} advances to the next clause.  Otherwise, the \textit{cdr} of
...
the selected clause,
then the single (and necessarily non-null) value of the \textit{test} is returned.
...
(Note moreover that \cd{(cond ... (\textit{x}))} may behave differently from
\cd{(cond ... ({\true} \textit{x}))} if \textit{x} might produce multiple values;
...
to obtain this behavior by writing \cd{(cond ... (t (values \textit{x})))},
...
\begin{lisp}
(case \textit{keyform} \\
~~(\textit{keylist-1} \textit{consequent-1-1} \textit{consequent-1-2} ...) \\
~~(\textit{keylist-2} \textit{consequent-2-1} ...) \\
~~(\textit{keylist-3} \textit{consequent-3-1} ...) \\
~~...)
\end{lisp}
...
The first thing \cdf{case} does is to evaluate the form \textit{keyform}
to produce an object called the \textit{key object}.
Then \cdf{case} considers
each of the clauses in turn.  If \textit{key} is in the \textit{keylist}
(that is, is \cdf{eql} to any item in the \textit{keylist}) of a clause,
...
The keys in the keylists are \textit{not} evaluated; literal key values
...
Instead of a \textit{keylist}, one may write one of the symbols
...
\begin{lisp}
(typecase \textit{keyform} \\*
~~(\textit{type-1} \textit{consequent-1-1} \textit{consequent-1-2} ...) \\*
~~(\textit{type-2} \textit{consequent-2-1} ...) \\*
~~(\textit{type-3} \textit{consequent-3-1} ...) \\
~~...)
\end{lisp}
...
for \textit{type} to indicate that the clause should always be selected.
...
The \cdf{block} construct executes each \textit{form} from left to right,
returning whatever is returned by the last \textit{form}.
If, however, a \cdf{return} or \cdf{return-from} form that specifies the
same \textit{name} is executed
during the execution of some \textit{form}, then the results
...
The \textit{name} is not evaluated; it must be a symbol.
The scope of the \textit{name} is lexical; only a \cdf{return} or \cdf{return-from}
textually contained in some \textit{form} can exit from the block.
...
The \textit{name} is not evaluated and must be a symbol.
A \cdf{block} construct with the same name must lexically
enclose the occurrence of \cdf{return-from};
whatever the evaluation of \textit{result} produces
is immediately returned from the block.
(If the \textit{result} form is omitted, it defaults to {\nil}.
...
If the evaluation of \textit{result} produces multiple values,
...
\cd{(return \textit{form})} is identical in meaning
to \cd{(return-from {\nil} \textit{form})}; it returns from a block named {\nil}.
...
over the elements of a list or \textit{n} consecutive integers, \cdf{dolist} and
...
Each \textit{form} is evaluated in turn from left to right.
When the last \textit{form} has been evaluated, then the first \textit{form}
...
\begin{obsolete}
A \cdf{loop} construct has this meaning only if every \textit{form} is
non-atomic (a list).  The case where some \textit{form} is
...
In contrast to \cdf{loop}, \cdf{do} and \cd{do*} provide a powerful
...
\begin{lisp}
(do ((\textit{var1} \textit{init1} \textit{step1}) \\
~~~~~(\textit{var2} \textit{init2} \textit{step2}) \\
~~~~~... \\
~~~~~(\textit{varn} \textit{initn} \textit{stepn})) \\
~~~~(\textit{end-test} . \textit{result}) \\
~~\Mstar{\textit{declaration}} \\
~~. \textit{tagbody})
\end{lisp}
A \cd{do*} loop looks exactly the same except that the name \cdf{do} is
replaced by \cd{do*}.
...
variable \textit{var}, an initial value \textit{init},
and a stepping form \textit{step}.
If \textit{init} is omitted, it defaults to {\false}.
If \textit{step} is omitted, the \textit{var} is not changed by the \cdf{do} construct
...
to regularize the binding formats for \cdf{do}, \cd{do*}, \cdf{let},
\cd{let*}, \cdf{prog}, \cd{prog*}, and \cdf{compiler-let}.
In the case of \cdf{do} and \cd{do*} the first edition was inconsistent;
...
For \cdf{do}, \textit{all} of the \textit{init} forms are evaluated \textit{before} any \textit{var}
...
For \cd{do*}, the first \textit{init} form is evaluated, then the first
...
{\false}, execution proceeds with the body of the \cdf{do} (or \cd{do*}) form.
If the
...
If there are no \textit{result} forms, the value of \cdf{do} is {\false}.
...
index variables are updated as follows.  All the \textit{step} forms
are evaluated, from left to right, and the resulting values are
assigned to the respective index variables.
Any variable that has no associated \textit{step} form is not assigned to.
For \cdf{do}, all the \textit{step} forms are evaluated before any variable
is updated; the assignment of values to variables is done in parallel,
as if by \cdf{psetq}.
Because \textit{all} of the \textit{step} forms are evaluated before \textit{any}
of the variables are altered, a \textit{step} form when evaluated always has
access to the \textit{old} values of \textit{all} the index variables,
even if other \textit{step} forms precede it.
For \cd{do*}, the first \textit{step} form is evaluated, then the
value is assigned to the first \textit{var}, then the second \textit{step} form
is evaluated, then the value is assigned to the second \textit{var}, and so on;
the assignment of values to variables is done sequentially,
as if by \cdf{setq}.
For either \cdf{do} or \cd{do*},
after the variables have been updated,
the \textit{end-test} is evaluated as described above, and the iteration continues.

If the \textit{end-test} of a \cdf{do} form is \cd{{\false}},
the test will never succeed.
Therefore this provides an idiom for ``do forever'':
the \textit{body} of the \cdf{do} is executed repeatedly, stepping variables
...
statements may not appear in the variable specifiers, the \textit{end-test},
or the \textit{result} forms).
When the end of a \cdf{do} body is reached, the next iteration cycle
(beginning with the evaluation of \textit{step} forms) occurs.
...
variables, to the \textit{init} forms, to the \textit{step} forms,
to the \textit{end-test}, and to the \textit{result} forms.

\beforenoterule
\begin{incompatibility}
``Old-style'' MacLisp \cdf{do} loops, that is, those
of the form \cd{(do \textit{var} \textit{init} \textit{step} \textit{end-test} . \textit{body})},
...
\begin{lisp}
(do ((i 0 (+ i 1))~~~~~;{\rm Sets every null element of \cdf{a-vector} to zero} \\*
~~~~~(n (length a-vector))) \\*
~~~~((= i n)) \\
...
in the \textit{step} forms of a \cdf{do}, and the \textit{body} is empty.
For example,
\begin{lisp}
(do ((x foo (cdr x)) \\*
~~~~~(y bar (cdr y)) \\*
...
~~~~~(nreverse z)))
\end{lisp}
does the same thing as \cd{(mapcar \#'f foo bar)}.  Note that the \textit{step}
...
of conses.  The \textit{car} of each cons is a list of symbols,
and the \textit{cdr} of each cons is a list of equal length containing
...
\end{lisp}
\cd{do*} is exactly like \cdf{do} except that the bindings and steppings
of the variables are performed sequentially rather than in parallel.
It is as if, in the above explanation,
\cdf{let} were replaced by \cd{let*} and \cdf{psetq} were replaced
by \cdf{setq}.
\end{defmac}
...
and \cdf{dotimes} examines integers from 0 to $\textit{n}-1$
...
First \cdf{dolist}
evaluates the form \textit{listform},
which should produce a list.  It then executes the body
once for each element in the list, in order, with
the variable \textit{var} bound to the element.
Then \textit{resultform} (a single form, \textit{not} an implicit \cdf{progn})
is evaluated, and the result is the value of the \cdf{dolist}
form.  (When the \textit{resultform} is evaluated, the control variable \textit{var}
is still bound and has the value {\nil}.)
If \textit{resultform} is omitted, the result is {\false}.
...
The expression
\cd{(dotimes (\textit{var} \textit{countform} \textit{resultform}) . \textit{progbody})}
evaluates the form \textit{countform}, which should produce an integer.  It then
performs \textit{progbody} once for each integer from zero (inclusive) to
\textit{count} (exclusive), in order, with the variable \textit{var} bound to the
integer; if the value of \textit{countform} is zero or negative,
then the \textit{progbody} is
performed zero times.  Finally, \textit{resultform} (a single form, \textit{not} an
implicit \cdf{progn}) is evaluated, and the result is the value of the
\cdf{dotimes} form.  (When the \textit{resultform} is evaluated, the control
variable \textit{var} is still bound and has as its value the number of times
...
Altering the value of \textit{var} in the body of the loop (by using \cdf{setq},
...
so are \cdf{mapcan} and \cdf{mapcon}; the lists returned by the \textit{function}
...
The part of a \cdf{tagbody} after the variable list is called the \textit{body}.
An item in the body may be a symbol or an integer, in which case it is called a
\textit{tag}, or an item in the body may be a list, in which case it is called a 
\textit{statement}.

Each element of the body is processed from left to right.
A \textit{tag} is ignored; a \textit{statement}
is evaluated, and its results are discarded.  If the end of the body
is reached, the \cdf{tagbody} returns {\false}.

If \cd{(go \textit{tag})} is evaluated, control jumps to the part of the body
labelled with the \textit{tag}.
...
been exited, it is no longer legal to \cdf{go} to a \textit{tag} in its body.
...
Any \textit{declaration} appearing in the \cdf{prog} is used as if appearing
...
~~(prog (x y z)~~~~~;{\rm Initialize \cdf{x}, \cdf{y}, \cdf{z} to {\false}} \\
...
follows:
\begin{lisp}
(prog \textit{variable-list} \Mstar{\textit{declaration}} . \textit{body}) \\
~~~\EQ\ (block nil (let \textit{variable-list} \Mstar{\textit{declaration}} (tagbody . \textit{body})))
\end{lisp}

The \cd{prog*} special form is almost the same as \cdf{prog}.  The only
difference is that the binding and initialization of the temporary
variables is done \textit{sequentially}, so that the \textit{init} form for each
one can use the values of previous ones.
Therefore \cd{prog*} is to \cdf{prog} as \cd{let*} is to \cdf{let}.
For example,
\begin{lisp}
(prog* ((y z) (x (car y))) \\
~~~~~~~(return x))
\end{lisp}
returns the \textit{car} of the value of \cdf{z}.
...
The \cd{(go \textit{tag})} special form is used to do a ``go to'' within
a \cdf{tagbody} construct.  The \textit{tag} must be a symbol or an integer;
the \textit{tag} is not evaluated.
...
For list traversal operations, the \textit{cdr} chain
...
\textit{except} that the very entry being processed by user code
...
any packages they use, \textit{except} that the very symbol
...
not only \textit{cdr} pointers (in order to chase down the list)
but also \textit{car} pointers (in order to obtain the elements themselves).
...
required both to \textit{produce} multiple values and to \textit{receive} them.
...
~~(values (floor (+ x y) y)))
\end{lisp}
This works because \cdf{values} returns exactly \textit{one} value for each of
...
All of the elements of \textit{list} are returned as multiple values.
For example:
\begin{lisp}
...
In general,
\begin{lisp}
(values-list \textit{list}) \EQ\ (apply \#'values \textit{list})
\end{lisp}
...
\cdf{multiple-value-list} evaluates \textit{form} and returns a list of
...
\cdf{multiple-value-call} first evaluates \textit{function} to obtain a function
and then evaluates all of the \textit{form\/}s.  All the values
of the \textit{form\/}s are gathered together (not just one value from each)
...
~~~\EQ\ (+ 1 2 4 3) \EV\ 10 \\
(multiple-value-list \textit{form}) \EQ\ (multiple-value-call \#'list \textit{form})
\end{lisp}
\end{defspec}
...
\cd{multiple-value-prog1} evaluates the first \textit{form} and saves all the values
produced by that form.  It then evaluates the other \textit{form}s
from left to right, discarding their values.  The values produced
by the first \textit{form} are returned by \cd{multiple-value-prog1}.  See \cd{prog1},
...
The \textit{values-form} is evaluated, and each of the variables \textit{var} is
...
The \textit{variables} must be a list of variables.  The \textit{form} is
evaluated, and the variables are \textit{set} (not bound) to the values
...
value returned by \textit{form}, or {\false} if \textit{form} produces zero values.
...
The argument forms \textit{n} and \textit{form} are both evaluated.
The value of \textit{n} must be a non-negative integer,
and the \textit{form} may produce any number of values.
The integer \textit{n} is used as a zero-based index into the
list of values.
Value \textit{n} of the \textit{form} is returned as the
single value of the \cdf{nth-value} form; \cdf{nil} is returned if the
\textit{form} produces no more than \textit{n} values.
...
\end{lisp}
Value number 1 is the \textit{second} value returned by \cdf{floor},
...
list of all the values of the \textit{form}.
\end{defmac}
\end{new}
...
\goodbreak
\begin{flushdesc}
\item[\textit{Evaluation and application}]\leavevmode
\begin{itemize}
\item
...
\item[\textit{Implicit \cdf{progn} contexts}]\leavevmode
\begin{itemize}
\item
...
\begin{flushdesc}
\item[\textit{Conditional constructs}]\leavevmode
\begin{itemize}
\item
\cdf{if} passes back multiple values from whichever subform is selected
(the \textit{then} form or the \textit{else} form).
...
the last clause of the \cdf{cond}.  It is \textit{not} permitted to
...
\item[\textit{Returning from a block}]\leavevmode
\begin{itemize}
\item
...
pass back multiple values from the \textit{resultform} if that is executed.
These situations are all examples of implicit uses of \cdf{return-from}.
\end{itemize}

\item[\textit{Throwing out of a catch}]\leavevmode
\begin{itemize}
\item
...
\item[\textit{Miscellaneous situations}]\leavevmode
\begin{itemize}
\item
...
Among special forms that \textit{never} pass back multiple values are
\cd{prog1},
\cd{prog2}, \cdf{setq}, and \cdf{multiple-value-setq}.
...
receive \textit{exactly} one argument (which is of course an error),
...
each argument form produces exactly \textit{one} argument;  if such a form
...
special forms for this purpose, called \textit{catch} forms and \textit{throw}
forms, or simply \textit{catches} and \textit{throws}.  A catch form evaluates some
...
The form \textit{tag} is evaluated first to produce an object
that names the catch; it may be any Lisp object.
A catcher is then established with the object as the tag.
The \textit{form\/}s are evaluated as an implicit \cdf{progn},
and the results of the last form are returned,
except that if during the evaluation of the \textit{form\/}s
...
then the evaluation of the \textit{form\/}s is aborted and the results
...
\cd{(catch 'foo \textit{form})} will catch a \cd{(throw 'foo \textit{form})} but
not a \cd{(throw 'bar \textit{form})}.  It is an error if \cdf{throw} is done
...
The MacLisp syntax was \cd{(catch \textit{form} \textit{tag})},
...
the \textit{cleanup-form\/}s before exiting, whether it terminates
normally or is aborted by a throw of some kind.
(If, however, an exit occurs during execution of the \textit{cleanup-form\/}s,
no special action is taken.  The \textit{cleanup-form\/}s of an \cdf{unwind-protect}
...
the \textit{protected-form} and discards all the results
from the \textit{cleanup-form\/}s.

It should be emphasized that \cdf{unwind-protect} protects against
\textit{all} attempts to exit from the protected form,
...
Let an \textit{exit} be a point out of which control can be transferred.
...
\item Intervening dynamic bindings of special variables and catch tags
    are undone.
\item Intervening exits are \textit{abandoned}, that is, their extent ends and it
...
The MacLisp syntax was \cd{(throw \textit{form} \textit{tag})},
where the \textit{tag} was not evaluated.
\end{incompatibility}
\afternoterule
\end{defspec}
