...
\begin{new}
Локально определенные функция (\cdf{flet}, \cdf{labels}) и макросы
...
\end{lisp}
Более того, для заданнох форм \emph{place} \cdf{shiftf} может быть более
...
Каждая \emph{place} может быть обобщенной переменной, как для \cdf{setf}.
...
Более того, для заданнох форм \emph{place} \cdf{rotatef} может быть более
производительной, чем версия с \cdf{prog1}.
...
В ссылках на обобщенные переменные, как в \cdf{shiftf}, \cdf{incf}, \cdf{push} и
\cdf{setf} или \cdf{ldb}, обобщенные переменные считываются и записываются в
...
общий механизм, \cdf{symbol-macrolet}, для обработки заданных имен переменным,
...
В качестве примера, \cdf{mod} мог бы быть определен так:
\begin{lisp}
(defun mod (number divisor) \\*
~~(nth-value 1 (floor number divisor)))
\end{lisp}
