...
\indexterm{symbols}{implicit \cdf{progn}}
...
\begin{lisp}
(let ((\#:temp1 \textit{arg1})~~~~~;\textrm{Force correct order of evaluation} \\*
~~~~~~(\#:temp2 \textit{arg2}) \\*
~~~~~~... \\*
~~~~~~(\#:temp0 newvalue)) \\*
...
if \emph{x} can produce multiple values.  See \cd{multiple-value-prog1}.
A point of style:
...
The \cd{let*} form returns the results of evaluating \emph{bodyn} (if the
...
to regularize the binding formats for \cdf{do}, \cd{do*}, \cdf{let},
\cd{let*}, \cdf{prog}, \cd{prog*}, and \cdf{compiler-let}.
The new syntactic definition for \cd{let*} makes the \emph{value} optional:

\begin{defmac}
let* ({var | (var [value])}*) {declaration}* {\,form}*

This changes \cd{let*} to allow a list \cd{(\emph{var})} to appear,
meaning the same as simply \emph{var}.
\end{defmac}
\end{new}
\end{defspec}
...
For \cd{do*}, the first \emph{init} form is evaluated, then the first
...
For \cd{do*}, the first \emph{step} form is evaluated, then the
...
the \emph{end-test} is evaluated as described above, and the iteration continues.
...
and \cdf{dotimes} examines integers from 0 to $\emph{n}-1$
...
\indexterm{symbols}{multiple values}
...
\cd{multiple-value-prog1} evaluates the first \emph{form} and saves all the values
...
by the first \emph{form} are returned by \cd{multiple-value-prog1}.  See \cd{prog1},
...
\indexterm{symbols}{non-local exit}
\indexterm{symbols}{dynamic exit}
\indexterm{symbols}{catch}
\indexterm{symbols}{throw}
...
\indexterm{symbols}{unwind protection}
\indexterm{symbols}{cleanup handler}
\begin{defspec}
