...
In general, characters in Common Lisp are not true objects; \cd{eq} cannot
be counted upon to operate on them reliably.  In particular,
it is possible that the expression
\begin{lisp}
(let ((x z) (y z)) (eq x y))
\end{lisp}
may be false rather than true, if the value of \cd{z} is a character.

\beforenoterule
\begin{rationale}
This odd breakdown of \cd{eq} in the case of characters
...
a character, then the predicate \cd{eql} is probably appropriate.
...
all of the characters in the repertoire \cd{standard-char}
...
as well as repertoires named \cd{base-character}, \cd{extended-character},
and \cd{character}.  Other repertoires may be supported as well.
X3J13 voted in June 1989 \issue{MORE-CHARACTER-PROPOSAL} to specify that names of
repertoires may be used as type specifiers.  Such types must be subtypes of \cd{character};
that is, in a given implementation
the repertoire named \cd{character} must encompass all the character objects supported
...
any canonicalization performed by \cd{format} in processing directives;
the behavior of \cd{char-upcase}, \cd{char-downcase}, and the predicates
\cd{alpha-char-p}, \cd{upper-case-p}, \cd{lower-case-p}, \cd{both-case-p},
\cd{graphic-char-p}, \cd{alphanumericp}, \cd{char-equal}, \cd{char-not-equal},
\cd{char-lessp}, \cd{char-greaterp}, \cd{char-not-greaterp}, and \cd{char-not-lessp}
...
The value of \cd{char-code-limit} is a non-negative
integer that is the upper exclusive bound on values produced
by the function \cd{char-code}, which returns the {\it code} component
of a given character; that is, the values returned by \cd{char-code}
...
zero and the value of \cd{char-code-limit} are valid character codes, and so
...
The value of \cd{char-font-limit} is a non-negative
integer that is the upper exclusive bound on values produced
by the function \cd{char-font}, which returns the {\it font} component
of a given character; that is, the values returned by \cd{char-font}
are non-negative and strictly less than the value of
\cd{char-font-limit}.
...
non-zero font attributes; if it does not, then \cd{char-font-limit}
should be \cd{1}.
\end{implementation}
\afternoterule
\end{defun}
...
\begin{newer}
X3J13 voted in March 1989 \issue{CHARACTER-PROPOSAL}
to eliminate \cd{char-font-limit}.
...
The value of \cd{char-bits-limit} is a non-negative
integer that is the upper exclusive bound on values produced
by the function \cd{char-bits}, which returns the {\it bits} component
of a given character; that is, the values returned by \cd{char-bits}
are non-negative and strictly less than the value of
\cd{char-bits-limit}.  Note that the value of \cd{char-bits-limit}
will be a power of 2.

\beforenoterule
\begin{implementation}
No Common Lisp implementation is required to support
non-zero bits attributes; if it does not, then \cd{char-bits-limit}
should be \cd{1}.
\end{implementation}
\afternoterule
\end{defun}
...
\begin{newer}
X3J13 voted in March 1989 \issue{CHARACTER-PROPOSAL}
to eliminate \cd{char-bits-limit}.
\end{newer}

\section{Predicates on Characters}

The predicate \cd{characterp} may be used to determine
...
\cd{standard-char-p} is true if the argument is a ``standard character,''
that is, an object of type \cd{standard-char}.
...
The argument {\it char} must be a character object.
\cd{graphic-char-p} is true if the argument is a ``graphic'' (printing)
...
as a single glyph, such as \cd{A} or \cd{*} or \cd{=}.
...
The argument {\it char} must be a character object.
\cd{string-char-p} is true if {\it char} can be stored into
a string, and otherwise is false.
Any character that satisfies \cd{standard-char-p}
also satisfies \cd{string-char-p}; others may also.
\end{defun}
\end{obsolete}
...
The argument {\it char} must be a character object.
\cd{alpha-char-p} is true if the argument is an alphabetic
...
Of the standard characters (as defined by \cd{standard-char-p}),
the letters \cd{A} through \cd{Z} and \cd{a} through \cd{z} are alphabetic.
\end{defun}
...
\cd{upper-case-p} is true if the argument is an uppercase
character, and otherwise is false.

\cd{lower-case-p} is true if the argument is a lowercase
character, and otherwise is false.

\cd{both-case-p} is true if the argument is an uppercase character and
there is a corresponding lowercase character (which can be obtained
using \cd{char-downcase}), or if the argument is a lowercase character and
there is a corresponding uppercase character (which can be obtained
using \cd{char-upcase}).
...
Of the standard characters (as defined by \cd{standard-char-p}),
the letters \cd{A} through \cd{Z} are uppercase and \cd{a}
through \cd{z} are lowercase.
\end{defun}
...
specified by {\it radix}, then \cd{digit-char-p} is
...
Of the standard characters (as defined by \cd{standard-char-p}),
the characters \cd{0} through \cd{9}, \cd{A} through \cd{Z},
and \cd{a} through \cd{z}
are digits.  The weights of \cd{0} through \cd{9} are the integers 0 through 9,
and of \cd{A} through \cd{Z} (and also \cd{a} through \cd{z}) are 10 through 35.
\cd{digit-char-p} returns the weight for one of these digits if and only if
...
Here is an example of the use of \cd{digit-char-p}:
\begin{lisp}
...
The argument {\it char} must be a character object.
\cd{alphanumericp} is true if {\it char} is either alphabetic
...
(as defined by the predicate \cd{graphic-char-p}).

Of the standard characters (as defined by \cd{standard-char-p}),
the characters \cd{0} through \cd{9}, \cd{A} through \cd{Z},
and \cd{a} through \cd{z} are alphanumeric.
\end{defun}
...
then their ordering by \cd{char<} is consistent with the numerical
...
for \cd{char>}, \cd{char<=}, and \cd{char>=}.

\item
If two characters differ in any implementation-defined
attribute, then they are not \cd{char=}.
\end{itemize}
\end{newer}

The total ordering is not necessarily the same as the total
ordering on the integers produced by applying \cd{char-int} to the
...
\#{\Xbackslash}z)} is {\it not} a valid way of determining whether or not \cd{x} is a
lowercase letter.  That is why a separate
\cd{lower-case-p} predicate is provided.
...
\cd{(char= c1 c2)} is true.  While \cd{eq} may distinguish two character
objects that \cd{char=} does not, it is distinguishing them not
...
However, \cd{eql} and \cd{equal}
compare character objects in the same
way that \cd{char=} does.
\end{defun}
...
\begin{obsolete}
The predicate \cd{char-equal} is like \cd{char=}, and similarly
...
\cd{char-equal}, \cd{char-not-equal}, \cd{char-lessp}, \cd{char-greaterp},
\cd{char-not-greaterp}, and \cd{char-not-lessp} must be specified
...
lowercase \cd{p} in some font).  Assuming italics to be in font 1
...
The argument {\it char} must be a character object.
\cd{char-code} returns the code attribute of the character object;
this will be a non-negative integer less than the (normal) value of
the variable \cd{char-code-limit}.
...
equal to \cd{char-code-limit}.  Be careful how you initialize this
vector; remember that you cannot necessarily
expect all non-negative integers less than
\cd{char-code-limit} to be valid character codes.
\end{new}
\end{defun}
...
The argument {\it char} must be a character object.
\cd{char-bits} returns the bits attribute of the character object;
this will be a non-negative integer less than the (normal) value of
the variable \cd{char-bits-limit}.
\end{defun}
\end{obsolete}

\begin{newer}
X3J13 voted in March 1989 \issue{CHARACTER-PROPOSAL}
to eliminate \cd{char-bits}.
\end{newer}
...
The argument {\it char} must be a character object.
\cd{char-font} returns the font attribute of the character object;
this will be a non-negative integer less than the (normal) value of
the variable \cd{char-font-limit}.
\end{defun}
\end{obsolete}

\begin{newer}
X3J13 voted in March 1989 \issue{CHARACTER-PROPOSAL}
to eliminate \cd{char-font}.
\end{newer}
\medskip
\begin{new}
The references to the ``normal'' values of the ``variables''
\cd{char-code-limit},
\cd{char-bits-limit}, and \cd{char-font-limit} in the descriptions
of \cd{char-code}, \cd{char-bits}, and \cd{char-font} were an oversight on
...
\end{lisp}
If the font and bits attributes of a character object \cd{c} are zero,
...
the specification of \cd{code-char}.
\end{newer}
\end{defun}
...
If {\it bits} and {\it font} are zero, then \cd{make-char} cannot fail.
...
The function \cd{character} coerces its argument to be a character
if possible; see \cd{coerce}.
\begin{lisp}
...
The argument {\it char} must be a character object.
\cd{char-upcase} attempts to convert its argument to an uppercase
equivalent; \cd{char-downcase} attempts to convert its argument
to a lowercase equivalent.

\begin{obsolete}
\cd{char-upcase} returns a character object with the same font
...
\cd{lower-case-p} is true of {\it char}, and \cd{upper-case-p}
...
\end{lisp}
Similarly,
\cd{char-downcase} returns a character object with the same font
and bits attributes as {\it char}, but with possibly a different code
attribute.  If the code is different from {\it char}'s, then the predicate
\cd{upper-case-p} is true of {\it char}, and \cd{lower-case-p}
is true of the result character.  Moreover, if \cd{(char= (char-downcase x) x)}
is {\it not} true, then it is true that
\begin{lisp}
(char= (char-upcase (char-downcase x)) x)
\end{lisp}
Note that the action of \cd{char-upcase} and \cd{char-downcase} may
...
\cd{char-upcase} and \cd{char-downcase} is to preserve
...
All arguments must be integers.  \cd{digit-char}
...
a digit of the radix {\it radix} (see the predicate \cd{digit-char-p}).
It returns such a character if that is possible, and otherwise returns {\false}.

\cd{digit-char} cannot return {\false} if {\it font} is zero,
...
(see \cd{digit-char-p}) and no graphic character has a non-zero
{\it bits} component (see \cd{graphic-char-p}).

\begin{newer}
X3J13 voted in March 1989 \issue{CHARACTER-PROPOSAL}
to eliminate the {\it font} argument from the specification of \cd{digit-char}.
\end{newer}
\end{defun}
...
The argument {\it char} must be a character object.
\cd{char-int} returns a non-negative integer encoding the character object.

If the font and bits attributes of {\it char} are zero, then
\cd{char-int} returns the same integer \cd{char-code} would.
Also,
\begin{lisp}
...
\cd{int-char} returns a character object \cd{c} such that
\cd{(char-int c)} is equal to {\it integer}, if possible; otherwise
\cd{int-char} returns false.
\end{defun}
\end{obsolete}

\begin{newer}
X3J13 voted in March 1989 \issue{CHARACTER-PROPOSAL}
to eliminate \cd{int-char}.
\end{newer}
...
(do not satisfy the predicate \cd{graphic-char-p}) have names.
Graphic characters may or may not have names.

The standard newline and space characters have the respective
names \cd{Newline} and \cd{Space}.
The semi-standard characters have the names
\cd{Tab}, \cd{Page}, \cd{Rubout}, \cd{Linefeed}, \cd{Return}, and \cd{Backspace}.
...
\cd{char-name} will only locate ``simple'' character names;
it will not construct names such as \cd{Control-Space} on the
basis of the character's bits attribute.

\begin{new}
The easiest way to get a name that includes the bits attribute of
a character \cd{c} is \cd{(format nil "{\Xtilde}:C" c)}.
\end{new}
\end{defun}
...
The argument \cd{name} must be an object coerceable to a string
as if by the function \cd{string}.
If the name is the same as the name of a character object
(as determined by \cd{string-equal}), that object
...
\cd{char-control-bit}, \cd{char-meta-bit}, \cd{char-super-bit},
and \cd{char-hyper-bit}.
\end{newer}
\medskip
\begin{new}
...
\cd{char-bit} takes a character object {\it char} and the name of a bit,
...
It is an error to give \cd{char-bit} the name of a bit not supported
by the implementation.

If the argument {\it char} is specified by a form that is a {\it place} form
acceptable to \cd{setf},
then \cd{setf} may be used with \cd{char-bit}
to modify a bit of the character stored in that
{\it place}.
The effect is to perform a \cd{set-char-bit} operation
...
\cd{char-bit} takes a character object {\it char}, the name of a bit,
