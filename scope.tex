...
Например: \cdf{with-open-file} открывает соединение с файлом и создает объект
...
результата.
Связи параметров для \cdf{f} и \cdf{g} не теряются, потому что возвращенная
...
одноименного параметра функции \cdf{test}. Ссылка на переменную \cdf{z} в форме
...
Рассматриваемая из \cd{fun3}, \cdf{catch} в \cd{fun2} скрывает одноименную в
\cd{fun1}.
...
\item Связывания имен функций устанавливаются, например, формами \cdf{flet} и
...
\item Связывания имен функций, для которых задекларировано \cdf{dynamic-extent},
...
\item
Цели для \cdf{go}, устанавливающиеся с помощью \cdf{tagbody}, именующияся с
...
также появлятся как теги в телах \cdf{do}, \cdf{prog} и других конструкций для итераций.)
...
\cd{4}. Во время исполнения, \cdf{contorted-example} будет вызывана три раза,
...
\cdf{contorted-example}, к нему добавится \cd{5} и резульата окажется \cd{9}, и это
...
exiting from the block {\it again} and giving \cdf{y} the value \cd{5}
...
\cdf{block}; ее значение получится равным анонимной функции. Так как \cdf{y} не
...
осуществляется выход из блока {\it еще раз} и \cdf{y} получает значение \cd{5},
