...
{\it symbol} for an indicator \cdf{eq} to {\it indicator}.
...
This removes from {\it symbol} the property with an indicator \cdf{eq}
...
Note that using \cdf{get} on the result of \cdf{symbol-plist} does {\it not} work.
...
\cdf{getf} searches the property list stored in {\it place}
for an indicator \cdf{eq} to {\it indicator}.
If one is found, then the corresponding value is returned;
...
\cdf{setf} may be used with \cdf{getf}, in which case the {\it place} must
indeed be acceptable as a {\it place} to \cdf{setf}.  The effect is to
...
The form {\it place} may be any generalized variable acceptable to \cdf{setf}.
See \cdf{remprop}.
...
(see \cdf{intern}) in the {\it package} (which defaults to the current
package; see \cd{*package*}).  \cdf{gentemp} guarantees the symbol
...
to the next; if {\it prefix} is omitted, the default prefix \cdf{T} is used.
\end{defun}

\begin{defun}[Function]
symbol-package sym

Given a symbol {\it sym}, \cdf{symbol-package} returns the contents of the
