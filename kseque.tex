...
The type \cd{sequence} encompasses both lists and vectors (one-dimensional
arrays).
...
and elements are tested for being \cd{eql} to that item.
(A test other than \cd{eql} can be specified by the \cd{:test}
or \cd{:test-not} keyword.  It is an error to use both
of these keywords in the same call.)
The variants formed by adding \cd{-if} and \cd{-if-not}
...
\end{lisp}
returns a copy of {\it sequence} from which all elements \cd{eql} to {\it item}
have been removed;
\begin{lisp}
(remove {\it item} {\it sequence} \cd{:test} \#'equal)
\end{lisp}
returns a copy of {\it sequence} from which all elements \cd{equal} to {\it item}
have been removed;
\begin{lisp}
...
\end{lisp}
This searches for the first element of {\it sequence} whose {\it car} is \cd{eq}
to {\it item}.
\begin{newer}
...
one could simply call \cd{subseq} instead to extract the subsequence
...
or \cd{:end2} but also to functions such as \cd{subseq}
...
defaults to zero if not supplied; it is not permissible to pass \cd{nil}
as a ``start'' argument.
\item An ``end'' argument must be either a
non-negative integer or \cd{nil} (which indicates the end of the
sequence) and defaults to \cd{nil}
if not supplied; therefore supplying \cd{nil} is equivalent to
...
(as returned by the function \cd{length}).
...
omission or an explicitly passed \cd{nil} value, to the active length of {\it x}, as
returned by \cd{length}.  Then it is an error if the test
\cd{(<=~0~{\it s}~{\it e}~(length {\it x}))}
is not true.
\end{newer}

For some functions, notably \cd{remove} and \cd{delete}, the keyword argument
...
\item
An \cd{-if} function was called, and
\cd{(funcall {\it predicate} ({\it keyfn} {\it x}))} is true.

\item
An \cd{-if-not} function was called, and
...
function).  See, for example, \cd{remove}.
...
and \cd{-if-not} functions. This means that these features are very
...
to add the \cd{complement} function, intended to reduce or eliminate the
...
Returns a function whose value is the same as that of \cd{not}
applied to the result of applying the function {\it fn} to the same
arguments.  One could define \cd{complement} as follows:
\begin{lisp}
(defun complement (fn) \\*
...
One intended use of \cd{complement} is to supplant the use of
\cd{:test-not} arguments and \cd{-if-not} functions.
\begin{lisp}
...
sequence; \cd{make-sequence} constructs a new sequence.
...
as returned by \cd{length}.
The first element of a sequence has index \cd{0}.

(Note that \cd{elt} observes the fill pointer in those vectors that have
fill pointers.  The array-specific function \cd{aref} may be used
to access vector elements that are beyond the vector's fill pointer.)

\cd{setf} may be used with \cd{elt} to destructively replace
...
{\it end}.
\cd{subseq} {\it always} allocates a new sequence for a result; it never
shares storage with an old sequence.  The result subsequence is always of
the same type as the argument {\it sequence}.

\cd{setf} may be used with \cd{subseq} to destructively replace
a subsequence with a sequence of new values; see also \cd{replace}.
\end{defun}

\begin{defun}[Function]
copy-seq sequence

A copy is made of the argument {\it sequence}; the result is \cd{equalp}
to the argument but not \cd{eq} to it.
\begin{lisp}
(copy-seq {\it x}) \EQ\ (subseq {\it x} 0)
\end{lisp}
but the name \cd{copy-seq} is more perspicuous when applicable.
\end{defun}
...
\cd{nreverse} is permitted to perform a \cd{setf} on any part,
{\it car} or {\it cdr}, of the top-level list structure of that list.
When the {\it sequence} is an array,
\cd{nreverse} is permitted to re-order the elements of the given array
...
\cd{make-sequence} should signal an error if the sequence {\it type} specifies the
number of elements and the {\it size} argument is different.
\end{newer}

\begin{newer}
X3J13 voted in March 1989 \issue{CHARACTER-PROPOSAL}
to specify that if {\it type} is \cd{string}, the result is the same
as if \cd{make-string} had been called with the same {\it size}
...
sequences except for \cd{reduce}, which is included here because
...
\cd{concatenate} differs from \cd{append}).  The type of the result is
specified by {\it result-type}, which must be a subtype of \cd{sequence},
as for the function \cd{coerce}.
...
\cd{concatenate} is required to copy the argument rather than simply
returning it.  If a copy is not required, but only possibly type conversion,
then the \cd{coerce} function may be appropriate.

\begin{newer}
X3J13 voted in June 1989 \issue{SEQUENCE-TYPE-LENGTH} to specify that
\cd{concatenate} should signal an error if the sequence type specifies the
...
The result of \cd{map} is a sequence such that element {\it j} is the result
...
(which must be a subtype of the type \cd{sequence}),
as for the function \cd{coerce}.
...
only for effect, and \cd{map} returns {\nil}.  This gives an effect similar
to that of \cd{mapc}.

\begin{newer}
X3J13 voted in June 1989 \issue{SEQUENCE-TYPE-LENGTH} to specify that
\cd{map} should signal an error if the sequence type specifies the number of
...
Lisp 1.5, the function \cd{map} has always meant a non-value-returning
...
to use \cd{map} to mean
what in the past Lisp implementations have called \cd{mapcar}.
To simplify things henceforth, Common Lisp follows current usage,
and what was formerly called \cd{map} is named \cd{mapl} in Common Lisp.
\end{incompatibility}
\afternoterule
...
map-into result-sequence function &rest sequences
    ...
    X3J13 voted in June 1989 \issue{MAP-INTO} to add the function \cd{map-into}.
    ...
    The \cd{function} must accept at least as many arguments as the
    number of argument {\it sequences} supplied to \cd{map-into}.
...
numbered \cd{1}, and so on.
    ...
    The function \cd{map-into} differs from \cd{map} in that it modifies
an existing sequence rather than creating a new one.  In addition,
\cd{map-into} can be called with only two arguments ({\it result-sequence}
and {\it function}), while \cd{map} requires at least three arguments.

If {\it result-sequence} is \cd{nil}, \cd{map-into} immediately returns
\cd{nil}, because \cd{nil} is a sequence of length zero.
\end{defun}
\end{newer}
...
\cd{some} returns as soon as any invocation of {\it predicate}
returns a non-{\false} value; \cd{some} returns that value.
If the end of a sequence is reached, \cd{some} returns {\false}.
Thus, considered as a predicate, it is true if {\it some} invocation of
{\it predicate} is true.

\cd{every} returns {\false} as soon as any invocation of {\it predicate}
returns {\false}.
If the end of a sequence is reached, \cd{every} returns a non-{\false} value.
Thus, considered as a predicate, it is true if {\it every} invocation of
{\it predicate} is true.

\cd{notany} returns {\false} as soon as any invocation of {\it predicate}
returns a non-{\false} value.
If the end of a sequence is reached, \cd{notany} returns a non-{\false} value.
Thus, considered as a predicate, it is true if {\it no} invocation of
{\it predicate} is true.

\cd{notevery} returns a non-{\false} value as soon as any invocation
...
of these functions to \cd{map}.  The functions are therefore extended
...
The \cd{reduce} function combines all the elements of a sequence using
a binary operation; for example, using \cd{+} one can add up all the
elements.
...
arguments, and \cd{reduce} returns whatever the function does.  (This is
...
to extend the \cd{reduce} function to take
...
length of the sequence).  \cd{fill} returns the modified {\it sequence}.
For example:
\begin{lisp}
...
\end{lisp}
The value returned by \cd{replace} is the modified {\it sequence1}.

If {\it sequence1} and {\it sequence2} are the same (\cd{eq}) object
...
then after the \cd{replace} operation
...
to clarify that the \cd{:count} argument must be either \cd{nil}
...
~~~\EV\ (1 2 4 1 3 5)
\end{lisp}
The result of \cd{remove} may share
with the argument {\it sequence}; a list result may share a tail
with an input list, and the result may be \cd{eq} to the input {\it sequence}
...
to clarify the permissible side effects of certain operations.
When the {\it sequence} is a list,
\cd{delete} is permitted to perform a \cd{setf} on any part,
{\it car} or {\it cdr}, of the top-level list structure of that list.
When the {\it sequence} is an array,
\cd{delete} is permitted to alter the dimensions of the given array
...
\beforenoterule
\begin{incompatibility}
In MacLisp, the \cd{delete} function uses
an \cd{equal} comparison rather than \cd{eql}, which is the default
test for \cd{delete} in Common Lisp.  Where in MacLisp one would write
...
\cd{remove-duplicates} is the non-destructive version
of this operation.
The result of \cd{remove-duplicates} may share
with the argument {\it sequence}; a list result may share a tail
with an input list, and the result may be \cd{eq} to the input {\it sequence}
if no elements need to be removed.

\cd{delete-duplicates} may destroy the argument {\it sequence}.
...
to clarify the permissible side effects of certain operations.
When the {\it sequence} is a list,
\cd{delete-duplicates} is permitted to perform a \cd{setf} on any part,
{\it car} or {\it cdr}, of the top-level list structure of that list.
When the {\it sequence} is an array,
\cd{delete-duplicates} is permitted to alter the dimensions of the given array
and to slide some of its elements into new positions without permuting them
in order to produce the resulting array.
\end{newer}
\end{defun}
...
~~~\EV\ (1 2 4 1 3 9 5)
\end{lisp}
The result of \cd{substitute} may share
with the argument {\it sequence}; a list result may share a tail
with an input list, and the result may be \cd{eq} to the input {\it sequence}
if no elements need to be changed.

See also \cd{subst}, which performs substitutions throughout a tree.
...
This is the destructive counterpart to \cd{substitute}.
The result is a sequence of the same kind as the argument {\it sequence}
that has the same elements except that those in the subsequence
delimited by \cd{:start} and \cd{:end} and satisfying the test (see
...
See also \cd{nsubst}, which performs destructive
...
to clarify the permissible side effects of certain operations.
When the {\it sequence} is a list,
\cd{nsubstitute} or \cd{nsubstitute-if}
is required to perform a \cd{setf} on any
{\it car} of the top-level list structure of that list
whose old contents must be replaced with {\it newitem}
but is forbidden to perform a \cd{setf} on any \cd{cdr} of the list.
When the {\it sequence} is an array,
\cd{nsubstitute} or \cd{nsubstitute-if}
is required to perform a \cd{setf} on any element of the array
whose old contents must be replaced with {\it newitem}.
...
functions, notably \cd{position-if} and \cd{find-if},
to process strings.  Note one use of \cd{loop} as well.
\begin{lisp}
...
For example, \cd{search} with a non-{\nil} \cd{:from-end}
...
The \cd{sort} function determines the relationship between two elements
...
selector function for a \cd{defstruct} structure, used in sorting
...
The sorting operation performed by \cd{sort} is not guaranteed {\it stable}.
...
The function \cd{stable-sort} guarantees
stability but may be slower than \cd{sort} in some situations.
...
\cd{nreverse}.  Thus if the argument should not be destroyed, the
...
\end{lisp}
If \cd{foovector} contained these items before the sort
\begin{lisp}
...
\end{lisp}
then after the sort \cd{foovector} would contain
\begin{lisp}
("Beach Boys" "I Get Around") \\
("Beatles" "I Want to Hold Your Hand") \\
("Carpenters" "Close to You") \\
...
which must be a subtype of \cd{sequence}, as for the function \cd{coerce}.
...
The \cd{merge} function determines the relationship between two elements
by giving keys extracted from the elements to the {\it predicate}.
...
The \cd{:key} function should not have any side effects.
A useful example of a \cd{:key} function would be a component
selector function for a \cd{defstruct} structure, used to merge
a sequence of structures.
...
The function \cd{char-lessp} ignores case, and so considers
the characters \cd{Y} and \cd{y} to be equal, for example;
the stability property then guarantees that the character from the
first argument (\cd{Y}) must precede the one from the second
argument (\cd{y}).

\begin{newer}
X3J13 voted in June 1989 \issue{SEQUENCE-TYPE-LENGTH} to specify that
\cd{merge} should signal an error if the sequence type specifies the number of
