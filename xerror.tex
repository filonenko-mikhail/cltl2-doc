...
\cdf{errset} special form in MacLisp) for a program to trap
...
\beforenoterule
\begin{incompatibility}
Lisp Machine Lisp calls this function \cdf{ferror}.
MacLisp has a function named \cdf{error} that takes
...
\beforenoterule
\begin{incompatibility}
The Lisp Machine Lisp function \cdf{fsignal} is similar to this, but
...
\cdf{warn} behaves like
\cdf{break}.  It prints its message and then goes to the debugger or break
loop.  Continuing causes \cdf{warn} to return {\false}.  This flag is intended
...
\beforenoterule
\begin{incompatibility}
In MacLisp, \cdf{break} is a special form (FEXPR)
...
evaluated to produce a truth value specifying whether \cdf{break} should
break (true) or return immediately (false).  In Common Lisp one makes a call
to \cdf{break} conditional by putting it inside a conditional form such as
\cdf{when} or \cdf{unless}.
\end{incompatibility}
\afternoterule
...
\begin{obsolete}\noindent
\cdf{check-type} signals an error if the contents of {\it place} are not
of the desired type.
Upon continuing from this error, the user will be asked for a new value;
\cdf{check-type} will store the new value in {\it place} and start over, 
...
\cdf{setf}.  These should be variables on which {\it test-form} depends,
...
to extend the specification of \cdf{assert} to allow a {\it place}
