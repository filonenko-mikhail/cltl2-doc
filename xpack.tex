...
глобальной переменной \cdf{*package*}. С помощью 
\emph{имени пакета} в выводимом имени символа существует возможность
ссылаться на символы других пакетов.
Например, когда \cd{foo:bar} будет прочтён Common Lisp'ом, то будет ссылаться на
символ \cd{bar} из пакета \cd{foo}.
(Но при условии, что \cd{bar} является экспортированным символом из
\cd{foo}, то есть символом, который является видимым извне пакета \cd{foo}. Ссылка на
внутренний символ требует удваивания двоеточия: \cd{foo::bar}.)

Связь строк и символов, доступных в данном пакете, делится на два вида:
...
одному и тому же символу (\cdf{eq} вернёт истину).
...
исходный символ (\cdf{eq} вернёт истину).
...
только явным действием: изменением значения \cdf{*package*}, продолжением
выполнения после ошибки, или вызовом одной из <<опасных>> функций
\cdf{unintern}, \cdf{unexport}, \cdf{shadow}, \cdf{shadowing-import} или
\cdf{unuse-package}.
...
\cdf{rename-package} заменяет текущее имя пакета и псевдонимы на
...
Большинство функций, которые принимают имя пакета, могут принимать или символ,
...
связана с объектом с именем \cd{editor}. Двойное двоеточие должно использоваться
с осторожностью.
...
\cd{FOO}. Такой вывод, когда символ \cd{bar} является внешним в домашнем пакете
\cd{foo} и недоступен в текущем пакете.

\item[\cd{foo::bar}]
При прочтении, интернирует символ \cd{BAR}, как если бы пакет \cd{FOO} являлся
текущим. Такой вывод, когда символ \cd{bar} является внутренним в его домашнем
пакете \cd{foo} и недоступен в текущем пакете.

\item[\cd{:bar}]
При прочтении, интернирует \cd{BAR} как внешний символ в пакете \cd{keyword} и
выполняет его самого в себя. Такой вывод, когда \cd{keyword} является домашним
пакетом для символа.

\item[\cd{\#:bar}]
При прочтении, создаёт новый дезинтернированный символ с именем \cd{BAR}.
Такой вывод, когда символ \cd{bar} не имеет домашнего пакета.
\end{flushdesc}
...
использованием функции \cdf{import}. Форма \cd{(import 'editor:buffer)}
принимает внешний 
символ с именем \cd{buffer} в пакете \cd{editor} (этот символ распознаётся Lisp
...
разделе~\ref{NAME-CONFLICTS-SECTION}. \cdf{import} не допускает сокрытия
...
затеняющих символов FIXME. \cdf{shadowing-import} должна использоваться
...
Обычно пользователь, по-умолчанию работая в пакете \cdf{common-lisp-user}, будет
загружать 
...
\cdf{use-package}. Внешние символы используемые пакетом перестают
...
Функция \cdf{unexport} откатывает ошибочное экспортирование символа. Она
...
ниже). Функции \cdf{shadow} и \cdf{shadowing-import} могут использоваться для
...
Функция \cdf{intern}, которая является одной из часто используемых Lisp'овыми считывателем
...
создаётся, чтобы затенить старый. \cdf{shadowing-import}  проверяет конфликт
...
Указание затеняющего символа в функцию \cdf{unintern} может раскрывать конфликт
имён, который был ранее разрешён с помощью затенения. Если пакет A использует
пакеты B и C, A содержит затеняющий символ \cdf{x}, и B, и C, каждый содержит
внешний символ с именем \cdf{x}, тогда при удалении затеняющего символа \cdf{x}
из A будет обнаружен конфликт между \cd{b:x} и \cd{c:x}, если эти два символа
различны.
В этом случае \cdf{unintern} будет сигнализировать ошибку.
...
Конфликт имён в \cdf{export} или \cdf{unintern} из-за пакета, наследующего два
...
Конфликт имён в \cdf{import} между символом для импортирования и символом
...
доступного символа с помощью отмены \cdf{import}. Конфликт имён в \cdf{import} с
символом уже присутствующем в пакете может быть разрешён с помощью
дезинтернирования этого символа, или отмены \cdf{import}.
...
только, если использования двух не-\cdf{eq} символов с одинаковым именем не
...
Пользовательским программам не могут выполнять следующие действия, которые
...
\emph{package} имеют значение по-умолчанию \cdf{*package*}. Там, где функция
...
значение \cdf{*package*}, старое значение будет восстановлено после завершения функции.
\end{defun}
...
\item[\cd{:inherited}]
Символ унаследован с помощью \cdf{use-package} (и следовательно символ является
внутренним).
\end{indentdesc}
...
\cdf{shadowing-import} должен использоваться с осторожностью. Она изменяет
...
\cdf{shadowing} должен использоваться с осторожностью. Она изменяет
состояние системы пакетов так, что правила согласованности перестают действовать.
