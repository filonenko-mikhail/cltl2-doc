...
\cd{bar} in the package whose name is \cd{foo}.
(Actually, this is true only if \cd{bar} is an external symbol of \cd{foo},
that is, a symbol that is supposed to be visible outside of \cd{foo}.
...
if they appear explicitly in an \cd{export} command for the package.
...
mappings are inherited from other packages via the \cd{use-package}
...
this process is performed by the function \cd{intern}.
...
This process is performed by the function \cd{unintern}.
...
the same (\cd{eq}) symbol.

\item
{\it Print-read consistency:} An interned symbol always prints as a
sequence of characters that, when read back in, yields the same (\cd{eq})
symbol.

\item
{\it Print-print consistency:} If two interned symbols are not \cd{eq}, then
...
\cd{unintern}, \cd{unexport}, \cd{shadow},
\cd{shadowing-import}, or \cd{unuse-package}.
...
like \cd{editor}; there might be a nickname that is shorter and easier to
type, such as \cd{ed}.

There is a single name space for packages.  The function
\cd{find-package} translates a package name or nickname into the
associated package.  The function \cd{package-name} returns the name of a
package.  The function \cd{package-nicknames} returns a list of all
nicknames for a package.  The function \cd{rename-package} removes a
...
\cd{find-package} function is case-sensitive, like that done for
symbols.  Note that \cd{|Foo|:|Bar|} refers to a symbol whose name is
\cd{Bar} in a package whose name is \cd{Foo}.  By contrast,
...
situations:
\begin{itemize}
\item the \cd{:use} argument to \cd{make-package}
\item the first argument to \cd{package-use-list}, \cd{package-used-by-list},
\cd{package-\discretionary{}{}{}name}, \cd{package-nicknames},
\cd{in-package}, \cd{find-package},
\cd{rename-package}, or \cd{delete-package},


\item the second argument to \cd{intern}, \cd{find-symbol},
\cd{unintern}, \cd{export}, \cd{unexport}, \cd{import}, \cd{shadowing-import},
or \cd{shadow}
\item the first argument, or a member of the list that is the first argument,
to \cd{use-package} or \cd{unuse-package}
\item the value of the {\it package} given to \cd{do-symbols},
\cd{do-external-symbols}, or \cd{do-all-symbols}
\item a member of the {\it package-list} given to \cd{with-package-iterator}
\end{itemize}
Note that the first argument to \cd{make-package} must still be a package
...
as package names and not as package objects.  If \cd{find-package} is
...
\cd{editor:buffer} refers to the external symbol named \cd{buffer}
accessible in the package named \cd{editor}, regardless of whether
there is a symbol named \cd{buffer} in the current package.  If there
is no package named \cd{editor}, or if no symbol named \cd{buffer}
is accessible in \cd{editor}, or if \cd{buffer} is an internal
symbol in \cd{editor}, the Lisp reader will signal
...
reading \cd{buffer} with \cd{*package*} temporarily rebound to the
package whose name is \cd{editor}.  This special-purpose qualifier
should be used with caution.

The package named \cd{keyword} contains all keyword symbols used by the
...
\cd{:foo}) is added to (or looked up in) the \cd{keyword} package as
an {\it external} symbol.  The \cd{keyword} package is also treated
specially in that whenever a symbol is added to the \cd{keyword} package
the symbol is always made external; the symbol
is also automatically declared to be a constant
(see \cd{defconstant}) and made to
...
\cd{use-package} to the \cd{keyword} package.
...
may be accessed by using the function \cd{symbol-package}.
When an interned
...
use of \cd{import} and \cd{unintern}, to create a symbol that has no
...
of the function \cd{import}.  The form \cd{(import 'editor:buffer)} takes
the external symbol named \cd{buffer} in the \cd{editor} package (this
...
\cd{import} it is possible to refer to \cd{buffer} in the importing package
without any qualifier.  The status of \cd{buffer} in the package named
\cd{editor} is unchanged, and \cd{editor} remains the home package for
this symbol.  Once imported, a symbol is {\it present} in the
importing package and can be removed only by calling \cd{unintern}.

If the symbol is already present in the importing package, \cd{import}
has no effect.  If a distinct symbol with the name \cd{buffer} is
accessible in the importing package (directly or by inheritance), then a
correctable error is signaled, as described in
section~\ref{NAME-CONFLICTS-SECTION}, because \cd{import} avoids letting
...
use the function \cd{shadowing-import}.  This inserts
...
(see \cd{unintern}).  The new symbol is
added to the package's shadowing-symbols list.  \cd{shadowing-import}
...
The second mechanism is provided by the function \cd{use-package}.  This
...
other package that uses this package.  Note that \cd{use-package},
unlike \cd{import}, does not cause any new symbols to be {\it present}
in the current package but only makes them {\it accessible} by inheritance.
\cd{use-package} checks carefully for
...
Typically a user, working by default in the \cd{user} package, will
load a number of packages into Lisp to provide an augmented working
environment, and then call \cd{use-package} on each of these packages
to allow easy access to their external symbols.
\cd{unuse-package} undoes the effects of a previous \cd{use-package}.  The
...
When \cd{intern} or some other function wants to look up a symbol in a
...
The function \cd{export} takes a symbol that is accessible in some
...
external symbol in the package, \cd{export} has no effect.  If the symbol
...
via \cd{use-package}, the symbol is first imported into the package, then
...
The function \cd{unexport} is provided mainly as a way to undo erroneous
calls to \cd{export}.  It works only on symbols directly present
in the current package, switching them back to internal status.  If
\cd{unexport} is given a symbol already accessible as an internal
...
(except for one instance involving \cd{import} described below).  The
functions \cd{shadow} and \cd{shadowing-import} may be used to declare
shadowing symbols.
...
The functions \cd{use-package}, \cd{import}, and \cd{export} check for name
conflicts.  \cd{use-package} makes the external symbols of the package
...
\cd{import} adds a single symbol to the internals of a package, checking
for a name conflict with an existing symbol either present in the
package or accessible to it.  \cd{import} signals a name conflict error
...
an \cd{export} during the initial definition of a package, the
result of \cd{package-used-by-list}
will be {\false} and the name-conflict checking
will take negligible time.

The function \cd{intern}, which is the one used most
...
\cd{shadow} and \cd{shadowing-import} never signal a name-conflict error
because the user, by calling these functions, has specified how any
possible conflict is to be resolved.  \cd{shadow} does name-conflict
...
is created to shadow it.  \cd{shadowing-import} does name-conflict
...
\cd{unuse-package}, \cd{unexport}, and \cd{unintern} (when the symbol being
...
Giving a shadowing symbol to \cd{unintern} can uncover a name conflict that had
previously been resolved by the shadowing.  If package A uses packages
B and C, A contains a shadowing symbol \cd{x}, and B and C each contain external
symbols named \cd{x}, then removing the shadowing symbol \cd{x}
from A will reveal a name
conflict between \cd{b:x} and \cd{c:x} if those two symbols are distinct.
In this case \cd{unintern} will signal an error.
...
however, for example when \cd{export} is given a list of symbols, it is
...
caused by \cd{export} of a single symbol will not leave that symbol accessible
to some packages and inaccessible to others; with respect to
each symbol processed, \cd{export}
...
name conflict, via \cd{shadowing-import}, \cd{unintern}, or \cd{unexport}.

A name conflict in \cd{use-package} between a symbol directly present in the
...
A name conflict in \cd{use-package} between two external symbols inherited
...
A name conflict in \cd{export} between the symbol being exported and a
...
A name conflict in \cd{export} or \cd{unintern} due to a package
...
A name conflict in \cd{import} between the symbol being imported and a
...
of the symbol already accessible by not doing the \cd{import}.  A
name conflict in \cd{import} with a symbol already present in the
package may be resolved by uninterning that symbol, or by not
doing the \cd{import}.

Good user-interface style dictates that \cd{use-package} and \cd{export},
...
\begin{flushdesc}
\item[\cd{lisp}]
The package named \cd{lisp} contains the primitives of the
...
Common Lisp system, such as \cd{car}, \cd{cdr}, and \cd{*package*}.
Almost all other packages will want to use \cd{lisp} so that these
symbols will be accessible without qualification.

\item[\cd{user}]
The \cd{user} package is, by default, the current package at the time
a Common Lisp system starts up.  This package uses the \cd{lisp} package.
\end{flushdesc}
\end{obsolete}

\begin{newer}
X3J13 voted in March 1989 \issue{LISP-PACKAGE-NAME} to specify that
the forthcoming ANSI Common Lisp will use the package name \cd{common-lisp}
instead of \cd{lisp} and the package name \cd{common-lisp-user}
instead of \cd{user}.  The purpose is to allow a single Lisp system
...
\begin{flushdesc}
\item[\cd{common-lisp}]
The package named \cd{common-lisp} contains the primitives of the
...
user-visible functions and global variables that are present in the
ANSI Common Lisp system, such as \cd{car}, \cd{cdr}, and \cd{*package*}.
Note, however, that the home package of such symbols is not
necessarily the \cd{common-lisp} package (this makes it easier for
symbols such as \cd{t} and \cd{lambda} to be shared between
the \cd{common-lisp} package and another package, possibly one named \cd{lisp}).
Almost all other packages ought to use \cd{common-lisp} so that these
symbols will be accessible without qualification.
This package has the nickname \cd{cl}.

\item[\cd{common-lisp-user}]
The \cd{common-lisp-user} package is, by default,
the current package at the time an ANSI Common Lisp system starts up.
This package uses the \cd{common-lisp} package
and has the nickname \cd{cl-user}.
...
system-interface functions.  This package uses \cd{lisp} and has the
nickname \cd{sys}. 
\end{flushdesc}
\end{obsolete}
...
so as to limit what may appear in the \cd{common-lisp} package
and to lift the requirement that every implementation have a package
named \cd{system}.  The details are as follows.

Not only must the \cd{common-lisp} package in any given implementation
contain all the external symbols prescribed by the standard;
the \cd{common-lisp} package moreover may not contain any external symbol
that is not prescribed by the standard.  However, the \cd{common-lisp}
package may contain additional internal symbols, depending on the
implementation.

An external symbol of the \cd{common-lisp} package may not have a function,
macro, or special form definition, or a top-level value,
or a \cd{special} proclamation, or a type definition, unless specifically
permitted by the standard.  Programmers may validly rely on this fact;
for example, \cd{fboundp} is guaranteed to be false for all
external symbols of the \cd{common-lisp} package except those explicitly
specified in the standard to name functions, macros, and special forms.
Similarly, \cd{boundp} will be false of all such external symbols
except those documented to be variables or constants.

Portable programs
may use external symbols in the \cd{common-lisp} package that are not documented
...
practice of using such names as \cd{list} and \cd{string} as names
for local variables.
...
or a symbol that is accessible from the \cd{common-lisp-user} package or any
package defined by the user.

This vote eliminates the requirement that every implementation have
a predefined package named \cd{system}.  An implementation may
provide any number of predefined packages; these should be described
in the documentation for that implementation.

The \cd{common-lisp-user} package may contain symbols not described by the standard
and may use other implementation-specific packages.
\end{new}
...
of the following actions are performed on a symbol in the \cd{common-lisp}
package.
\begin{itemize}
...
\item defining it as a type specifier (\cd{defstruct}, \cd{defclass}, \cd{deftype})
\item defining it as a structure (\cd{defstruct})
...
\item declaring or proclaiming its \cd{type} or \cd{ftype}
\item removing it from the package \cd{common-lisp}
\end{itemize}
...
a user program is allowed to lexically bind it and declare the \cd{type} of that binding.
...
\cd{ftype} of that binding and to trace that binding.
...
for example, the macro expansion of \cd{with-open-file} might refer
to the \cd{open} function and might not.  However, the preceding rules eliminate
...
symbols in the \cd{common-lisp} package.
\end{newer}
...
\cd{(eval-when (compile load eval) ...)}; see \cd{eval-when}.
These operations are
\cd{make-package}, \cd{in-package}, \cd{shadow}, \cd{shadowing-import},
\cd{export}, \cd{unexport}, \cd{use-package}, \cd{unuse-package}, and \cd{import}.
...
As part of this change \cd{in-package} was changed to be a macro
rather than a function and its functionality was restricted.
The actions of
\cd{shadow}, \cd{shadowing-import}, \cd{use-package},
\cd{import}, \cd{intern}, and \cd{export} for compilation
purposes may be accomplished with the new macro \cd{defpackage}.
\end{newer}
...
the current package.  The initial value of \cd{*package*} is the \cd{user}
package.

\begin{newer}
X3J13 voted in March 1989 \issue{LISP-PACKAGE-NAME} to specify that
the forthcoming ANSI Common Lisp will use the package name \cd{common-lisp-user}
instead of \cd{user}.
\end{newer}
...
to require \cd{compile-file} to rebind \cd{*package*} to its current value.
\end{newer}
\end{defun}
...
\cd{:use} defaults to a list of one package, the \cd{lisp} package.
...
to change the specification of \cd{make-package} so that the default value
...
the \cd{lisp} package but also (or instead) a package containing
...
The \cd{in-package} function is intended to be placed at the start of a
...
function is similar to \cd{make-package}, except that after the
...
\cd{in-package} call) or until the \cd{*package*} variable reverts to its
old value at the completion of a \cd{load} operation.
...
to specify that \cd{in-package} returns the new package, that is, the
...
\begin{newer}
X3J13 voted in March 1989 \issue{IN-PACKAGE-FUNCTIONALITY}
to restrict the functionality of \cd{in-package} and to make it a macro.
This is an incompatible change.

    Making \cd{in-package} a macro rather than a function means that there
    is no need to require \cd{compile-file} to handle it specially.  Since
    \cd{defpackage} is also defined to have side
...
is returned; if no such package exists, \cd{find-package} returns {\false}.
The matching of names observes case (as in \cd{string=}).

\begin{new}
X3J13 voted in January 1989
\issue{PACKAGE-FUNCTION-CONSISTENCY}
to allow \cd{find-package} to accept a package object, in which case
...
X3J13 voted in January 1989
\issue{PACKAGE-FUNCTION-CONSISTENCY}
to allow \cd{package-name} to accept a package name or nickname, in which case
...
One consequence of this vote is that \cd{package-name}
will return \cd{nil} instead of a package name if applied
to a deleted package object.  See \cd{delete-package}.
\end{new}
\end{defun}
...
X3J13 voted in January 1989
\issue{PACKAGE-FUNCTION-CONSISTENCY}
to allow \cd{package-nicknames} to accept a package name or nickname,
...
X3J13 voted in January 1989
\issue{RETURN-VALUES-UNSPECIFIED}
to specify that \cd{rename-package} returns {\it package}.
\end{new}
\end{defun}
...
symbols in this package by \cd{shadow} or \cd{shadowing-import}.  All
...
X3J13 voted in January 1989
\issue{PACKAGE-DELETION}
to add the \cd{delete-package} function, which
...
the effect of the function \cd{unuse-package} is performed on all
such other packages so as to remove their dependency on the
specified package, after which \cd{delete-package} proceeds to
...
the call to \cd{delete-package}, then its home package is unspecified
(that is, the contents of its package cell are unspecified)
after the \cd{delete-package} operation has been completed.
...
names.  The package object is still a package, but anonymous; \cd{packagep} will
be true of it, but \cd{package-name} applied to it will return \cd{nil}.
...
deleted package (using \cd{intern} or \cd{find-symbol}, for example)
will have unspecified results.

\cd{delete-package} returns \cd{t} if the deletion succeeds,
and \cd{nil} otherwise.
\end{defun}
\end{new}
...
if the package is the \cd{keyword} package); the specified package becomes the
home package of the created symbol.


\begin{newer}
X3J13 voted in March 1989 \issue{CHARACTER-PROPOSAL}
to specify that \cd{intern} may in effect perform the
...
\item[\cd{:inherited}]
The symbol was inherited via \cd{use-package} (which
...
Conceptually, \cd{intern} translates a
string to a symbol.  In MacLisp and several other dialects of Lisp,
\cd{intern} can take either a string or a symbol as its argument; in the 
...
\cd{intern} finds a symbol that is not \cd{eq} to the argument symbol.  To
...
This is identical to \cd{intern}, but it never creates a new symbol.  If
...
value and the second value is as specified for \cd{intern}.  If the
...
\cd{unintern} returns {\true} if it actually removed a symbol,
and {\false} otherwise.

\cd{unintern} should be used with caution.  It changes the state of the
...
\beforenoterule
\begin{incompatibility}
The equivalent of this in MacLisp is \cd{remob}.
\end{incompatibility}
\afternoterule
\end{defun}
...
\cd{export} returns {\true}.

By convention, a call to \cd{export} listing all exported symbols is
...
It is an error to unexport a symbol from the \cd{keyword} package
(see section~\ref{EXPORT-IMPORT-SECTION}).
\cd{unexport} returns {\true}.
...
then \cd{import} sets the home package of the symbol to the
...
This is like \cd{import}, but it does not signal an error even if the
...
\cd{shadowing-import} returns {\true}.

\cd{shadowing-import} should be used with
...
(see section~\ref{NAME-CONFLICTS-SECTION}).
\cd{shadow} returns {\true}.

\begin{newer}
X3J13 voted in March 1988 \issue{SHADOW-ALREADY-PRESENT}
to change \cd{shadow} to accept strings as well as well as symbols
...
for that {\it package}, then \cd{shadow} does add it to the shadowing-symbols
list rather than simply doing nothing.
\end{newer}

\cd{shadow} should be used with
caution.  It changes the state of the package system in such a way that
the consistency rules do not hold across the change.
...
as internal symbols
(see section~\ref{EXPORT-IMPORT-SECTION}).
It is an error to try to use the \cd{keyword} package.
\cd{use-package} returns {\true}.
...
X3J13 voted in January 1989
\issue{DEFPACKAGE}
to add a macro \cd{defpackage} to the
...
the \cd{defpackage} form.

Each standard {\it option} is a list of a keyword (the name of the option)
and associated arguments.  No part of a \cd{defpackage} form is evaluated.
Except for the \cd{:size} option, more than one option of the same
kind may occur within the same \cd{defpackage} form.

The standard options for \cd{defpackage} are as follows.
...
other symbols if necessary, just as with the function \cd{shadowing-import}.
...
the function \cd{use-package}.
If no \cd{:use} option is supplied, then a default list is assumed
as for \cd{make-package}.

X3J13 voted in January 1989
\issue{MAKE-PACKAGE-USE-DEFAULT}
to change the specification of \cd{make-package} so that the default value
for the \cd{:use} argument is unspecified.  This change affects \cd{defpackage}
...
just as with the function \cd{import}.
In no case will symbols be created in a package other than
the one being defined;
a continuable error is signaled if for any {\it symbol-name} there
is no symbol of that name accessible in the package named {\it package-name}.
...
\cd{intern}.  Note that the action of this option may be
...
\cd{export}.  Note that the action of this option may be
...
The order in which options appear in a \cd{defpackage} form does not matter;
part of the convenience of \cd{defpackage} is that it sorts out the options
...
\cd{defpackage} creates it.  If such a package does already exist,
...
An error is signaled if the same \cd{symbol-name} argument (in the sense
...
the underlying operations \cd{use-package}, \cd{import}, and \cd{export}
would handle them.

Implementations may support other \cd{defpackage} options.
Every implementation should signal an error on encountering
a \cd{defpackage} option it does not support.

The function \cd{compile-file} should treat top-level \cd{defpackage}
...
Here is an example of a call to \cd{defpackage} that ``plays it safe''
...
\end{lisp}
The preceding \cd{defpackage} example is designed to operate correctly
even if the package current when the form is read happens not to
``use'' the \cd{common-lisp} package.  (Note the use in this example
of the nickname \cd{cl} for the \cd{common-lisp} package.)
Moreover, neither reading in nor evaluating
this \cd{defpackage} form will ever create any symbols in the
...
\cd{defpackage} that ``plays the whale'' by using all sorts of
permissible syntax.
\begin{lisp}
...
for example, this \cd{defpackage} form will cause the symbol
\cd{cdr} to be shadowed {\it in the new package}; it will not be
shadowed in the package \cd{common-lisp}.  The fact that the name ``\cd{CDR}''
was specified by a package-qualified reference to a symbol in the
\cd{common-lisp} package is a red herring.
The moral is that the syntactic flexibility of \cd{defpackage},
...
\beforenoterule
\begin{implementation}
An implementation of \cd{defpackage} might choose to transform
...
is expressed in terms of strange symbols in the \cd{defpackage} form,
...
behavior required by the specification of \cd{defpackage}.
\end{implementation}
\afternoterule

Note that \cd{defpackage} is not capable by itself of defining
mutually recursive packages, for example two packages each of
which uses the other.  However, nothing prevents one from using
\cd{defpackage} to perform much of the initial setup and then
using functions such as \cd{use-package}, \cd{import}, and \cd{export}
to complete the links.

The purpose of \cd{defpackage} is to encourage the user to
...
constructed, can simply be loaded into the \cd{common-lisp-user} package.
...
\cd{do-symbols} provides straightforward iteration over the symbols of a
...
\cd{progn}) is evaluated, and the result is the value of the
\cd{do-symbols} form.  (When the {\it result-form} is evaluated, the control
variable {\it var} is still bound and has the value {\false}.)  If the
{\it result-form} is omitted, the result is {\false}.  \cd{return} may be used
...
remove the symbol currently the value of {\it var} by using \cd{unintern},
...
to specify that the body of a \cd{do-symbols}
...
\begin{lisp}
(setq *a* (make-package 'a))~~~~~~;{\rm Implicitly uses package \cd{common-lisp}} \\*
(setq *b* (make-package 'b))~~~~~~;{\rm Implicitly uses package \cd{common-lisp}} \\*
(setq *c* (make-package 'c :use '(a b))) \\
\\
(do-symbols (x *c*) (print x))~~~~;{\rm Symbols in package \cd{common-lisp}} \\*
...
\end{new}
\begin{new}
Note that the \cd{loop} construct provides a kind of \cd{for} clause that
...
This is similar to \cd{do-symbols} but executes the body once for every
...
\issue{DO-SYMBOLS-DUPLICATES},
regarding redundant executions of the body for the same symbol,
applies also to \cd{do-all-symbols}.
\end{new}
...
to add a new macro \cd{with-package-iterator} to the language.
...
If the {\it package-list} value is \cd{nil}, it is considered
...
four values are returned: \cd{t}, the symbol,
a keyword
...
list of packages, then one value is returned: \cd{nil}.
...
Each {\it symbol-type} in an invocation of \cd{with-package-iterator}
...
to extend the syntax of \cd{with-package-iterator} by recognizing
...
once the \cd{with-package-iterator} form has been exited.

Any number of invocations of \cd{with-package-iterator}
...
\beforenoterule
\begin{rationale}
This facility is a bit more flexible in some ways than \cd{do-symbols}
and friends.
In particular, it makes it possible to implement \cd{loop}
...
\cd{provide} and \cd{require} are deleted.
X3J13 commented that the file-loading feature of \cd{require} is not
...
\cd{require} is so vague that different implementations are likely to
...
This list is used by the functions \cd{provide} and \cd{require}.
\end{defun}
...
Each module has a unique name (a string).  The \cd{provide} and \cd{require}
...
The \cd{require} function tests whether a module is already present
(using a case-sensitive comparison); if the module is not present,
\cd{require} proceeds to load the appropriate
...
Of course, this is moot if \cd{require} is not in the language.
\end{new}

\begin{new}
X3J13 voted in January 1989
\issue{RETURN-VALUES-UNSPECIFIED}
to specify that the values returned by \cd{provide} and \cd{require}
are implementation-dependent.  Of course, this is moot
if \cd{provide} and \cd{require} are not in the language.
\end{new}
...
\cd{user} package whenever using Common Lisp.  Typically an implementation
...
\begin{enumerate}
\item
A call to \cd{provide} that announces the module name.

\item
A call to \cd{in-package} that establishes the package.

\item
A call to \cd{shadow} that establishes any local symbols that will shadow
symbols that would otherwise be inherited from packages that this
package will use.

\item
A call to \cd{export} that establishes all of this package's external
symbols.

\item
Any number of calls to \cd{require} to load other modules that the
contents of this file might want to use or refer to.  (Because the
calls to \cd{require} follow the calls to \cd{in-package},
\cd{shadow}, and \cd{export}, it is possible for the packages that may
be loaded to refer to external symbols in this package.)

\item
Any number of calls to \cd{use-package}, to make external
symbols from other packages accessible in this package.

\item
Any number of calls to \cd{import}, to make
...
that the \cd{phlogiston} and \cd{alchemy} packages are
single-file, single-package modules as described above.  The \cd{phlogiston}
package needs to use the \cd{alchemy} package, and the \cd{alchemy} package
needs to use several
external symbols from the \cd{phlogiston} package.
The definitions in the \cd{alchemy} and \cd{phlogiston} files
(see tables~\ref{ALCHEMY-FILE-TABLE} and~\ref{PHLOGISTON-FILE-TABLE})
allow a
user to specify \cd{require} statements for either of these modules, or for
...
(the \cd{lisp} package is an example), it is recommended that the user
...
\begin{new}
Indeed, the \cd{defpackage} macro
...
of I.~Newton's files using \cd{defpackage}.
\end{new}
...
\end{table}
\begin{new}
The new version of the initialization file avoids using \cd{require};
instead, we assume that \cd{load} will do the job
...
named, say, ``\cd{foo}'' the file named ``\cd{foo-package}''
contains the necessary \cd{defpackage} and/or other package-establishing
code.  The idiom
\begin{lisp}
...
The file \cd{alchemy-package} is shown in
table~\ref{DEFPACKAGE-ALCHEMY-PACKAGE-TABLE}.
The tricky point here is that the \cd{alchemy} and \cd{phlogiston}
packages contain mutual references (each imports from the other),
and so \cd{defpackage} alone cannot do the job.  Therefore
the \cd{phlogiston} package is not mentioned in a \cd{:use} option
in the \cd{defpackage} for the \cd{alchemy} package.  Instead,
the function \cd{use-package} is called explicitly, after the package definition
for \cd{phlogiston} has been loaded.  Note that this file has
...
the \cd{common-lisp} package. In particular, the standard load-package-definition
idiom has been peppered with package qualifiers:
\begin{lisp}
(cl:unless (cl:find-package "PHLOGISTON") \\
~~(cl:load "phlogiston-package"))
\end{lisp}
Note the use of the nickname \cd{cl} for the \cd{common-lisp} package.

The \cd{alchemy} file, shown in table~\ref{DEFPACKAGE-ALCHEMY-FILE-TABLE},
simply loads the \cd{alchemy} package definition,
...
The file \cd{phlogiston-package} is shown in
table~\ref{DEFPACKAGE-PHLOGISTON-PACKAGE-TABLE}.
This one is a little more straightforward than the file \cd{alchemy-package},
because the latter bears the responsibility for breaking the
circular package references.
This file simply makes sure that the \cd{alchemy} package is defined
and then performs a \cd{defpackage} for the \cd{phlogiston} package.

The \cd{phlogiston} file, shown in table~\ref{DEFPACKAGE-PHLOGISTON-FILE-TABLE},
simply loads the \cd{phlogiston} package definition,
makes that package current, and then defines the ``real contents''
of the package.
...
Suppose that the file \cd{alchemy-package} is loaded first.
It defines the \cd{alchemy} package and then loads
file \cd{phlogiston-package}.  That file in turn finds that the
package \cd{alchemy} has already been defined and therefore does
not attempt to load file \cd{alchemy-package} again; it merely
defines package \cd{phlogiston}.  The file \cd{alchemy-package}
then has a chance to import \cd{phlogiston:make-fire-bottle} and everything
is fine.

On the other hand, suppose that the file \cd{phlogiston-package} is
loaded first.  It finds that the
package \cd{alchemy} has {\it not} already been defined, and therefore
it immediately loads file \cd{alchemy-package}.
That file in turn defines the \cd{alchemy} package; then it
finds that package \cd{phlogiston} is not yet defined and so
loads file \cd{phlogiston-package} {\it again} (indeed, in nested fashion).
This time file \cd{phlogiston-package} {\it does} find that the
package \cd{alchemy} has already been defined, so it simply defines
package \cd{phlogiston} and terminates.
The file \cd{alchemy-package} then imports \cd{phlogiston:make-fire-bottle}
and terminates.
Finally, the outer loading of file \cd{phlogiston-package}
{\it re-defines} package \cd{phlogiston}.  Oh, dear.  Fortunately the
two definitions of package \cd{phlogiston} agree in every detail, so
...
Conclusion: \cd{defpackage} goes a long way, but it certainly doesn't
solve all the possible problems of package and file management.
Neither did \cd{require} and \cd{provide}.  Perhaps further experimentation
