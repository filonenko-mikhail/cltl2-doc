...
приёмников данных.  Потоки букв (или строковые потоки)
возвращают или принимают буквы. Бинарные потоки
возвращают или принимают целые числа.  Главная функция Common Lisp
системы заключается в чтении символов из входного потока,
распознавании символов как представлений Common Lisp'овых объектов
данных, вычислении каждого объекта (как формы) и выводе результата в
выходной поток.
...
для чтения, имеют имена заканчивающиеся на \cd{-input}, и переменные, которые
содержат поток для записи, имеют имена, заканчивающиеся на \cd{-output}.  Имена
...
Данный поток последовательно будет сохранять буквы в подстроке в
