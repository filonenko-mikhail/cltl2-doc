...
\cdf{make-synonym-stream} создает и возвращает поток-синоним.
...
Результатом \cdf{make-string-output-stream} всегдя является поток типа
\cdf{string-stream}.
\end{defun}
\end{newer}
...
Добавлен предикат \cdf{open-stream-p}.
...
подможеством \cdf{character} или \cdf{integer}. Но в приципе поток может
...
Аргумент должен быть типа \cdf{synonyn-stream}. Эта функция возвращает символ,
