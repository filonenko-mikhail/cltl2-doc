...
The value of \cd{*trace-output*} is the stream on which the \cdf{trace}
...
Note that the type of a synonym stream is {\it always} \cdf{synonym-stream},
...
The body is executed as an implicit \cdf{progn} with the variable {\it var}
bound to a character input stream that supplies successive characters from
the value of the form {\it string}.  \cdf{with-input-from-string}
...
One may specify \cdf{nil} instead of a string as the {\it string}
...
if the {\it stream\/} is interactive and otherwise returns \cdf{nil}.
