...
другими значениями. Символы констант определённых с помощью \cdf{defconstant}
...
равен \cdf{setf}---смотрите раздел~\ref{FUNCTION-NAME-SECTION}), в качестве
параметра \emph{name}.
...
полностью переработать \cdf{eval-when} для разрешения некоторых проблем, касательно её
...
образом только ключевой символ \cd{:execute} берётся во внимание. В время
