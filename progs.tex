...
each \emph{keyword} must be a keyword symbol, such as \cd{:start}.
...
\cdf{progn}, but only in the situations listed.  Each \emph{situation}
...
\emph{not-compile-time} mode.  There is another mode called \emph{compile-time-too}
...
\begin{itemize}
\item
If the situation \cdf{compile} is specified, \emph{or} if the
current processing mode is \emph{compile-time-too} and the situation \cdf{eval}
is specified, then process each of the forms in the body
in \emph{compile-time-too} mode.

\item
Otherwise, process each of the forms in the body in \emph{not-compile-time} mode.
\end{itemize}
...
First, if the current processing mode is \emph{compile-time-too} mode,
...
\begin{newer}
X3J13 проголосовал в январе 1989 \issue{CONSTANT-MODIFICATION} для определения
...
\cd{3}).
Переменные могут \emph{назначены} с помощью \cdf{setq} или \emph{связаны} с
помощью \cdf{let}.
...
true, необходимо применить \cdf{macroexpand} или \cdf{macroexpand-1} для раскрытия
...
называется \emph{макровызовом или вызовом марокса (macrocall)} FIXME. Макрос
...
с первым элементом равным \cdf{lambda}. Лябда выражение \emph{не} является
...
списком. Он называется \emph{лямбда списком}, и задает имена для
...
списком. \emph{body} может впоследствии ссылаться на аргументы используя имена
параметров. \emph{body} состоит из любого количества форм (возможно нулевого
количества). Эти формы последовательно выполняются, и результат только \emph{последней} формы,
...
Каждый элемент лямбда списка является или спецификатором параметра или \emph{ключевым
...
\item
Спецификатор для \emph{неопределенного количества или оставшегося (rest)} параметра. Если указан ключевой символ
...
параметры. Каждый из них задается просто именем переменной \emph{var}
параметра.
...
(\emph{n} может равняться нулю), тогда должно быть как мимимум \emph{n}
...
Если в спецификаторе указано имя еще одной переменной \emph{svar}, то она
...
\begin{newer}
X3J13 проголосовал в марте 1989 \issue{FUNCTION-NAME} расширить \cdf{defun} до
принятия любого имени функции (символ или список, у которого \emph{car} элемент
раен \cdf{setf}---смотрите раздел~\ref{FUNCTION-NAME-SECTION}), в качестве
парметра \emph{name}.
...
\emph{forms} составляют тело определяемой функции. Они выполняются как неявный \cdf{progn}.
...
выполнятся в \cdf{defun}. \emph{name} возвращается в качестве значения формы
\cdf{defun}.
Например:
\begin{lisp}
(defun discriminant (a b c) \\
...
(defvar \emph{variable} \emph{initial-value})
\end{lisp}
тогда переменная \emph{variable}, если она еще не была проинициализирована,
...
\begin{newer}
X3J13 проголосовал в марте 1989 \issue{EVAL-WHEN-NON-TOP-LEVEL}
полностью переработать \cdf{eval-when} для разрешения некоторых проблем, касательно ее
...
eval-when ({situation}*) {\,form}*
  ...
  ниже перечисленных ситуациях. Каждая ситуация \emph{situatuon} должна быть
