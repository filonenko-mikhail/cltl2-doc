...
Функциональность структур вополщена в макросе \cdf{defstruct}, который позволяет
...
(setf (ship-x-position ship2) 100)
\end{lisp}
Данная запись именяет позицию-\emph{x} переменной \emph{ship2} в \cd{100}.
Она работает, потому что \cdf{defstruct} ведет себя так, будто создаёт
соотвествующие \cdf{defsetf} формы для \emph{функций доступа}.
\end{itemize}
