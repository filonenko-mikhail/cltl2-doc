...
\begin{itemize}
\item
{\rm [}\emph{form}{\rm ]} is interpreted as \cd{(list {\Xbq}\emph{form})}, which
contains a backquoted form that must then be further interpreted.

\item
{\rm [}\cd{,\emph{form}}{\rm ]} is interpreted as \cd{(list \emph{form})}.
...
\beforenoterule
\begin{incompatibility}
\cd{*print-escape*} controls what was called \emph{slashification}
in MacLisp.
\end{incompatibility}
\afternoterule
\end{defun}
...
using \cd{\#\emph{n\/}\#} syntax (when \cd{*print-circle*} is non-\cdf{nil}, of course).
...
\emph{object} is printed with the \cd{*print-pretty*} flag non-{\nil}
...
and \cdf{princ} return {\true}, not the argument \emph{object}.
\end{incompatibility}
\afternoterule
\end{defun}
...
outputs a space, and \cd{{\Xtilde}\emph{n}X} outputs \emph{n} spaces,
...
\cd{G\emph{w}.\emph{d}} edit descriptor in Fortran.
