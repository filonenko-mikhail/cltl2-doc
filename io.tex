...
(Lisp) {\it printer}.  The \cdf{read} function takes characters from a
...
\indexterm{symbols}{printer}
\indexterm{symbols}{printed representation}
\indexterm{symbols}{reader}
...
\end{obsolete}
\begin{newer}
For the purposes of \cdf{readtable-case}, {\it y} is not replaceable.
\end{newer}
...
\end{obsolete}
\begin{newer}
For the purposes of \cdf{readtable-case}, {\it z} is not replaceable.
\end{newer}
...
as three distinct tokens, \cd{|foo|}, \cdf{bar}, and \cd{|baz|}, whereas in Common Lisp
...
{\it ratio} ::= \Mopt{{\it sign}} \Mplus{{\it digit}} \cdf{/} \Mplus{{\it digit}} \\
...
\Mstar{{\it digit}}} {\it exponent} \\ {\it sign} ::= \cdf{+} {\Mor} \cdf{-} \\
{\it decimal-point} ::= \cd{.} \\
...
{\it exponent-marker} ::= \cdf{e} {\Mor} \cdf{s} {\Mor} \cdf{f} {\Mor} \cdf{d} {\Mor} \cdf{l}
...
\cd{\&}&{\it alphabetic}&\cdf{+}&\multicolumn{3}{l}{{\it alphabetic}, {\it plus sign}} \\
\cd{'}&{\it alphabetic} *&\cdf{-}&\multicolumn{3}{l}{{\it alphabetic}, {\it minus sign}} \\
\cd{(}&{\it alphabetic} *&\cdf{*}&{\it alphabetic} \\
\cd{)}&{\it alphabetic} *&\cdf{/}&\multicolumn{3}{l}{{\it alphabetic}, {\it ratio marker}} \\
\cd{,}&{\it alphabetic} *&\cd{{\Xatsign}}&{\it alphabetic} \\
\cd{0}&{\it alphadigit}&\cdf{A}, \cdf{a}&{\it alphadigit} \\
\cd{1}&{\it alphadigit}&\cdf{B}, \cdf{b}&{\it alphadigit} \\
\cd{2}&{\it alphadigit}&\cdf{C}, \cdf{c}&{\it alphadigit} \\
\cd{3}&{\it alphadigit}&\cdf{D}, \cdf{d}&\multicolumn{3}{l}{{\it alphadigit}, {\it double-float exponent marker}} \\
\cd{4}&{\it alphadigit}&\cdf{E}, \cdf{e}&\multicolumn{3}{l}{{\it alphadigit}, {\it float exponent marker}} \\
\cd{5}&{\it alphadigit}&\cdf{F}, \cdf{f}&\multicolumn{3}{l}{{\it alphadigit}, {\it single-float exponent marker}} \\
\cd{6}&{\it alphadigit}&\cdf{G}, \cdf{g}&{\it alphadigit} \\
\cd{7}&{\it alphadigit}&\cdf{H}, \cdf{h}&{\it alphadigit} \\
...
\begin{lisp}
~~~~~~~~~~~~~~~~\=\kill
(a . b)\>;{\rm A dotted pair of \cdf{a} and \cdf{b}} \\
(a.b)\>;{\rm A list of one element, the symbol named \cd{a.b}} \\
(a. b)\>;{\rm A list of two elements \cd{a.} and \cdf{b}} \\
(a .b)\>;{\rm A list of two elements \cdf{a} and \cd{.b}} \\
(a {\Xbackslash}. b)\>;{\rm A list of three elements \cdf{a}, \cd{.}, and \cdf{b}} \\
(a |.| b)\>;{\rm A list of three elements \cdf{a}, \cd{.}, and \cdf{b}} \\
(a {\Xbackslash}... b)\>;{\rm A list of three elements \cdf{a}, \cd{...}, and \cdf{b}} \\
(a |...| b)\>;{\rm A list of three elements \cdf{a}, \cd{...}, and \cdf{b}} \\
(a b . c)\>;{\rm A dotted list of \cdf{a} and \cdf{b} with \cdf{c} at the end} \\
...
called \cdf{ibase} in MacLisp and to the function called \cdf{radix}
in Interlisp.
\end{incompatibility}
\afternoterule
\end{defun}
...
\cd{*read-eval*}, whose default value is \cdf{t}.
...
\indexterm{symbols}{parsing}
\indexterm{symbols}{macro character}
...
\indexterm{symbols}{\cd{) }macro character}
...
\cdf{quote} and {\it foo}.

\item[\cd{;}]
Semicolon is used to write comments.
\indexterm{symbols}{\cd{; }macro character}
\indexterm{symbols}{comments}
...
\cd{\#'}&{\it \cdf{function} abbreviation}&\cd{\#}$\langle$space$\rangle$&{\it signals error} \\
...
\indexterm{symbols}{character syntax}
...
of the Lisp object represented by {\it foo},
which may be the printed representation of any Lisp object.
The evaluation is done during the \cdf{read} process,
...
\cdf{eval-when}, whereas \cd{\#.} is more like specifying \cd{(eval compile)}.
...
is not properly restricted.  See \cdf{load-time-eval}.
\end{new}
...
(\cdf{eq}) object labelled by \cd{\#{\it n}=}.
...
\beforenoterule
\begin{incompatibility}
MacLisp uses the \cdf{status} special form for
this purpose, and Lisp Machine Lisp duplicates \cdf{status} essentially only
...
Otherwise,
{\it feature} should be a Boolean expression composed of \cdf{and}, \cdf{or}, and
\cdf{not} operators on (recursive) {\it feature} expressions.
...
\cdf{string-char} must be represented in the readtable.
\end{obsolete}

\begin{newer}
X3J13 voted in March 1989 \issue{CHARACTER-PROPOSAL}
to remove the type \cdf{string-char} and to replace the bits and font attributes
...
then \cdf{*} will become a {\it constituent} that is
...
to specify that the \cdf{set-syntax-from-char} function returns \cdf{t}.
\end{new}
\end{defun}
...
\cdf{set-macro-character} causes {\it char} to be a macro character that
when seen by \cdf{read} causes {\it function} to be called.
...
\cdf{get-macro-character} returns the function associated with {\it char}
...
characters of MacLisp or the \cdf{splice} macro characters of Interlisp.
...
characters are macro characters with the \cdf{ALWAYS} or \cdf{FIRST} option,
respectively.  Common Lisp has nothing equivalent to the Interlisp \cdf{ALONE}
...
others into single-character-object macros, and then use \cdf{read}
...
\cd{*read-default-float-format*}, then the exponent marker \cdf{E}
is used.
...
\end{obsolete}
\begin{newer}
It is also controlled by \cd{*print-escape*} and the \cdf{readtable-case} slot of the current
...
then \cdf{A}, then {\it n} open parentheses.  Next the elements
...
The function \cdf{princ} effectively binds \cd{*print-escape*} to {\false}.
...
\beforenoterule
\begin{incompatibility}
MacLisp calls this variable \cdf{base}, and its default value is \cd{8}, not \cd{10}.
...
X3J13 voted in June 1989 \issue{PRINT-CASE-PRINT-ESCAPE-INTERACTION}
to clarify the interaction of \cd{*print-case*} with \cd{*print-escape*}.
When \cd{*print-escape*} is \cdf{nil}, \cd{*print-case*} determines the
case in which to print all uppercase characters in the print name of the symbol.
When \cd{*print-escape*} is not \cdf{nil}, the implementation has some freedom
...
\end{table}
    ...
    of the last body {\it form}, or \cdf{nil} if there are no body forms.
...
Using \cdf{with-standard-io-syntax} to bind all the variables,
  ...
  \cd{*package*} or \cd{*read-eval*}, it can be bound by \cdf{let} inside the body of
...
Note that \cdf{read-preserving-whitespace} behaves {\it exactly} like \cdf{read}
...
as if with \cdf{peek-char}.  If it is {\it char}, then the
...
Note that \cdf{read-delimited-list} does not take an {\it eof-error-p}
(or {\it eof-value})
...
\cdf{read-char} inputs one character from {\it input-stream} and returns it
...
\cdf{unread-char} puts the {\it character} onto the front of {\it input-stream}.
...
{\it input-stream}.  \cdf{unread-char} returns {\false}.

One may apply \cdf{unread-char} only to the character most recently
read from {\it input-stream}.  Moreover, one may not invoke \cdf{unread-char}
...
What \cdf{peek-char} does depends on the {\it peek-type}, which
...
If {\it peek-type} is {\true}, then \cdf{peek-char} skips over whitespace
...
If {\it peek-type} is a character object, then \cdf{peek-char} skips
...
Note, however, that if the {\it peek-type} is not \cdf{nil}, then characters
...
\cdf{read-byte} reads one byte from the {\it binary-input-stream} and returns
...
the function \cdf{read}.  \cd{prin1} returns the {\it object} as its value.
\begin{lisp}
...
of {\it object} is preceded by a newline (see \cdf{terpri})
and followed by a space.
\cdf{print} returns {\it object}.
...
the same manner as \cdf{write-char}: the character is simply sent to the output {\it stream}.
...
\end{newer}
\cdf{princ} returns the {\it object} as its value.
\begin{lisp}
...
\beforenoterule
\begin{incompatibility}
In MacLisp, the functions \cd{prin1}, \cdf{print},
and \cdf{princ} return {\true}, not the argument {\it object}.
\end{incompatibility}
\afternoterule
\end{defun}
...
\beforenoterule
\begin{incompatibility}
The Interlisp function \cdf{mkstring} corresponds
to the Common Lisp function \cdf{princ-to-string}.
\end{incompatibility}
\afternoterule
...
\cdf{write-char} outputs the {\it character} to {\it output-stream},
...
The function \cdf{terpri} outputs a newline to {\it output-stream}.
...
print-unreadable-object (object stream
    ...
    beginning with
    \cd{\#<} and ending with \cdf{>}.  Everything output to the {\it stream} during
...
\cdf{write-byte} writes one byte, the value of {\it integer}.
...
\indexterm{symbols}{formatted output}
...
\cdf{format} outputs the characters of {\it control-string},
...
In all other cases
\cdf{format} returns {\false}, performing output to {\it destination}
as a side effect.
...
\cdf{V} (or \cdf{v}), which takes an argument from {\it arguments} for use as a parameter to
...
(as by \cdf{princ}).  In particular, if {\it arg} is a string, its characters
...
X3J13 voted in January 1989
\issue{FORMAT-PRETTY-PRINT}
to specify that \cdf{format} binds \cd{*print-escape*}
...
in a manner analogous to Fortran \cdf{X} format.
...
\cd{{\Xtilde}R} prints {\it arg} as a cardinal English number: \cdf{four};
\cd{{\Xtilde}:R} prints {\it arg} as an ordinal English number: \cdf{fourth};
\cd{{\Xtilde}{\Xatsign}R} prints {\it arg} as a Roman numeral: \cdf{IV}; and
\cd{{\Xtilde}:{\Xatsign}R} prints {\it arg} as an old Roman numeral: \cdf{IIII}.
...
\item[\cd{{\Xtilde}P}]
{\it Plural}.
If {\it arg} is not \cdf{eql} to the integer \cd{1}, a lowercase \cdf{s} is
printed; if {\it arg} is \cdf{eql} to \cd{1}, nothing is printed.  (Notice
that if {\it arg} is a floating-point \cd{1.0}, the \cdf{s} {\it is}
printed.)
\cd{{\Xtilde}:P} does the same thing, after doing a \cd{{\Xtilde}:*} to back up one argument;
that is, it prints a lowercase \cdf{s} if the {\it last} argument was not
...
X3J13 voted in January 1989
\issue{FORMAT-PRETTY-PRINT}
to specify that \cdf{format} binds \cd{*print-escape*} to \cdf{t}
...
If {\it arg} is a rational number, then it is coerced to be a \cdf{single-float}
...
X3J13 voted in January 1989
\issue{FORMAT-PRETTY-PRINT}
to specify that \cdf{format} binds \cd{*print-escape*} to \cdf{nil}
...
\>{\rm should be \cd{0}}\>{\rm should be \cdf{E}}
\end{lisp}
...
\cd{{\Xtilde}{\it n}\&} calls \cdf{fresh-line}
...
Note that {\it str} may include \cdf{format} directives.
...
\indexterm{symbols}{querying the user}
\indexterm{symbols}{yes-or-no functions}
