...
\Mstar{{\it digit}}} {\it exponent} \\ {\it sign} ::= \cd{+} {\Mor} \cdf{-} \\
{\it decimal-point} ::= \cd{.} \\
...
\begin{itemize}
\item
It consists entirely of digits, signs (\cd{+} or \cdf{-}),
...
\cd{<}&{\it alphabetic}&\cdf{M}, \cdf{m}&{\it alphadigit} \\
\cd{=}&{\it alphabetic}&\cdf{N}, \cdf{n}&{\it alphadigit} \\
\cd{>}&{\it alphabetic}&\cdf{O}, \cdf{o}&{\it alphadigit} \\
...
For example, if the definition of \cdf{S} is copied to \cd{*},
...
to \cdf{peek-char} would see the following \cd{/}, and the loop would
