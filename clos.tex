...
\chapter{Common Lisp Object System. Daniel G.~Bobrow, Linda G.~DeMichiel,
...
A class $C\sub{1}$ is a \emph{direct superclass\/} of a class
...
\cd{(and $T\sub 1$ $\ldots$ $T\sub {n}$)}
where $T\sub 1, \ldots, T\sub {n}$ are
...
Remove \emph{C} from $S\sub {C}$, and remove all pairs of the form
$(C,D)$,
$D\in S\sub {C}$, from \emph{R}. Repeat the process, adding
classes with no predecessors to the end of the result.  Stop when no
element can be found that has no predecessor.

If $S\sub {C}$ is not empty and the process has stopped, the set \emph{R} is
...
classes $C\sub 1,\ldots,C\sub {n}$
such that $C\sub {i}$ precedes
$C\sub{i+1}$, $1\leq i<n$,
and $C\sub {n}$ precedes $C\sub 1$.
...
In more precise terms, let $\{N\sub 1,\ldots,N\sub {m}\}$,
$m\geq 2$, be
...
$N\sub {i}$
is a direct superclass of $C\sub {j}$;
$N\sub {i}$ is placed next.
...
\cd{standard{\Qhyphen}object})$,
$(\cdf{fruit},
\cd{standard{\Qhyphen}object})$, and
$(\cdf{spice},
\cd{standard{\Qhyphen}object})$
...
$R\sub {c}$ is then generated from this augmented list of superclasses;
...
\emph{applicable\/} when each $A\sub {i}$
\emph{satisfies\/} $P\sub {i}$.
If $P\sub {i}$ is a class,
and if $A\sub {i}$ is an instance of a class
\emph{C}, then it is said that $A\sub {i}$ \emph{satisfies\/}
$P\sub {i}$ when $C=P\sub {i}$ or when C is a subclass of $P\sub {i}$.  If
$P\sub {i}$ is of the form
\cd{(\cdf{eql} \emph{object\/})}, then it is said that
$A\sub {i}$ satisfies $P\sub {i}$
when the function \cdf{eql} applied to
$A\sub {i}$ and \emph{object} is true.
...
$P\sub{1,i}=\hbox{{\tt(\cdf{eql} $\hbox{object}\sub 1$)}}$,
$P\sub{2,i}=\hbox{{\tt(\cdf{eql} $\hbox{object}\sub 2$)}}$, and
...
A method for \cd{compute-effective-method} can be defined directly by
...
\begin{lisp}
(\emph{generic-function\/} $a\sub 1$ ... $a\sub {n}$)
\end{lisp}
Let $M\sub 1,\ldots,M\sub {k}$ be the applicable primary methods
in order; then the derived Lisp form is
\begin{lisp}
(\emph{operator\/} $\langle M\sub 1\;a\sub 1\ldots a\sub {n}\rangle$
... $\langle M\sub k\;a\sub 1\ldots a\sub {n}\rangle$)
\end{lisp}
If the expression $\langle M\sub {i} \;a\sub 1\ldots a\sub
{n}\rangle$ is
...
\cd{update-instance-for-redefined-class}
...
different class: \cd{update-instance-for-different-class} and 
...
\cd{update-instance-for-different-class}, and
\cd{update-instance-for-redefined-class}.
Thus methods can be written for 
\cd{shared-initialize} to specify actions that should be taken in all of
these contexts.
...
\cd{initialize-instance} and \cdf{shared-initialize}.  The
description of \cd{shared-initialize} in
section~\ref{Functions-in-the-Programmer-Interface-SECTION}
mentions the
...
methods for the generic function \cd{default-initargs} and the
system-supplied primary methods for \cd{allocate-instance}, 
\cd{initialize-instance},
and \cd{shared-initialize} might not be called on
every call to \cd{make-instance} or might not receive exactly the
...
slots are passed as arguments to \cd{update-instance-for-redefined-class}
...
function \cd{update-instance-for-redefined-class}, which is called after
completion of the first step of modifying the structure of the
instance.

The generic function \cd{update-instance-for-redefined-class} takes
...
The generic function \cd{update-instance-for-redefined-class} also
...
\cd{update-instance-for-redefined-class} whose parameter specializer for
...
Methods for \cd{update-instance-for-redefined-class} may be defined
to specify actions to be taken when an instance is updated.  If only
\cd{:after} methods for \cd{update-instance-for-redefined-class} are
...
behavior of \cd{update-instance-for-redefined-class}.  Because no
initialization arguments are passed to 
\cd{update-instance-for-redefined-class} when it is called by the system,
the \cd{:initform} forms for slots that are filled by \cd{:before}
methods for \cd{update-instance-for-redefined-class} will not be
...
the generic function \cd{update-instance-for-different-class}.  The
generic function \cd{update-instance-for-different-class} is invoked
by \cdf{change-class} after the first step of the update has been
completed.

The generic function \cd{update-instance-for-different-class} is
...
The generic function \cd{update-instance-for-different-class} also
takes any number of initialization arguments.  When it is called by
\cdf{change-class}, no initialization arguments are provided.

There is a system-supplied primary method for the generic function
\cd{update-instance-for-different-class} that has two parameter
...
Methods for \cd{update-instance-for-different-class} may be defined
to specify actions to be taken when an instance is updated.  If only
\cd{:after} methods for \cd{update-instance-for-different-class} are
defined, they will be run after the system-supplied primary method for
initialization and will not interfere with the default behavior of
\cd{update-instance-for-different-class}.  Because no initialization
arguments are passed to \cd{update-instance-for-different-class} when
it is called by \cdf{change-class}, the \cd{:initform} forms for slots
that are filled by \cd{:before} methods for 
\cd{update-instance-for-different-class} will not be evaluated by 
\cdf{shared-initialize}.
...
\cdf{make-instances-obsolete}&\cd{update-instance-for-different-class}\\
\cdf{no-applicable-method}&\cd{update-instance-for-redefined-class}\\
\cdf{no-next-method}&
\end{tabular}
\end{flushleft}
...
combination type allows \cd{call-next-method}
...
function \cd{update-instance-for-different-class}.  The generic function
\cd{update-instance-for-different-class}
...
\cd{compute-applicable-methods} returns the set of methods
...
\cd{method-combination-error} or
\cd{invalid-method-error}; these functions
...
\cd{compute-effective-method} is called.
...
\cd{define-method-combination} form.
...
\cd{update-instance-for-redefined-class} will be invoked.
...
system-supplied primary method for \cd{initialize-instance}, 
\cd{reinitialize-instance},
\cd{update-instance-for-redefined-class}, and
\cd{update-instance-for-different-class}.
...
\cd{slot-makunbound-using-class}.
See \cdf{slot-missing}.
\end{defun}
...
The generic function \cd{update-instance-for-different-class} is not
...
\cd{update-instance-for-different-class} checks the validity of
initialization arguments and signals an error if an initialization
...
defined, they will be run after the system-supplied primary method for
initialization and therefore will not interfere with the default
behavior of \cd{update-instance-for-different-class}.





The arguments to \cd{update-instance-for-different-class} are
...
\cd{update-instance-for-different-class}, \emph{previous\/}, is that
...
way once \cd{update-instance-for-different-class} returns, the
results are undefined.  The second argument to 
\cd{update-instance-for-different-class}, \emph{current}, is the altered
original instance.
...
The value returned by \cd{update-instance-for-different-class} is
...
Initialization arguments are declared valid by using the 
\cd{:initarg} option to \cdf{defclass}, or by defining methods for 
\cd{update-instance-for-different-class} or \cdf{shared-initialize}.  The
keyword name of each keyword parameter specifier in the lambda-list of
any method defined on \cd{update-instance-for-different-class} or 
...
Methods on \cd{update-instance-for-different-class} can be defined to
initialize slots differently from \cdf{change-class}.  The default
behavior of \cd{change-class} is described in
section~\ref{Changing-the-Class-of-an-Instance-SECTION}.
...
\vskip 0pt plus 10pt
\noindent
The generic function \cd{update-instance-for-redefined-class} is not
intended to be called by programmers. Programmers may
write methods for it.  The generic function 
\cd{update-instance-for-redefined-class} is called by the mechanism
...
arguments to \cd{update-instance-for-redefined-class} are this
...
The value returned by \cd{update-instance-for-redefined-class} is ignored.


Initialization arguments are declared valid by using the 
\cd{:initarg} option to \cdf{defclass} or by defining methods for 
\cd{update-instance-for-redefined-class} or \cdf{shared-initialize}.  The
keyword name of each keyword parameter specifier in the lambda-list of
any method defined on \cd{update-instance-for-redefined-class} or 
\cdf{shared-initialize} is declared a valid initialization argument name
for all classes for which that method is applicable.
...
\ref{Rules-for-Initialization-Arguments-SECTION}, and
\ref{Declaring-the-Validity-of-Initialization-Arguments-SECTION} as well as
\cdf{shared-initialize} and \cd{make-instances-obsolete}.
...
\begin{lisp}
(with-accessors (\textrm{$slot\sub 1$} ... \textrm{$slot\sub {n}$}) \emph{instance\/} \\*
...
~~(symbol-macrolet ((\textrm{$variable\sub 1$} (\textrm{$accessor\sub 1$} \emph{in\/})) \\*
~~~~~~~~~~~~~~~~~~~~... \\*
~~~~~~~~~~~~~~~~~~~~(\textrm{$variable\sub {n}$} (\textrm{$accessor\sub {n}$} \emph{in\/}))) \\*
~~~~$declaration\sub 1$ ... $declaration\sub {m}$) \\*
~~~~$form\sub 1$ ... $form\sub {k}$)
\end{lisp}
...
\begin{lisp}
(with-slots (\emph{$slot{\foohyphenentry}\sub 1$} ... $slot{\foohyphenentry\sub {n}$}) \emph{instance\/} \\*
~~$declaration\sub 1$ ... $declaration\sub {m}$) \\*
~~$form\sub 1$ ... $form\sub {k}$)
\end{lisp}
expands into the equivalent of
\begin{lisp}
(let ((\emph{in\/} \emph{instance\/})) \\
~~(symbol-macrolet ($Q\sub 1$ ... $Q\sub {n}$) \\*
~~~~$declaration\sub 1$ ... $declaration\sub {m}$) \\*
~~~~$form\sub 1$ ... $form\sub {k}$)
\end{lisp}
where ${Q}\sub {j}$ is 
\begin{lisp}
(\textrm{$slot{\foohyphen}entry\sub {j}$} (slot-value \emph{in\/} '\emph{$slot{\foohyphen}entry\sub {j}$}))
\end{lisp}
if ${\hbox{slot-entry}}\sub {j}$ is a symbol and is
\begin{lisp}
(\textrm{$variable{\foohyphen}name\sub {j}$} (slot-value \emph{in\/} '\textrm{$slot{\foohyphen}name\sub {j}$}))
\end{lisp}
if ${\hbox{slot-entry}}\sub {j}$
is of the form \cd{(\textrm{$variable{\foohyphen}name\sub {j}$} \textrm{$slot{\foohyphen}name\sub {j}}$})}.
