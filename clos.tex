...
superclass, $C _2$, by adding \cd{:before} and \cd{:after}
...
instances of the class $C_1$.  Class $C _1$'s \cd{:before}
method runs before all of class $C _2$'s methods.  Class $C \sub
1$'s \cd{:after} method runs after all of class $C _2$'s methods.
...
\Mor~\cd{:operator \emph{operator\/}} \poptabs \\
...
\item 
The \cd{*print-base*}, \cd{*print-radix*}, \cdf{*print-case*}, 
...
\begin{lisp}
(with-slots (\emph{$slot{\foohyphenentry}_1$} ... $slot{\foohyphenentry_{n}$}) \emph{instance\/} \\*
~~$declaration_1$ ... $declaration_{m}$) \\*
~~$form_1$ ... $form_{k}$)
\end{lisp}
expands into the equivalent of
...
where ${Q}_{j}$ is 
\begin{lisp}
(\textrm{$slot{\foohyphen}entry_{j}$} (slot-value \emph{in\/} '\emph{$slot{\foohyphen}entry_{j}$}))
\end{lisp}
if ${\hbox{slot-entry}}_{j}$ is a symbol and is
\begin{lisp}
(\textrm{$variable{\foohyphen}name_{j}$} (slot-value \emph{in\/} '\textrm{$slot{\foohyphen}name_{j}$}))
\end{lisp}
if ${\hbox{slot-entry}}_{j}$
is of the form \cd{(\textrm{$variable{\foohyphen}name_{j}$} \textrm{$slot{\foohyphen}name_{j}}$})}.
