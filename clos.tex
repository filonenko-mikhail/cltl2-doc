...
\cd{funcall} and \cd{apply} and can be given a global or a local name.
...
A class can have a {\bit name}. The function \cd{class-name} takes a
class object and returns its name. The name of an anonymous class is
\cd{nil}.  A symbol can {\bit name\/} a class.  The function 
\cd{find-class} takes a symbol and returns the class that the symbol
...
two distinguished classes, named \cd{t} and \cd{standard-object}.
The class named \cd{t} has no superclasses.  It is a superclass of
every class except itself.  The class named \cd{standard-object} is
an instance of the class \cd{standard-class} and is a superclass of
every class that is an instance of \cd{standard-class} except itself.
...
The \CLOS\ provides a default metaclass, \cd{standard-class}, that is
...
chapter are instances of the class \cd{standard-class}, all generic
functions are instances of the class \cd{standard-generic-function},
and all methods are instances of the class \cd{standard-method}.

\subsubsection{Defining Classes}

The macro \cd{defclass} is used to define a new named class.
...
\end{itemize}
The slot options and class options of the \cd{defclass} form provide
...
The generic function \cd{make-instance} creates and returns a new
...
instances either by giving arguments to \cd{make-instance} or by
...
An object that has \cd{standard-class} as its metaclass has zero or
...
function \cd{slot-unbound} is invoked. The system-supplied primary method
for \cd{slot-unbound} signals an error.
...
the \cd{defclass} form was evaluated. The \cd{:initform} along with
the lexical environment in which the \cd{defclass} form was evaluated
...
the \cd{defclass} form for that class contains a slot specifier with
...
The \cd{:allocation} slot option to \cd{defclass} controls the kind
...
\cd{slot-value} and by use of generic functions generated by the 
\cd{defclass} form.

The function \cd{slot-value} can be used with any slot name
specified in the \cd{defclass} form to access a specific slot
accessible in an instance of the given class.

The macro \cd{defclass} provides syntax for generating methods to
...
Note that \cd{slot-value} can be used to read or write the value of a
slot whether or not reader or writer methods exist for that slot.
When \cd{slot-value} is used, no reader or writer methods are
invoked.

The macro \cd{with-slots} can be used to establish a lexical
environment in which specified slots are lexically available as if they
were variables.  The macro \cd{with-slots} invokes the function 
\cd{slot-value} to access the specified slots.

The macro \cd{with-accessors} can be used to establish a lexical
environment in which specified slots are lexically available through
their accessors as if they were variables.  The macro 
\cd{with-accessors} invokes the appropriate accessors to access the
specified slots. Any accessors specified by \cd{with-accessors} must
...
A class can inherit methods, slots, and some \cd{defclass} options
...
using one of the \cd{defclass} options that causes methods to be
...
contents of the slot will always be of type \cd{t}. The result
...
1$ will share this single slot unless the \cd{defclass} form for
...
class2\/})} returns the values \cd{t}~and~\cd{t} if {\it class1\/} is a
subclass of {\it class2\/} or if they are the same class; otherwise it
returns the values \cd{nil}~and~\cd{t}.  If {\it I} is an instance of some class
{\it C} named {\it S} and {\it C} is an instance of \cd{standard-class}, the
...
be used in the special form \cd{the} and in type declarations.
...
type \cd{array} has a corresponding class named \cd{array}.  No type specifier
that is a list, such as \cd{(vector double-float 100)}, has a corresponding
class. The form \cd{deftype} does not create any classes.
...
defined by \cd{defclass}), a {\bit structure class\/} (defined
by \cd{defstruct}), or a {\bit built-in class\/} (implemented in
a special, non-extensible way).

A built-in class is one whose instances have restricted capabilities or
special representations.  Attempting to use \cd{defclass} to define 
subclasses of a built-in class signals an error.  Calling 
\cd{make-instance} to create an instance of a built-in class signals an error.
Calling \cd{slot-value} on an instance of a built-in class signals an
error.  Redefining a built-in class or using \cd{change-class} to change
...
\cd{standard-class}, a built-in class is an instance of 
\cd{built-in-class}, and a structure class is an instance of 
\cd{structure-class}.

Each structure type created by \cd{defstruct} without using the 
\cd{:type} option has a corresponding class.  This class is an instance of
\cd{structure-class}.  
%A portable program must assume that 
%\cd{structure-class} is a subclass of \cd{built-in-class} and has the
%same restrictions as built-in classes.  Whether \cd{structure-class}
%in fact is a subclass of \cd{built-in-class} is
%implementation-dependent. 
The \cd{:include} option of \cd{defstruct} creates a direct
...
given type specifier.  For example, \cd{null} is a subtype of both
\cd{symbol} and \cd{list}, but the first edition
did not specify whether \cd{symbol} is more specific or less
specific than \cd{list}.  The CLOS specification defines those
...
\cd{cons}, \cd{symbol}, \cd{array}, \cd{number}, and \cd{character}.
...
The \cd{defclass} form for a class provides a total ordering on that
...
its right in the superclasses list of the \cd{defclass} form.  For
...
they are mentioned in the \cd{defclass} form. These ordered pairs
...
class \cd{pie}.  The following classes are defined:
...
\cd{t}\}$.
The set ${\it R}=\{(\cd{pie},\discretionary{}{}{}
\cd{apple}),\discretionary{}{}{}
(\cd{apple},\discretionary{}{}{}
\cd{cinnamon}),\discretionary{}{}{}
(\cd{cinnamon},\discretionary{}{}{}
...
It is important to understand that \cd{defclass} implicitly appends the
class \cd{standard-object} to the list of superclasses when the metaclass
is \cd{standard-class} (the normal situation),
in order to insure that \cd{standard-object} will be a superclass
of every instance of \cd{standard-class} except \cd{standard-object} itself
...
the class precedence list for \cd{pie},
is unchanged.---GLS]

The class \cd{pie} is not preceded by anything, so it comes first;
the result so far is \cd{(pie)}.  Remove \cd{pie} from {\it S} and pairs
mentioning \cd{pie} from {\it R} to get
...
The class \cd{apple} is not preceded by anything, so it is next; the
result is \cd{(pie~apple)}. Removing \cd{apple} and the relevant
...
The classes \cd{cinnamon} and \cd{fruit} are not preceded by
anything, so the one with a direct subclass rightmost in the class
precedence list computed so far goes next.  The class \cd{apple} is a
direct subclass of \cd{fruit}, and the class \cd{pie} is a direct
subclass of \cd{cinnamon}.  Because \cd{apple} appears to the right
of \cd{pie} in the precedence list, \cd{fruit} goes next, and the
result so far is \cd{(pie apple fruit)}.  ${\it S}=\{\cd{cinnamon},\discretionary{}{}{}
\cd{spice},\discretionary{}{}{}
\cd{food},\discretionary{}{}{}
\cd{standard{\Qhyphen}\discretionary{}{}{}object},\discretionary{}{}{}
...
The class \cd{cinnamon} is next, giving the result so far as 
\cd{(pie apple fruit cinnamon)}.  At this point ${\it S}=\{\cd{spice},\discretionary{}{}{}
\cd{food},\discretionary{}{}{}
\cd{standard{\Qhyphen}\discretionary{}{}{}object},\discretionary{}{}{}
...
The classes \cd{spice}, \cd{food}, \cd{standard-object}, and 
\cd{t} are then added in that order, and the final class precedence list for \cd{pie} is
\begin{lisp}
...
The class \cd{fruit} must precede \cd{apple} because the local
ordering of superclasses must be preserved.  The class \cd{apple} must
precede \cd{fruit} because a class always precedes its own
...
The class precedence list for \cd{pie} is
\begin{lisp}
(pie apple cinnamon standard-object t)
\end{lisp}

The class precedence list for \cd{pastry} is
\begin{lisp}
(pastry cinnamon apple standard-object t)
\end{lisp}

It is not a problem for \cd{apple} to precede \cd{cinnamon} in the
ordering of the superclasses of \cd{pie} but not in the ordering for
\cd{pastry}.  However, it is not possible to build a new class that
has both \cd{pie} and \cd{pastry} as superclasses.
...
used as the first argument to \cd{funcall} and \cd{apply}, and otherwise used in all the ways
...
\cd{defun} construct; a {\bit local\/} name can be given using the
\cd{flet} or \cd{labels} special forms.  A generic function can be
given a global name using the \cd{defmethod} or \cd{defgeneric}
construct.  A generic function can be given a local name using the
\cd{generic-flet}, \cd{generic-labels}, or \cd{with-added-methods}
...
first element is \cd{setf} and whose second element is a symbol.
This is true for both local and global names.

The \cd{generic-flet} special form creates new local generic
functions using the set of methods specified by the method definitions
in the \cd{generic-flet} form.  The scoping of generic function names
within a \cd{generic-flet} form is the same as for \cd{flet}.

The \cd{generic-labels} special form creates a set of new mutually
recursive local generic functions using the set of methods specified
by the method definitions in the \cd{generic-labels} form.  The
scoping of generic function names within a \cd{generic-labels} form
is the same as for \cd{labels}.

The \cd{with-added-methods} special form creates new local generic
functions by adding the set of methods specified by the method
definitions with a given name in the \cd{with-added-methods} form to
...
The \cd{generic-function} macro creates an anonymous generic
...
When a \cd{defgeneric} form is evaluated, one of three actions
is taken:
...
by the current \cd{defgeneric} form are added, and any methods in the
existing generic function that were defined by a previous 
\cd{defgeneric} form are removed.  Methods added by the current 
\cd{defgeneric} form might replace methods defined by \cd{defmethod} or
\cd{defclass}.  No other methods in the generic function are affected
or replaced.
...
methods specified by the method definitions in the \cd{defgeneric}
form.
...
specify generic function options.'' These forms are \cd{defgeneric},
\cd{generic-function}, \cd{generic-flet}, \cd{generic-labels}, and
\cd{with-added-methods}.

Some forms define methods for a generic function.  They will be
referred to as ``method-defining forms.'' These forms are 
\cd{defgeneric}, \cd{defmethod}, \cd{generic-function}, 
\cd{generic-flet}, \cd{generic-labels}, \cd{with-added-methods}, and
\cd{defclass}. Note that all the method-defining forms except 
\cd{defclass} and \cd{defmethod}
...
a name that names a class or a list \cd{(\cd{eql} {\it form\/})}.
A parameter specializer name denotes a parameter specializer as follows:

\begin{itemize}
\item  A name that names a class denotes that class.

\item  The list \cd{(\cd{eql} {\it form\/})} denotes the type specifier
\cd{(\cd{eql} {\it object\/})}, where {\it object\/} is the result of
...
user-level interface (\cd{defmethod}), while parameter specializers
...
\cd{(\cd{eql} {\it form\/})} is not a type specifier, for it contains
...
but not all type specifiers are valid parameter specializers.  Macros such as \cd{defmethod}
...
with such functions as \cd{find-method}.---GLS]
...
specializer defaults to the class named \cd{t}.
...
be a list is \cd{(\cd{eql} {\it object\/})}.  This requires that
Common Lisp define the type specifier \cd{eql}
...
\cd{t} is called a {\bit default method}; it is always applicable but
...
any non-\cd{nil} atom.  The qualifiers defined by standard method combination
...
\cd{define-method-combination}, primary methods are methods qualified with
...
\cd{(\cd{eql} $\hbox{{\it object}}\sub 1$ $\hbox{{\it object}}\sub 2$)}.
...
non-\cd{nil} atoms in the same order. That is, the lists are \cd{equal}.
...
is \cd{t} were supplied, though no such argument pair will be passed.
...
For example, suppose there are two methods defined for \cd{width}
as follows:
...
function definition for \cd{width}. The evaluation of the
...
if the class named \cd{character-picture-class} is a subclass of
both \cd{picture-class} and \cd{character-class}.
...
\cd{no-applicable-method} is invoked.
...
\cd{:argument-precedence-order} option to \cd{defgeneric} or to any of
...
If just one parameter specializer is \cd{(\cd{eql} {\it
object\/})}, the method with that parameter specializer precedes the
other method.  If both parameter specializers are \cd{eql}
forms, the
...
method by using the function \cd{call-next-method}.  The method that
\cd{call-next-method} will call is referred to as the {\bit next
method}.  The predicate \cd{next-method-p} tests whether a next
method exists.  If \cd{call-next-method} is called and there is no
next most specific method, the generic function \cd{no-next-method}
is invoked.
...
makes some of the methods accessible by means of \cd{call-next-method}.
...
\cd{:method-\discretionary{}{}{}combination} option of \cd{defgeneric} or of any of the other
...
using \cd{defmethod} or indirectly by using 
...
\cd{call-next-method} can be used to call the next method.  When the next
...
\cd{no-next-method} is invoked if \cd{call-next-method} is used and
there is no applicable method to call.  The function 
\cd{next-method-p} may be used to determine whether a next method exists.

\item  
If an \cd{:around} method invokes \cd{call-next-method}, the next
most specific \cd{:around} method is called, if one is applicable.
If there are no \cd{:around} methods or if 
\cd{call-next-method} is called by the least specific \cd{:around}
...
An error is signaled if \cd{call-next-method} is used in a
\cd{:before} method.

\item  The most specific primary method is called.  Inside the
body of a primary method, \cd{call-next-method} may be used to call
...
returned value or values.  The generic function \cd{no-next-method}
is invoked if \cd{call-next-method} is used and there are no more
applicable primary methods.  The function \cd{next-method-p} may be
used to determine whether a next method exists.  If 
\cd{call-next-method} is not used, only the most specific primary method
is called.


\item  All the \cd{:after} methods are called in
most-specific-last order.  Their values are ignored.
An error is signaled if \cd{call-next-method} is used in an
\cd{:after} method.
\end{itemize}
...
\cd{call-next-method} in the least specific \cd{:around} method are
...
If only primary methods are used and if \cd{call-next-method} is not
used, only the most specific method is invoked; that is, more specific
methods shadow more general ones. 

\subsubsection{Declarative Method Combination}

The macro \cd{define-method-combination} defines new forms of method
...
\cd{define-method-combination}.  The short form is a simple facility;
the long form is more powerful and more verbose.  The long form
resembles \cd{defmacro} in that the body is an expression that
...
forms of \cd{define-method-combination} are explained in
...
\cd{defgeneric} or to the \cd{:method-combination} option to any of the
other forms that specify generic function options.

The names of the built-in  method combination types are
\cd{+}, \cd{and}, \cd{append}, \cd{list}, \cd{max}, \cd{min}, 
\cd{nconc}, \cd{or}, \cd{progn}, and \cd{standard}.

The semantics of the \cd{standard} built-in method combination type were
...
defined by the short form of \cd{define-method-combination}.  They
...
functions \cd{call-next-method} and \cd{next-method-p} is supported
...
\cd{call-next-method} and \cd{next-method-p} is not supported in primary
methods.
...
\cd{call-next-method} can be used to call the next method.  The
generic function \cd{no-next-method} is invoked if 
\cd{call-next-method} is used and there is no applicable method to call.
The function \cd{next-method-p} may be used to determine whether a
...
\cd{call-next-method}, the next most specific \cd{:around} method is
called, if one is applicable.  If there are no \cd{:around} methods
or if \cd{call-next-method} is called by the least specific 
...
$1\leq {\it j}<{\it i}$, returned \cd{nil}.
...
classes \cd{standard-class}, \cd{built-in-class}, and 
\cd{structure-class}:

\begin{itemize}

\item 
The class \cd{standard-class} is the default class of classes defined
by \cd{defclass}.

\item  The class \cd{built-in-class} is the class whose
...
Common Lisp type
might be an instance of \cd{built-in-class}.
...
\item 
All classes defined by means of \cd{defstruct} are instances of 
\cd{structure-class}.
\end{itemize}
...
meta-objects}. These include the class \cd{standard-object} and
instances of the classes \cd{standard-method}, 
\cd{standard-generic-function}, and \cd{method-combination}.

\begin{itemize}

\item  
The class \cd{standard-method} is the default class of
methods that are defined by the forms \cd{defmethod}, 
\cd{defgeneric}, \cd{generic-function}, \cd{generic-flet}, 
\cd{generic-labels}, and \cd{with-added-methods}.

\item 
The class \cd{standard-generic-function} is the default class of 
generic functions defined by the forms \cd{defmethod},
\cd{defgeneric}, \cd{generic-function}, \cd{generic-flet},
\cd{generic-labels}, \cd{with-added-methods}, and \cd{defclass}.

\item  The class named \cd{standard-object} is an instance of
the class \cd{standard-class} and is a superclass of every class that
is an instance of \cd{standard-class} except itself.

\item  Every method combination object is an instance of a
subclass of the class \cd{method-combination}.
...
initialization.  Each step of \cd{make-instance} is implemented by a
generic function to provide a mechanism for customizing that step.  In
addition, \cd{make-instance} is itself a generic function and thus
...
\cd{:initarg} slot option to \cd{defclass}.  This provides a mechanism
for supplying a value for a slot in a call to \cd{make-instance}.
...
\cd{defclass}.  If an initialization argument is not explicitly provided
as an argument to \cd{make-instance}, the default value form is
evaluated in the lexical environment of the \cd{defclass} form that
...
\cd{:initform} slot option to \cd{defclass}.  If no initialization
argument associated with that slot is given as an argument to 
\cd{make-instance} or is defaulted by \cd{:default-initargs}, this
default initial value form is evaluated in the lexical environment of
the \cd{defclass} form that defined it, and the resulting value is
...
\item  Defining methods for \cd{initialize-instance} and 
\cd{shared-initialize}.  The slot-filling behavior described above is
implemented by a system-supplied primary method for 
\cd{initialize-instance} which invokes \cd{shared-initialize}. The
generic function \cd{shared-initialize} implements the parts of
...
primary method for \cd{shared-initialize} directly implements the
slot-filling behavior described above, and \cd{initialize-instance}
simply invokes \cd{shared-initialize}.
...
can be any symbol, including \cd{nil}.  An initialization argument
...
\cd{make-instance} (after the first argument) form an initialization
argument list.  Error checking of initialization argument names is
disabled if the keyword argument pair whose keyword is 
\cd{:allow-other-keys} and whose value is non-\cd{nil} appears in the
...
supplied, the generic functions \cd{initialize-instance}, 
\cd{shared-initialize}, and \cd{allocate-instance} are called with that
...
primary method for \cd{make-instance} defined for the class 
\cd{standard-class} checks the validity of its initialization arguments
...
valid by the \cd{:initarg} slot option to \cd{defclass}.  The 
...
\begin{itemize}
\item Making an instance of a class: \cd{allocate-instance},
\cd{initialize-instance}, and \cd{shared-initialize}.
Initialization arguments declared valid by these methods are
valid when making an instance of a class.

\item  Re-initializing an instance: the functions \cd{reinitialize-instance}
...
\cd{:allow-other-keys} is \cd{nil}.  The meaning of 
...
does not appear in the arguments to \cd{make-instance} and is not
defaulted by a more specific class.  The default  value form is
evaluated in the lexical environment of the \cd{defclass} form that
supplied it; the result is used as the initialization
argument's value.

The initialization arguments supplied to \cd{make-instance} are combined
...
to \cd{make-instance}, the default  value form is used, just
...
\cd{make-instance} or is defaulted by \cd{:default-initargs}.
...
\cd{initialize-instance} or \cd{shared-initialize} methods.
...
the same slot are given in the arguments to \cd{make-instance}, the
...
explicitly in the arguments to \cd{make-instance}, the initialization
...
in the arguments to \cd{make-instance}, the leftmost argument in the 
...
arguments to \cd{make-instance} appear to the left of defaulted
...
farther to the left in the \cd{default-initargs} class option appears
...
using \cd{:default-initargs} or is supplied to \cd{make-instance},
...
The generic function \cd{shared-initialize} is used to fill the slots
...
The second argument to \cd{shared-initialize} may be one of the following:
...
\item  It can be \cd{nil}, which specifies the empty set of
slot names.

\item  It can be the symbol \cd{t}, which specifies the set of
all of the slots.

\end{itemize}

There is a system-supplied primary method for 
\cd{shared-initialize} whose first parameter specializer is the class 
\cd{standard-object}.  This method behaves as follows on each slot,
...
slots are indicated by the second argument to \cd{shared-initialize}.
...
\cd{defclass} form and the result is stored into the slot.  For example,
...
The generic function \cd{shared-initialize} is called by the
...
The generic function \cd{initialize-instance} is called by 
\cd{make-instance} to initialize a newly created instance.  It uses
standard method combination.  Methods for 
\cd{initialize-instance} can be defined in order to perform any
initialization that cannot be achieved with the simple slot-filling
mechanisms.

\penalty-10000 %required

During initialization, \cd{initialize-instance} is invoked
...
The generic function \cd{initialize-instance} is called with the
new instance and the defaulted initialization arguments.  There is
a system-supplied primary method for \cd{initialize-instance}
whose parameter specializer is the class \cd{standard-object}.  This
method calls the generic function \cd{shared-initialize} to fill in
the slots according to the initialization arguments and the 
\cd{:initform} forms for the slots; the generic function 
\cd{shared-initialize} is called with the following arguments: the instance,
\cd{t}, and the defaulted initialization arguments.

Note that \cd{initialize-instance} provides the defaulted
initialization argument list in its call to \cd{shared-initialize},
so the first step performed by the system-supplied primary method for
\cd{shared-initialize} takes into account both the initialization
arguments provided in the call to \cd{make-instance} and the
defaulted initialization argument list.

Methods for \cd{initialize-instance} can be defined to specify
actions to be taken when an instance is initialized.  If only \cd{:after}
methods for \cd{initialize-instance} are defined, they will be
...
\cd{initialize-instance}.

The \OS\ provides two functions that are useful in the bodies of 
\cd{initialize-instance} methods.  The function \cd{slot-boundp}
...
\cd{initialize-instance} that initialize slots only if they have
not already been initialized.  The function \cd{slot-makunbound}
causes the slot to have no value.


\subsubsection{Definitions of Make-Instance and Initialize-Instance}

The generic function \cd{make-instance} behaves as if it were defined as
...
The elided code in the definition of \cd{make-instance} checks the
...
functions \cd{class-prototype}, \cd{compute-applicable-methods}, 
\cd{function-keywords}, and \cd{class-slot-initargs}. See the third
...
The generic function \cd{initialize-instance} behaves as if it were
...
\cd{defclass}, as well as defining methods for \cd{make-instance}
and \cd{initialize-instance}.  It is also possible to define
methods for \cd{shared-initialize}, which would be invoked by the
generic functions \cd{reinitialize-instance}, 
\cd{update-\discretionary{}{}{}instance-\discretionary{}{}{}for-\discretionary{}{}{}redefined-\discretionary{}{}{}class}, 
\cd{update-\discretionary{}{}{}instance-\discretionary{}{}{}for-\discretionary{}{}{}different-\discretionary{}{}{}class}, and 
\cd{initialize-instance}.  The meta-object level supports additional
customization by allowing methods to be defined on 
\cd{make-instance}, \cd{default-initargs}, and 
\cd{allocate-instance}.  Parts~2 and~3 of the \CLOS\ specification document each of these generic
...
\cd{initialize-\discretionary{}{}{}instance} and \cd{shared-initialize}.  The
...
\cd{class-prototype}, \cd{compute-\discretionary{}{}{}applicable-\discretionary{}{}{}methods}, 
\cd{function-keywords}, and \cd{class-slot-initargs}. In addition,
...
A class that is an instance of \cd{standard-class} can be redefined
if the new class will also be an instance of \cd{standard-class}.
...
\cd{:accessor} option specified by the old \cd{defclass} form is
removed from the corresponding generic function.
Methods specified by the new \cd{defclass} form are added.

% any function specified by the \cd{:constructor}
% option of the old \cd{defclass} form is removed from the
...
instance does not change its identity as defined by the \cd{eq}
...
\cd{defclass} form for the new class. If there is no \cd{:initform}
...
\cd{make-instances-obsolete}.  This two-step process can happen in other
...
its instance argument is the class \cd{standard-object}.  First this
...
\cd{shared-initialize} with the following arguments: the instance,
...
evaluated by \cd{shared-initialize}.

Methods for \cd{shared-initialize} may be defined to customize class
...
class other than \cd{standard-class} that is not a built-in class.
...
The function \cd{change-class} can be used to change the class of an
...
When \cd{change-class} is invoked on an instance, a two-step updating
...
by \cd{change-class} after the first step of the update has been
completed.

The generic function \cd{update-\discretionary{}{}{}instance-\discretionary{}{}{}for-\discretionary{}{}{}different-\discretionary{}{}{}class} is
invoked on two arguments computed by \cd{change-class}.  The first
...
dynamic extent within the generic function \cd{change-class}.  The
second argument is the instance as updated so far by \cd{change-class}
...
takes any number of initialization arguments.  When it is called by
\cd{change-class}, no initialization arguments are provided.

There is a system-supplied primary method for the generic function
\cd{update-\discretionary{}{}{}instance-\discretionary{}{}{}for-\discretionary{}{}{}different-\discretionary{}{}{}class} that has two parameter
specializers, each of which is the class \cd{standard-object}.  First
...
Then it calls the
generic function \cd{shared-initialize} with the following arguments:
...
it is called by \cd{change-class}, the \cd{:initform} forms for slots
...
The generic function \cd{reinitialize-instance} may be used to change
...
The generic function \cd{reinitialize-instance} may be called
...
This description indicates that the generic function \cd{f} 
...
\cd{f} has two required parameters, {\it x}, which must be an
instance of the class \cd{class}, and {\it y}, which can be any
...
method on \cd{f} is a primary method and has no qualifiers.
...
\cf
\begin{tabular}{@{}ll@{}}
\cd{call-next-method~~~~~~~~~~~}&\cd{initialize-instance}\\
\cd{change-class}&\cd{make-instance}\\
\cd{defclass}&\cd{next-method-p}\\
\cd{defgeneric}&\cd{slot-boundp}\\
\cd{defmethod}&\cd{slot-value}\\
\cd{generic-flet}&\cd{with-accessors}\\
\cd{generic-function}&\cd{with-added-methods}\\
\cd{generic-labels}&\cd{with-slots}
\end{tabular}
\end{flushleft}
...
\cf
\begin{tabular}{@{}ll@{}}
\cd{add-method}&\cd{reinitialize-instance}\\
\cd{class-name}&\cd{remove-method}\\
\cd{compute-applicable-methods}&\cd{shared-initialize}\\
\cd{ensure-generic-function}&\cd{slot-exists-p}\\
\cd{find-class}&\cd{slot-makunbound}\\
\cd{find-method}&\cd{slot-missing}\\
\cd{function-keywords}&\cd{slot-unbound}\\
\cd{make-instances-obsolete}&\cd{update-\discretionary{}{}{}instance-\discretionary{}{}{}for-\discretionary{}{}{}different-\discretionary{}{}{}class}\\
\cd{no-applicable-method}&\cd{update-\discretionary{}{}{}instance-\discretionary{}{}{}for-\discretionary{}{}{}redefined-\discretionary{}{}{}class}\\
\cd{no-next-method}&
\end{tabular}
\end{flushleft}
...
\cf
\begin{tabular}{@{}ll@{}}
\cd{call-method}&\cd{method-combination-error}\\
\cd{define-method-combination~~}&\cd{method-qualifiers}\\
\cd{invalid-method-error}&
\end{tabular}
\end{flushleft}
...
\cf
\begin{tabular}{@{}ll@{}}
\cd{class-of}&\cd{print-object}\\
\cd{documentation~~~~~~~~~~~~~~}&\cd{symbol-macrolet}
\end{tabular}
\end{flushleft}

[Note that \cd{describe} appeared in this list in the original CLOS proposal
\cite{SIGPLAN-CLOS,LASC-CLOS-PART-2}, but X3J13 voted in March 1989 \issue{DESCRIBE-UNDERSPECIFIED}
not to make \cd{describe} a generic function after all (see \cd{describe-object}).---GLS]
\end{itemize}
...
The generic function \cd{add-method} adds a method to a generic
...
The modified generic function is returned.  The result of \cd{add-method} 
is \cd{eq} to the {\it generic-function\/} argument.
...
The macro \cd{call-method} is used in method combination.  This macro hides
the implementation-dependent details of how methods are called. It can be used only within
an effective method form, for the name \cd{call-method} is defined only 
within the lexical scope of such a form.

The macro \cd{call-method} invokes the specified method, supplying it
with arguments and with definitions for \cd{call-next-method} and for
\cd{next-method-p}.  The arguments are the arguments that were
supplied to the effective method form containing the invocation of
\cd{call-method}.  The definitions of \cd{call-next-method} and 
\cd{next-method-p} rely on the list of method objects given as the second
argument to \cd{call-method}.

The \cd{call-next-method} function available to the method that
is the first subform will call the first method in the list that
is the second subform.  The \cd{call-next-method} function
...
A list whose first element is the symbol \cd{make-method} and whose
second element is a Lisp form can be used instead of a method object
as the first subform of \cd{call-method} or as an element of the
second subform of \cd{call-method}.  Such a list specifies a method
object whose method function has a body that is the given form.


The result of \cd{call-method} is the value or values returned by
the method invocation.

See \cd{call-next-method}, \cd{define-method-combination}, and \cd{next-method-p}.
\end{defmac}
...
The function \cd{call-next-method} can be used within the body of a
method defined by a method-defining form to call the next method.

The function \cd{call-next-method} returns the value or values
returned by the method it calls.  If there is no next method, 
the generic function \cd{no-next-method} is called.

The type of method combination used determines which 
methods can invoke \cd{call-next-method}.  The standard method
...
\begin{itemize}
\item 
If \cd{call-next-method} is used in an \cd{:around} method,
...
\begin{itemize}
\item  All the \cd{:before} methods are called, in
most-specific-first order.  The function \cd{call-next-method}
cannot be used in \cd{:before} methods.

\item 
The most specific primary method is called.  Inside the body of a
primary method, \cd{call-next-method} may be used to pass control to
the next most specific primary method.  The generic function 
\cd{no-next-method} is called if \cd{call-next-method} is used and there
are no more primary methods.

\item  All the \cd{:after} methods are called in
most-specific-last order.  The function \cd{call-next-method}
cannot be used in \cd{:after} methods.
\end{itemize}
\end{itemize}

For further discussion of the use of \cd{call-next-method}, see
...
When \cd{call-next-method} is called with no arguments, it passes the
current method's original arguments to the next method.  Neither
argument defaulting, nor using \cd{setq}, nor rebinding variables
with the same names as parameters of the method affects the values
\cd{call-next-method} passes to the method it calls.

When \cd{call-next-method} is called with arguments, the next method
is called with those arguments.  When providing arguments to 
\cd{call-next-method}, the following rule must be satisfied or an error is
signaled: The ordered set of methods applicable for a changed set of
arguments for \cd{call-next-method} must be the same as the ordered set of
...
not change the semantics of \cd{call-next-method}.

If \cd{call-next-method} is called with arguments but omits
...
Further computation is possible after \cd{call-next-method} returns.

The definition of the function \cd{call-next-method} has lexical scope (for it
...
the short form of \cd{define-method-combination}, 
\cd{call-next-method} can be used in \cd{:around} methods only.

The function \cd{next-method-p} can be used to test whether or not there is
a next method.

If \cd{call-next-method} is used in methods that do not support it,
an error is signaled.
...
The generic function \cd{change-class} changes the class of an
...
\cd{slot-value} after \cd{change-class} is invoked is \cd{eql} to the
value returned by \cd{slot-value} before \cd{change-class} is
...
invokes \cd{change-class} on {\it instance\/} and 
\cd{(find-class {\it new-class\/})}.


The modified instance is returned.  The result of \cd{change-class}
is \cd{eq} to the {\it instance} argument.
...
After completing all other actions, \cd{change-class} invokes the generic
...
The generic function \cd{change-class} has several semantic difficulties.
...
\cd{change-class} inside a method if any methods for that generic function 
...
See also section~\ref{Classes-SECTION} and \cd{find-class}.
\end{defun}
...
The function \cd{class-of} returns the class of which
the given object is an instance.
The argument to \cd{class-of} may be any Common Lisp object.
The function \cd{class-of} returns the class of which
...
\end{tabbing}
The macro \cd{defclass} defines a new named class.  It returns the new class
object as its result.

The syntax of \cd{defclass} provides options for specifying
...
The {\it class-name\/} argument is a non-\cd{nil} symbol.  It becomes
the proper name of the new class.  If a class with the same proper
name already exists and that class is an instance of 
\cd{standard-class}, and if the \cd{defclass} form for the definition of the
new class specifies a class of class \cd{standard-class}, the definition
of the existing class is replaced.

Each {\it superclass-name\/} argument is a non-\cd{nil} symbol that
...
The {\it reader-function-name\/} argument is a non-\cd{nil}
...
used with \cd{setf} to modify the value of the slot.  The {\it
reader-function-name\/} argument is a non-\cd{nil} symbol.
...
${\it C}\sub1$ will share this single slot unless the \cd{defclass} form
...
in which the \cd{defclass} form was evaluated.  Note that the lexical
...
\cd{make-instance} was called; for shared slots, the dynamic
environment is the dynamic environment in which the \cd{defclass}
form was evaluated.  See section \ref{Object-Creation-and-Initialization-SECTION}.

No implementation is permitted to extend the syntax of \cd{defclass}
...
value in the call to \cd{initialize-instance}, the value will be
...
the initialization argument list supplied to \cd{make-instance}, the
...
environment in which this form is evaluated is the lexical environment
in which the \cd{defclass} form was evaluated.  The dynamic
environment is the dynamic environment in which \cd{make-instance}
...
\cd{type}.  The form \cd{(documentation {\it class-name\/} 'type)}
...
provided by the system (the class \cd{standard-class}).  The {\it
...
an instance of \cd{standard-class}, and if the \cd{defclass} form for
the definition of the new class specifies a class of class 
\cd{standard-class}, the existing class is redefined, and instances of it
...
Note the following rules of \cd{defclass} for standard classes:

\begin{itemize}

\item 
It is not required that the superclasses of a class be defined before
the \cd{defclass} form for that class is evaluated.
...
specializer in a \cd{defmethod} form.
...
The options to \cd{defclass} can be extended.
...
can be accessed only by the function \cd{slot-value}.
...
\end{tabbing}
The macro \cd{defgeneric} is used to define a generic function or to
specify options and declarations that pertain to a generic function as
a whole.

If \cd{(fboundp {\it function-name\/})} is \cd{nil}, a new
...
[X3J13 voted in March 1989 \issue{FUNCTION-NAME} to use \cd{fdefinition}
in the previous paragraph, as shown, rather than \cd{symbol-function},
...
The macro \cd{defgeneric} returns the generic function object 
as its result.




The {\it function-name} argument is a non-\cd{nil} symbol or a
...
\item 
The \cd{declare} option is used to specify declarations that pertain
to the generic function.  The following standard Common Lisp
declaration is allowed:

\begin{itemize}
\item
An \cd{optimize} declaration specifies whether method selection
should be optimized for speed or space, but it has no effect on
methods.  To control how a method is optimized, an \cd{optimize}
declaration must be placed directly in the \cd{defmethod} form or
method description.  The optimization qualities \cd{speed} and 
\cd{space} are the only qualities this standard requires, but an
...
ignores the \cd{optimize} declaration is valid.
\end{itemize}

The \cd{special}, \cd{ftype}, \cd{function}, \cd{inline}, 
\cd{notinline}, and \cd{declaration} declarations are not permitted.
Individual implementations can extend the \cd{declare} option to
...
as a non-standard declaration name in a \cd{declaration} proclamation, it
...
\cd{function}.  The form \cd{(documentation {\it
...
provided by the system (the class \cd{standard-generic-function}).
...
is compatible with the old, \cd{change-class} is called to change the
...
provided by the system (the class \cd{standard-method}).  The {\it
...
short form of \cd{define-method-combination} accept an optional
...
are the same as for \cd{defmethod}.
...
The effect of the \cd{defgeneric} macro is as if the following three
steps were performed: first, methods defined by previous 
\cd{defgeneric} forms are removed; second, \cd{ensure-generic-function}
is called; and finally, methods specified by the current 
\cd{defgeneric} form are added to the generic function. 
...
\cd{defgeneric} specifies the shape of lambda-lists for the methods on
...
\cd{defgeneric} form is evaluated and some methods for that generic
function have lambda-lists that are not congruent with that given in
the \cd{defgeneric} form, an error is signaled.  For further details
on method congruence,
see section~\ref{Congruent-Lambda-Lists-for-All-Methods-of-a-Generic-Function-SECTION}.

Implementations can extend \cd{defgeneric} to include other options.
It is required that an implementation signal an error if
it observes an option that is not implemented locally.

See section~\ref{Congruent-Lambda-Lists-for-All-Methods-of-a-Generic-Function-SECTION}
as well as \cd{defmethod}, \cd{ensure-generic-function}, and \cd{generic-function}.
\end{defmac}
...
\end{tabbing}
The macro \cd{define-method-combination} is used to define new types
of method combination.

There are two forms of \cd{define-method-combination}.  The short
...
verbose.  It resembles \cd{defmacro} in that the body is an
...
non-keyword, non-\cd{nil} symbols are usually used.

\medskip

The short-form syntax of \cd{define-method-combination} is recognized
when the second subform is a non-\cd{nil} symbol or is not present.
...
used with operators such as \cd{progn}, \cd{and}, \cd{+}, and
\cd{max}.
...
method combination.  The function \cd{call-next-method} can be
used only in \cd{:around} methods; it cannot be used in primary methods
defined by the short form of the \cd{define-method-combination} macro.
...
The long-form syntax of \cd{define-method-combination} is recognized 
...
the forms in the body of \cd{define-method-combination}, this
...
a qualifier pattern if the method's list of qualifiers is \cd{equal}
...
because it is a symbol other than \cd{nil} or \cd{*}.

If there is an applicable method whose qualifiers are not valid
for the method combination type, the function \cd{invalid-method-error}
is called.
...
\cd{format} directives, but they are available for generality.  If 
...
non-\cd{nil} and the method group is empty (that is, no applicable
...
it defaults to \cd{nil}.  The {\it boolean\/} argument is not
evaluated.
...
and the functions \cd{method-qualifiers} and 
\cd{invalid-method-error}.  It is permissible to use \cd{setq} on the
...
effective method uses the macro \cd{call-method}.  The definition of this macro has
...
specifiers and a list of next methods, the macro \cd{call-method}
will invoke the method so that \cd{call-next-method} will have available
the next methods.
...
the \cd{call-method} macro whose first subform is a method object and
whose second subform is \cd{nil}.  Each 
\cd{define-method-combination} body is responsible for stripping off
redundant invocations of \cd{progn}, \cd{and}, 
...
The functions \cd{method-combination-error} and 
\cd{invalid-method-error} can be called from the body {\it forms\/} or
...
\cd{define-method-combination}.

\penalty-10000 %required

The value returned by the \cd{define-method-combination} macro is the new
method combination object.


Most examples of the long form of \cd{define-method-combination} also
...
The \cd{:method-combination} option of \cd{defgeneric} is used to
...
\cd{call-method},
\cd{method-qualifiers},
\cd{method-combination-error},
\cd{invalid-method-error},
and \cd{defgeneric}.
\end{defmac}
...
{\it parameter-specializer-name\/} ::= {\it symbol} {\Mor} \cd{(eql {\it eql-specializer-form\/})}
\end{tabbing}
The macro \cd{defmethod} defines a method on a generic function.  

If \cd{(fboundp {\it function-name\/})} is \cd{nil}, a generic
...
\cd{standard-generic-function}), for the method class (the class 
\cd{standard-method}), and for the method combination type (the standard
...
\cd{defmethod} form mentions keyword arguments, the lambda-list of
...
is a non-\cd{nil} atom.  The method combination type may further
...
that the corresponding argument must be \cd{eql} to the object that
...
named \cd{t}.  See section \ref{Introduction-to-Methods-SECTION}.
...
The result of \cd{defmethod} is the method object.
...
\cd{defmethod} replaces the existing method with the one now being
defined.  See
section~\ref{Agreement-on-Parameter-Specializers-and-Qualifiers-SECTION}
for a definition of agreement in this context.
...
The expansion of the \cd{defmethod} macro refers to each
specialized parameter (see the \cd{ignore} declaration specifier), including
parameters that
have an explicit parameter specializer name of \cd{t}.  This means
...
specializes on \cd{t} is not synonymous with an unspecialized
...
contained a specification for \cd{describe} as a generic function.
...
The ordinary function \cd{documentation} (see section~\ref{DOCUMENTATION-SECTION})
is replaced by a generic
function.  The generic function \cd{documentation} returns the
documentation string associated with the given object if it is
available; otherwise \cd{documentation} returns \cd{nil}.


The first argument of \cd{documentation} is a symbol, a
...
documentation string of the \cd{defstruct} structure named by the
symbol.
...
string of the \cd{defsetf} or \cd{define-setf-method} definition
...
unless none is available, in which case \cd{documentation} returns
\cd{nil}.
...
The second argument of \cd{documentation} is a symbol, a
function-name list of the form \cd{(setf {\it symbol\/})}, a
method object, a class object, a generic function object, a method
combination object, or a slot description object.
Whether a third argument should be supplied depends on the
type of the second argument.
See \cd{documentation}.
\end{defun}
...
{\it function-name\/} ::= \Mgroup{{\it symbol\/} {\Mor} \cd{(setf {\it symbol\/})}}
\end{tabbing}
The function \cd{ensure-generic-function} is used to define a
...
\cd{change-class} is called to change the class of the generic function;
...
\cd{defgeneric}, except that the \cd{:method-class} and
...
The function \cd{find-class} returns the class object named by the
given symbol in the given environment.




The first argument to \cd{find-class} is a symbol. 

If there is no such class and the {\it errorp\/} argument is
not supplied or is non-\cd{nil}, \cd{find-class} signals an error.
If there is no such class and the {\it errorp\/} argument is
\cd{nil}, \cd{find-class} returns \cd{nil}.  The default value of
{\it errorp\/} is \cd{t}.
...
The result of \cd{find-class} is the class object named by the given symbol.


The class associated with a particular symbol can be changed by using
\cd{setf} with \cd{find-class}.  The results are undefined if
...
The generic function \cd{find-method} takes a generic function and
...
specializers\/} arguments of \cd{find-method}.
...
elements are the class named \cd{t} must be given.

If there is no such method and the {\it errorp\/} argument is
not supplied or is non-\cd{nil}, \cd{find-method} signals an error.
If there is no such method and the {\it errorp\/} argument is
\cd{nil}, \cd{find-method} returns \cd{nil}.  The default value of
{\it errorp\/} is \cd{t}.


The result of \cd{find-method} is the method object with the given
...
The generic function \cd{function-keywords} is used to return the keyword
...
The generic function \cd{function-keywords} returns two values:
...
The \cd{generic-flet} special form is analogous to the
\cd{flet} special form.  It produces new generic functions and
...
The special form \cd{generic-flet} is used to define generic functions whose
...
The names of functions defined by \cd{generic-flet} have lexical
scope; they retain their local definitions only within the body of the
\cd{generic-flet}.  Any references within the body of the 
\cd{generic-flet} to functions whose names are the same as those defined
within the \cd{generic-flet} are thus references to the local
...
the body of \cd{generic-flet}, not the definitions themselves.
...
\cd{generic-flet}.  It is thus not possible to define recursive functions
with \cd{generic-flet}.
...
the same as for \cd{defgeneric}.

A \cd{generic-flet} local method definition is identical in form to the
method definition part of a \cd{defmethod}.
...
The result returned by \cd{generic-flet} is the value or values
returned by the last form executed.  If no forms are specified, 
\cd{generic-flet} returns \cd{nil}.

See \cd{generic-labels}, \cd{defmethod}, \cd{defgeneric}, and \cd{generic-function}.
\end{defspec}
...
See \cd{defgeneric}, \cd{generic-flet}, \cd{generic-labels}, and \cd{defmethod}.
\end{defmac}
...
The \cd{generic-labels} special form is analogous to the
\cd{labels} special form.  It produces new generic functions and
establishes new lexical function definition bindings.  Each generic
function is created with the set of methods specified by its method
descriptions.

The special form \cd{generic-labels} is used to define generic functions
...
The names of functions defined by \cd{generic-labels} have lexical
scope; they retain their local definitions only within the body of the
\cd{generic-labels} construct.  Any references within the body of the
\cd{generic-labels} construct to functions whose names are the same
as those defined within the \cd{generic-labels} form are thus
...
A \cd{generic-labels} local method definition is identical in form to the
method definition part of a \cd{defmethod}.
...
The result returned by \cd{generic-labels} is the value or values
returned by the last form executed.  If no forms are specified, 
\cd{generic-labels} returns \cd{nil}.

See \cd{generic-flet}, \cd{defmethod}, \cd{defgeneric}, \cd{generic-function}.
\end{defspec}
...
The generic function \cd{initialize-instance} is called by 
\cd{make-instance} to initialize a newly created instance.  The generic
function \cd{initialize-instance} is called with the new instance and
the defaulted initialization arguments.

The system-supplied primary method on \cd{initialize-instance}
...
It does this by calling the generic function \cd{shared-initialize}
with the following arguments: the instance, \cd{t} (this indicates
...
Programmers can define methods for \cd{initialize-instance} to
...
not interfere with the default behavior of \cd{initialize-instance}.
...
The function \cd{invalid-method-error} is used to signal an error
...
using a \cd{format} string and any arguments to it.  Because an
implementation may need to add additional contextual information to
the error message, \cd{invalid-method-error} should be called only
within the dynamic extent of a method combination function.

The function \cd{invalid-method-error} is called automatically when a
...
\cd{invalid-method-error} explicitly if it encounters a method it cannot
accept.
...
given to \cd{format}, and {\it args\/} are any arguments required by
that string.


Whether \cd{invalid-method-error} returns to its caller or exits via
\cd{throw} is implementation-dependent.

See \cd{define-method-combination}.
...
The generic function \cd{make-instance} creates a new
instance of the given class.

The generic function \cd{make-instance} may be used as described in
...
\cd{make-instance} on the arguments \cd{(find-class {\it class\/})} and
{\it initargs}.

The initialization arguments are checked within \cd{make-instance}
(see section~\ref{Object-Creation-and-Initialization-SECTION}).
...
\cd{make-instance} to replace the object-creation protocol.

See section \ref{Object-Creation-and-Initialization-SECTION} as well as
\cd{defclass}, \cd{initialize-instance}, and \cd{class-of}.
\end{defun}
...
The generic function \cd{make-instances-obsolete} is invoked
automatically by the system when \cd{defclass} has been used to
...
The function \cd{make-instances-obsolete} has the effect of
...
If the second of the preceding methods is selected, that method invokes
\cd{make-instances-obsolete} on \cd{(find-class {\it class\/})}.


The modified class is returned.  The result of \cd{make-instances-obsolete}
is \cd{eq} to the {\it class} argument supplied to the first of the preceding
methods.
...
The function \cd{method-combination-error} is used to signal an error
in method combination.  The error message is constructed by using a
\cd{format} string and any arguments to it.  Because an implementation may
need to add additional contextual information to the error message,
\cd{method-combination-error} should be called only within the
...
Whether \cd{method-combination-error} returns to its caller or exits
via \cd{throw} is implementation-dependent.
...
The generic function \cd{method-qualifiers} returns a list of the
...
The locally defined function \cd{next-method-p} can be used within
...
The function \cd{next-method-p} takes no arguments.


The function \cd{next-method-p} returns true or false.


Like \cd{call-next-method}, the function \cd{next-method-p} has 
lexical scope (for it
is defined only within the body of a method defined by a method-defining form)
and indefinite extent.

See \cd{call-next-method}.
...
The generic function \cd{no-applicable-method} is called when a
generic function of the class \cd{standard-generic-function} is invoked
and no method on that generic function is applicable.
The default method signals an error.

The generic function \cd{no-applicable-method} is not intended to be called
...
The {\it generic-function\/} argument of \cd{no-applicable-method} is the
...
The generic function \cd{no-next-method} is called by 
\cd{call-next-method} when there is no next method.  The system-supplied
method on \cd{no-next-method} signals an error.

The generic function \cd{no-next-method} is not intended to be called
by programmers.  Programmers may write methods for it.
...
\cd{call-next-method} for which there is no next method.
...
The generic function \cd{print-object} writes the printed
representation of an object to a stream.  The function 
\cd{print-object} is called by the print system; it should not be called
by the user.

\penalty-10000 %required

Each implementation must provide a method on the class 
\cd{standard-object} and methods on enough other classes so as to ensure
...
\cd{print-object} for their own classes if they do not wish to inherit an
implementation-supplied method.

The first argument is any Lisp object.   The second argument is a
stream; it cannot be \cd{t} or \cd{nil}. 

The function \cd{print-object} returns its first argument, the object.  

Methods on \cd{print-object} must obey the print control special
...
calls \cd{write} (or an equivalent function) recursively if
...
implementations its \cd{print-object} method is not called; in
...
\item X3J13 voted in June 1989 \issue{DATA-IO} to add the following point.
   All methods for \cd{print-object} must obey \cd{*print-readably*},
...
In general, the printer and the \cd{print-object} methods should not
...
\cd{print-object} method is not the original stream but is an
...
All of the existing printing functions (\cd{write}, \cd{prin1}, 
\cd{print}, \cd{princ}, \cd{pprint}, \cd{write-to-string}, 
\cd{prin1-to-string}, \cd{princ-to-string}, the \cd{{\Xtilde}S} and 
\cd{{\Xtilde}A} \cd{format} operations, and the \cd{{\Xtilde}B}, \cd{{\Xtilde}D},
\cd{{\Xtilde}E}, \cd{{\Xtilde}F}, \cd{{\Xtilde}G}, \cd{{\Xtilde}\$}, 
\cd{{\Xtilde}O}, \cd{{\Xtilde}R}, and \cd{{\Xtilde}X} \cd{format} operations when they
encounter a non-numeric value) are required to be changed to go
through the \cd{print-object} generic function.  Each implementation is
required to replace its former implementation of printing with one or
more \cd{print-object} methods.  Exactly which classes have methods for
\cd{print-object} is not specified; it would be valid for an implementation
...
The generic function \cd{reinitialize-instance} can be used to change
...
The system-supplied primary method for \cd{reinitialize-instance}
...
The method then calls the generic function \cd{shared-initialize}
with the following arguments:  the instance, \cd{nil} (which means no slots
...
\cd{:initarg} option to \cd{defclass}, or by defining methods for 
\cd{reinitialize-instance} or \cd{shared-\discretionary{}{}{}initialize}.  The keyword name
of each keyword parameter specifier in the lambda-list of any method
defined on \cd{reinitialize-instance} or \cd{shared-\discretionary{}{}{}initialize} is
...
\ref{Declaring-the-Validity-of-Initialization-Arguments-SECTION} as well as
\cd{initialize-instance}, \cd{slot-boundp},
\cd{update-\discretionary{}{}{}instance-\discretionary{}{}{}for-\discretionary{}{}{}redefined-\discretionary{}{}{}class},
\cd{update-\discretionary{}{}{}instance-\discretionary{}{}{}for-\discretionary{}{}{}different-\discretionary{}{}{}class},
\cd{slot-makunbound}, and \cd{shared-initialize}.
...
The generic function \cd{remove-method} removes a method from a
...
\cd{remove-method} does not signal an error if the method is not one of the
methods on the generic function.


The modified generic function is returned.  The result of \cd{remove-method} 
is \cd{eq} to the {\it generic-function\/} argument.
...
The generic function \cd{shared-initialize} takes the following
...
\cd{shared-initialize} initializes the slots with values according to the
...
\cd{:initform} forms.  For any such slot that has an \cd{:initform} form,
that form is evaluated in the lexical environment of its defining 
\cd{defclass} form and the result is stored into the slot.  For example, if
...
Initialization arguments are declared valid by using the 
\cd{:initarg} option to \cd{defclass}, or by defining methods for 
\cd{shared-initialize}.  The keyword name of each keyword parameter
specifier in the lambda-list of any method defined on 
\cd{shared-initialize} is declared a valid initialization argument
...
\cd{initialize-instance} methods, rather than by handling them in the
primary \cd{initialize-instance} method.  (This optimization might
be implemented by having the \cd{allocate-instance} method copy a
prototype instance.)
...
\cd{standard-object}.  In this case, default initial value forms can be 
...
The function \cd{slot-boundp} tests whether a specific slot in an
instance is bound.
...
The function \cd{slot-boundp} returns true or false.


This function allows for writing \cd{:after}
methods on \cd{initialize-instance} in order to initialize only
those slots that have not already been bound.

If no slot of the given name exists in the instance, \cd{slot-missing}
...
The function \cd{slot-boundp} is implemented using 
\cd{slot-boundp-using-class}.
See \cd{slot-missing}.
\end{defun}
...
The function \cd{slot-exists-p} tests whether the specified object has
...
The function \cd{slot-exists-p} returns true or false.


The function \cd{slot-exists-p} is implemented using 
...
The function \cd{slot-makunbound} restores a slot in an instance to
the unbound state.
...
The arguments to \cd{slot-makunbound} are the instance and the name of
the slot.
...
The function \cd{slot-makunbound} is implemented using 
...
The generic function \cd{slot-missing} is invoked when an attempt is
made to access a slot in an object whose metaclass is 
\cd{standard-class} and the name of the slot provided is not a name of a
slot in that class.  
The default method signals an error.

The generic function \cd{slot-missing} is not intended to be called by
...
The required arguments to \cd{slot-missing} are the class of the object
that is being accessed, the object, the slot name, and a symbol that
indicates the operation that caused  \cd{slot-missing} to be invoked.
The optional argument to \cd{slot-missing} is used when the operation
is attempting to set the value of the slot.


If a method written for \cd{slot-missing} returns values, these
values get returned as the values of the original function invocation.


The generic function \cd{slot-missing} may be called during
evaluation of \cd{slot-value}, \cd{(setf slot-value)}, 
\cd{slot-boundp}, and \cd{slot-makunbound}.  For each
of these operations the corresponding symbol for the {\it operation\/}
argument is \cd{slot-value}, \cd{setf}, \cd{slot-boundp}, and 
\cd{slot-makunbound}, respectively.

The set of arguments (including the class of the instance) facilitates
defining methods on the metaclass for \cd{slot-missing}.
...
The generic function \cd{slot-unbound} is called when an
unbound slot is read in an instance whose metaclass is 
\cd{standard-class}.
The default method signals an error.

The generic function \cd{slot-unbound} is not intended to be called
by programmers.  Programmers may write methods for it.
The function \cd{slot-unbound} is called only by the function
\cd{slot-value-using-class} and thus indirectly by \cd{slot-value}.





The arguments to \cd{slot-unbound} are the class of the instance
whose slot was accessed, the instance itself, and the name of the
slot.


If a method written for \cd{slot-unbound} returns values, these
values get returned as the values of the original function invocation.


An unbound slot may occur if no \cd{:initform} form was
specified for the slot and the slot value has not been set, or if 
\cd{slot-makunbound} has been called on the slot.

See \cd{slot-makunbound}.
\end{defun}
...
The function \cd{slot-value} returns the value contained in the slot
{\it slot-name\/} of the given object.  If there is no slot with that
name, \cd{slot-missing} is called.  If the slot is unbound,
\cd{slot-unbound} is called.

The macro \cd{setf} can be used with \cd{slot-value} to change the value
of a slot. 
...
exists in the instance, \cd{slot-missing} is called as follows: 
\begin{lisp}
(slot-missing (class-of {\it instance\/}) \\
~~~~~~~~~~~~~~{\it instance} \\
~~~~~~~~~~~~~~{\it slot-name\/} \\
...
The function \cd{slot-value} is implemented using 
\cd{slot-value-using-class}.

Implementations may optimize \cd{slot-value} by compiling it in-line.

See \cd{slot-missing} and \cd{slot-unbound}.
\end{defun}


[At this point the original CLOS report \cite{SIGPLAN-CLOS,LASC-CLOS-PART-2}
contained a specification for \cd{symbol-macrolet}.
This specification is omitted here.  Instead, a description
of \cd{symbol-macrolet} appears with those of related constructs in chapter~\ref{CONTRL}.---GLS]
...
methods for it.  This function is called only by the function \cd{change-class}.
...
function \cd{shared-initialize} with the following arguments: the instance,
...
computed by \cd{change-class}.  When \cd{change-class} is invoked on
an instance, a copy of that instance is made; \cd{change-class} then
...
dynamic extent within \cd{change-class}; if it is referenced in any
...
\cd{slot-value} or \cd{with-slots} or by invoking a reader generic
...
ignored by \cd{change-class}.

See the example for the function \cd{change-class}.


Initialization arguments are declared valid by using the 
\cd{:initarg} option to \cd{defclass}, or by defining methods for 
\cd{update-\discretionary{}{}{}instance-\discretionary{}{}{}for-\discretionary{}{}{}different-\discretionary{}{}{}class} or \cd{shared-initialize}.  The
...
\cd{shared-initialize} is declared a valid initialization argument name
...
initialize slots differently from \cd{change-class}.  The default
...
\ref{Rules-for-Initialization-Arguments-SECTION}, and
\ref{Declaring-the-Validity-of-Initialization-Arguments-SECTION} as well as
\cd{change-class} and \cd{shared-initialize}.
\end{defun}
...
activated by \cd{make-instances-obsolete}.
...
When \cd{make-instances-obsolete} is invoked or when a class has been
...
Initialization arguments are declared valid by using the 
\cd{:initarg} option to \cd{defclass} or by defining methods for 
\cd{update-\discretionary{}{}{}instance-\discretionary{}{}{}for-\discretionary{}{}{}redefined-\discretionary{}{}{}class} or \cd{shared-initialize}.  The
keyword name of each keyword parameter specifier in the lambda-list of
...
\ref{Rules-for-Initialization-Arguments-SECTION}, and
\ref{Declaring-the-Validity-of-Initialization-Arguments-SECTION} as well as
\cd{shared-initialize} and \cd{make-\discretionary{}{}{}instances-\discretionary{}{}{}obsolete}.
...
The macro \cd{with-accessors} creates a lexical environment in which
specified slots are lexically available through their accessors as if
they were variables.  The macro \cd{with-accessors} invokes the
appropriate accessors to access the specified slots.  Both \cd{setf}
and \cd{setq} can be used to set the value of the slot.
...
A \cd{with-accessors} expression of the form
\begin{lisp}
...
of \cd{with-accessors}.---GLS]

See \cd{with-slots} and \cd{symbol-macrolet}.
\end{defmac}
...
\vskip 0pt plus 4pt
\noindent
The \cd{with-added-methods} special form
...
The special form \cd{with-added-methods} is used to define functions
whose names are meaningful only locally and to execute a series of
forms with these function definition bindings.


The names of functions defined by \cd{with-added-methods} have lexical
scope; they retain their local definitions only within the body of the
\cd{with-added-methods} construct.  Any references within the body of the
\cd{with-added-methods} construct to functions whose names are the same
as those defined within the \cd{with-added-methods} form are thus
references to the local functions instead of to any global functions
of the same names.  The scope of these generic function definition bindings
includes the method bodies themselves as well as the body of the 
\cd{with-added-methods} construct.
...
specialized-lambda-list\/} arguments are the same as for \cd{defgeneric}.
...
The result returned by \cd{with-added-methods} is the value or values
of the last form executed.  If no forms are specified, 
\cd{with-added-methods} returns \cd{nil}.


If a generic function with the given name already exists, the
lambda-list specified in the \cd{with-added-methods} form must be
...
\cd{with-added-methods} form; otherwise an error is signaled.
...
this behavior differs from that of \cd{defgeneric}.
...
the same default values as the {\it option\/} arguments to \cd{defgeneric}.

See \cd{generic-labels},
\cd{generic-flet},
\cd{defmethod},
\cd{defgeneric},
...
\end{tabbing}
The macro \cd{with-slots} creates a lexical context for referring to
...
it were a lexically bound variable.  Both \cd{setf} and \cd{setq}
can be used to set the value of the slot.

The macro \cd{with-slots} translates an appearance of the slot name as
a variable into a call to \cd{slot-value}.
...
A \cd{with-slots} expression of the form:
\begin{lisp}
...
of \cd{symbol-macrolet} but with peculiar treatment of \cd{special}
and type declarations.  The syntactic changes are reflected in this definition
of \cd{with-slots}.---GLS]

See \cd{with-accessors} and \cd{symbol-macrolet}.
