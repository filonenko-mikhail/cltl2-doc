...
always be the same as the {\it car} of the value of \cdf{/}.
The variable \cd{//} holds the previous value of \cdf{/} (that is, the results
printed two interactions ago), and \cd{///} holds the previous value
of \cd{//}.  Therefore the value of \cdf{**} should always be the same
as the {\it car} of \cd{//}, and similarly for \cdf{***} and \cd{///}.

If the evaluation of \cdf{+} is aborted for some reason,
then the values associated with \cdf{/}, \cd{//}, and \cd{///} are not updated;
they are updated only if the printing of values is at least begun (though not
necessarily completed).
\end{defun}
