...
When \cd{eval} is given a list whose {\it car} is a symbol, it looks
for local definitions of that symbol (by \cd{flet}, \cd{labels},
and \cd{macrolet}); if that fails, it looks for a global definition.
...
it is acceptable for the \cd{defun} special form to expand all macro
calls within its body at the time the \cd{defun} form is executed
...
by \cd{defun}, even when operating in an ``interpretive'' mode.)
...
functional arguments to such functions as \cd{apply}, \cd{funcall},
or \cd{map}; in such situations, the list representing the ``original macro
...
The function \cd{macro-function} determines whether a given symbol
is the name of a macro.  The \cd{defmacro} construct provides
...
{\false} is returned.  The function \cd{macroexpand}
is the best way to invoke the expansion function.

It is possible for {\it both} \cd{macro-function} and \cd{special-form-p}
...
\cd{macro-function} cannot be used to determine whether a symbol names
a locally defined macro established by \cd{macrolet};
\cd{macro-function} can
examine only global definitions.

\cd{setf} may be used with \cd{macro-function} to install
...
to add an optional environment argument to \cd{macro-function}.
...
environment {\it env} by \cd{macrolet} or a global one established as
if by \cd{defmacro},
...
as an ordinary function or as a special form but not as a macro, then
{\false} is returned.  The function \cd{macroexpand} or \cd{macroexpand-1}
is the best way to invoke the expansion function.
...
macro definition as its global function definition; any previous
definition, whether as a macro or as a function, is lost.
One cannot use \cd{setf} to establish a local macro definition;
it is an error to supply a second argument to \cd{macro-function}
when using it with \cd{setf}.
It is an error to attempt to redefine the name of a special form.

See also \cd{compiler-macro-function}.
\end{defun}
\end{newer}
...
\cd{defmacro} is a macro-defining macro that
arranges to decompose the macro-call form in an elegant and useful way.
\cd{defmacro} has essentially the same syntax as \cd{defun}: {\it name} is the
...
The \cd{defmacro} construct arranges to install this expander function,
...
outside the \cd{defmacro} form that would ordinarily be lexically apparent
...
Furthermore, \cd{defmacro} should define the expander function
...
by \cd{defmacro} is implicitly enclosed in a \cd{block} construct
...
as the value of the \cd{defmacro} form.
...
\begin{obsolete}
Like the lambda-list in a \cd{defun}, a \cd{defmacro} {\it lambda-list} may contain
...
supplied-p parameters may be specified, just as for \cd{defun}.
Three additional markers
are allowed in \cd{defmacro} variable lists only.
\end{obsolete}
\begin{new}
...
the function \cd{macroexpand} for the sake of any local
macro definitions that the \cd{macrolet} construct may have
...
\begin{itemize}
\item \cd{\&body} may appear at any level of a \cd{defmacro} lambda-list.
\item \cd{\&whole} may appear at any level of a \cd{defmacro} lambda-list.
...
\item \cd{\&environment} may occur only at the outermost level of a \cd{defmacro}
...
\cd{defmacro}, unlike any other Common Lisp construct that has a lambda-list
as part of its syntax, provides an additional facility known as
{\it destructuring}.
\begin{newer}
See \cd{destructuring-bind}, which provides the destructuring facility separately.
\end{newer}
...
for \cd{dolist} in this manner:
\begin{lisp}
(defmacro dolist ((var listform \cd{\&optional} resultform) \\
~~~~~~~~~~~~~~~~~~\&rest body) \\
~~...)
\end{lisp}
More examples of embedded lambda-lists in \cd{defmacro} are shown below.

Another destructuring rule is that \cd{defmacro} allows any lambda-list
...
For example, the definition skeleton for \cd{dolist} shown above could
...
If the compiler encounters a \cd{defmacro},
...
If this is not the desired effect, the \cd{defmacro} form can be wrapped
in an \cd{eval-when} construct.

It is permissible to use \cd{defmacro} to redefine a macro
...
See \cd{macrolet}, which establishes macro
definitions over a restricted lexical scope.

\begin{newer}
See also \cd{define-compiler-macro}.
\end{newer}
...
or zero.
Using \cd{defmacro}, a definition for such a construct
...
Also note the use of \cd{gensym} to generate a new variable name.
...
function definition of the symbol \cd{arithmetic-if}
...
\end{lisp}
The lambda-expression is produced by the \cd{defmacro} declaration.
The calls to \cd{list} are the (hypothetical) result of the backquote (\cd{{\Xbq}})
...
~~~~~~~~(t x)))
\end{lisp}
and \cd{eval} tries again on this new form.
...
in much the same way that the {\it else} form of a Common Lisp \cd{if} construct
may be omitted:
\begin{lisp}
...
the \cd{defmacro} lambda-list to express the structure of
a complicated macro-call syntax.  If no lambda-list keywords
appear, then the \cd{defmacro} lambda-list is simply a list,
...
~~~~~~~~~my-favorite-tail)
\end{lisp}
The fact that the value of the variable \cd{my-favorite-head}
might happen to be a list is irrelevant here.  It is the macro call
itself whose structure must match that of the \cd{defmacro} lambda-list.

The use of lambda-list keywords adds even greater flexibility.
For example, suppose it is convenient within the expansion
function for \cd{halibut} to be able to refer to the list
...
values for its parameters and also a value for the parameter \cd{head}:
\begin{flushleft}
\cf
\begin{tabular}{@{}ll@{}}
{\rm Parameter}&{\rm Value} \\
...
optional parameter named \cd{a} whose default value is that of the
form \cd{b}, with a supplied-p parameter named \cd{\&rest} (not legal),
and an extraneous symbol \cd{c} in the list (also not legal).  An almost
...
parameters \cd{a} and \cd{b}.  The fully correct definition would be either
\begin{lisp}
...
specifies \cd{a} explicitly then it must also specify \cd{b} explicitly,
...
The \cd{macroexpand} function is the conventional means for
...
call {\it once} and return two values: the expansion and \cd{t}.
...
The initial value of \cd{*macroexpand-hook*} is \cd{funcall},
...
\item
 ...
 macro-call form) won't work because the same (\cd{eq}) macro-call
 form may appear in distinct lexical contexts.  In addition, the macro-call
 form may be a read-only constant (see \cd{quote} and also
...
defined by \cd{symbol-macrolet}; therefore a {\it form} may also be
...
the point is that \cd{eval} also uses \cd{*macroexpand-hook*}.

\cd{macroexpand} is similar to \cd{macroexpand-1},
but repeatedly expands {\it form} until it is no longer a macro call.
(In effect, \cd{macroexpand} simply calls \cd{macroexpand-1} repeatedly
until the second value returned is {\nil}.)
A second value of \cd{t} or {\nil} is returned as for \cd{macroexpand-1},
...
to make the destructuring feature of \cd{defmacro}
...
destructuring-bind lambda-list expression {declaration}* {\,form}*
   ...
   then executes the {\it form\/}s as an implicit \cd{progn}.

A \cd{destructuring-bind} {\it lambda-list} may contain
the lambda-list keywords \cd{\&optional}, \cd{\&rest}, \cd{\&key},
\cd{\&allow-other-keys}, and \cd{\&aux}; \cd{\&body} and \cd{\&whole}
may also be used as they are in \cd{defmacro}, but \cd{\&environment} may
{\it not} be used.  Nested and dotted lambda-lists are also permitted
as for \cd{defmacro}.
The idea is that a \cd{destructuring-bind} {\it lambda-list}
has the same format as inner levels of a \cd{defmacro} lambda-list.
...
define-compiler-macro name lambda-list
  ...
  This is just like \cd{defmacro} except the definition is not stored in the
  symbol function cell of {\it name} and is not seen by \cd{macroexpand-1}.
  It is, however, seen by \cd{compiler-macroexpand-1}.  As with \cd{defmacro}, the
  ...
  macros in the way that \cd{macrolet} defines local macros.)

  A top-level call to \cd{define-compiler-macro} in a file being compiled by
  \cd{compile-file} has an effect on the compilation environment similar to
  that of a call to \cd{defmacro}, except it is noticed as a
...
\cd{function-information}; they are global, and their interaction
with other lexical and global definitions can be reconstructed by
\cd{compiler-macro-function}.  It is up to code-walking programs to decide
...
by \cd{defmacro} is implicitly enclosed in a \cd{block} construct
whose name is the same as the {\it name} of the defined macro;
presumably this applies also to \cd{define-compiler-macro}.
Therefore \cd{return-from} may be used to exit from the function.
\end{newer}
...
compiler-macro-function name &optional env
  ...
  \cd{compiler-macro-function} returns the macro expansion
  function; otherwise it returns \cd{nil}.  The
  lexical environment {\it env} may override any global definition for {\it name}
  by defining a local function or local macro (such as by \cd{flet}, \cd{labels}, or
  \cd{macrolet}) in which case \cd{nil} is returned.

  \cd{setf} may be used with \cd{compiler-macro-function} to install a function as
  the expansion function for the compiler macro {\it name}, in the same manner as for
  \cd{macro-function}.  Storing the value \cd{nil} removes any existing
  compiler macro definition.  As with \cd{macro-function}, a non-\cd{nil} stored value
  must be a function of two arguments, the entire macro call and 
  the environment.  The second argument to \cd{compiler-macro-function} must
  be omitted when it is used with \cd{setf}.
\end{defun}
...
compiler-macroexpand-1 form &optional env
  ...
  These are just like \cd{macroexpand} and \cd{macroexpand-1}
  except that the expander function is obtained as if by a call to
  \cd{compiler-macro-function} on the {\it car} of the {\it form} rather than by a call to
  \cd{macro-function}.
  Note that \cd{compiler-macroexpand} performs repeated expansion
  ...
  and a value that is true if any expansion occurred and \cd{nil} otherwise.

  There are three cases where no expansion happens:
  \begin{itemize}
    \item There is no compiler macro definition for the {\it car} of {\it form}.
    \item There is such a definition but there is also a \cd{notinline}
    ...
    \item A global compiler macro definition is shadowed by a local
        function or macro definition (such as by \cd{flet}, \cd{labels}, or
        \cd{macrolet}).
  \end{itemize}
  Note that if there is no expansion, the original {\it form} is returned as
  the first value, and \cd{nil} as the second value.
  
  Any macro expansion performed by the function \cd{compiler-macroexpand}
...
There is a minimal set of accessors (\cd{variable-information},
\cd{function-information}, and \cd{declaration-information}) and a constructor
(\cd{augment-environment}) for environments.

All of the standard declaration specifiers, with the exception of \cd{special},
can be defined fairly easily using \cd{define-declaration}.  It also
seems to be able to handle most extended declarations.

The function \cd{parse-macro} is provided so that
...
to add \cd{destructuring-bind} to the language.
  However, \cd{parse-macro} is worth having anyway, since any program-analyzing
  program is going to need to define it, and the implementation isn't completely
  trivial even with \cd{destructuring-bind} to build upon.

  The function \cd{enclose} allows expander functions to be defined in a non-null
...
object.  (It is not required that implementations
 ...
 arguments default to \cd{nil}, which represents the local null lexical environment
 ...
 \cd{type-error} if the value of an environment argument is not a syntactic
 environment object.

 The accessor functions \cd{variable-information}, \cd{function-information}, and
 \cd{declaration-information} retrieve information about
 declarations that are in
 effect in the environment.  Since implementations are permitted to ignore
 declarations (except for \cd{special} declarations and \cd{optimize safety}
 ...
 the environment using \cd{augment-environment}.  They might also return
 ...
 identical to the information that was passed to \cd{augment-environment}.
...
The {\it variable} refers to a \cd{symbol-macrolet} binding.

\item[\cd{:constant}]
Either the {\it variable} refers to a named constant defined by
\cd{defconstant} or the {\it variable} is a keyword symbol.
\end{indentdesc}
...
A non-\cd{nil} value indicates that the {\it variable} has been
                declared \cd{dynamic-extent}. If the value is \cd{nil}, the pair
                might be omitted.

\item[\cd{ignore}]
A non-\cd{nil} value indicates that the {\it variable} has been declared
                \cd{ignore}. If the value is \cd{nil}, the pair might be omitted.

\item[\cd{type}]
The value is a type specifier associated with the {\it variable} by a \cd{type}
                declaration or an abbreviated declaration such as
                \cd{(fixnum {\it variable})}.
                If no explicit association exists, either by \cd{proclaim} or
                \cd{declare}, then the type specifier is \cd{t}.  It is permissible for
                implementations to use a type specifier that is equivalent
                to or a supertype of the one appearing in the original
                declaration.  If the value is \cd{t}, the pair might be
                omitted.
\end{indentdesc}
  ...
  or that is otherwise accessible in the \cd{common-lisp-user} package.
  ...
  appear in the a-list as a result of \cd{define-declaration}) are undefined.

  Note that the global binding might differ from the
  local one and can be retrieved by calling \cd{variable-information}
...
\begin{indentdesc}{7pc}
\item[\cd{nil}] There is no apparent definition for {\it function}.
...
\end{indentdesc}
  ...
  local, the second value is true; it is \cd{nil} when the definition is
  global.
...
\begin{indentdesc}{7pc}
\item[\cd{dynamic-extent}]
A non-\cd{nil} value indicates that the function has been
                declared \cd{dynamic-extent}.  If the value is \cd{nil}, the pair
                might be omitted.

\item[\cd{inline}]
The value is one of the symbols \cd{inline}, \cd{notinline}, or \cd{nil} to indicate
                whether the function-name has been declared \cd{inline},
                declared \cd{notinline}, or neither, respectively.
                If the value is \cd{nil}, the pair might be omitted.

\item[\cd{ftype}]
The value is the type specifier associated with the function-name in the
                environment, or the symbol \cd{function} if there is no functional
                type declaration or proclamation associated with the function-name.
                This value might not include all the apparent \cd{ftype}
                declarations for the function-name.  It is permissible for
                implementations to use a type specifier that is equivalent
                to or a supertype of the one that appeared in the original
                declaration.  If the value is \cd{function}, the pair might be
                omitted. 
\end{indentdesc}
  If an implementation supports additional declaration specifiers that
  ...
  that is otherwise accessible in the \cd{common-lisp-user} package.
  ...
  that appear in the a-list as a result of \cd{define-declaration}) are 
  undefined.

  Note that the global binding might differ from the local
  one and can be retrieved by calling \cd{function-information} with a null
  lexical environment.
\end{defun}
...
declaration-information decl-name &optional env
  ...
  It is required that this function recognize \cd{optimize} and \cd{declaration} as
...
a list whose entries are of the form \cd{({\it quality} {\it value})}, where
                ...
                (\cd{speed}, \cd{safety}, \cd{compilation-speed}, \cd{space}, \cd{debug})
...
a list of the declaration names that have been proclaimed as
                valid through the use of the \cd{declaration} proclamation.
                The consequences of destructively modifying this list or
		its elements are undefined.
\end{indentdesc}
  If an implementation is extended to recognize additional
  declaration specifiers in \cd{declare} or \cd{proclaim}, it is required that
  either the \cd{declaration-information} function should recognize those
  declarations also or the implementation should provide a similar accessor that is
  specialized for that declaration specifier.  If \cd{declaration-information}
...
\item[\cd{:variable}]
     The argument is a list of symbols that will be visible as bound variables in
                ...
                as special or lexical depends on \cd{special} declarations recorded
...
\item[\cd{:symbol-macro}]
 The argument is a list of symbol macro definitions, each of the form
                ...
                {\it cadr} of a \cd{symbol-macrolet} special form.  The new environment
                will have local symbol-macro bindings of each symbol to the
                corresponding expansion, so that \cd{macroexpand} will be able to
                ...
                is to wrap a \cd{the} form mentioning the type around the
                definition.
...
\item[\cd{:macro}]
        ...
        Note that the argument is {\it not}
                in the same format as the
                {\it cadr} of a \cd{macrolet} special form.
                ...
                \cd{macro-function} and used by \cd{macroexpand}.
...
\item[\cd{:declare}]
      ...
      Information about these declarations can
                ...
                \cd{variable-information}, \cd{function-information}, and
                \cd{declaration-information}.
\end{flushdesc}
  ...
  also included in a \cd{special} declaration specifier in the \cd{:declare} argument.
  An error is
  signaled if any symbol naming a macro in the \cd{:macro} argument is also included
  in the \cd{:function} argument.
  The condition type of each of these errors is \cd{program-error}.
  ...
  environment argument to \cd{augment-environment}, the consequences are undefined if an
  attempt is made to use the result of \cd{augment-environment} as the environment
  argument for \cd{evalhook}.  The environment
  returned by \cd{augment-environment} can be used only for syntactic analysis, that is,
  as an argument to
  the functions defined in this section and functions such as \cd{macroexpand}.
\end{defun}
...
define-declaration decl-name lambda-list {\,form}*
  ...
  \cd{augment-environment} is extended to support additional declaration
  ...
  \cd{augment-environment}.  This function must return two values.  The
...
then the second value must be a list, the elements of which are lists of the
  ...
  function (either \cd{variable-information} or \cd{function-information}) is applied to
  ...
  \cd{declaration-information} will return {\it value} when applied to the
  {\it key} and the augmented environment.

  \cd{define-declaration} causes {\it decl-name} to be proclaimed to be a
  ...
  defined with \cd{define-declaration} to the macro expansion and \cd{*evalhook*}
  environments.
  ...
  declaration handler defined with \cd{define-declaration} includes a {\it key} name
  ...
  value may not use the symbols \cd{dynamic-extent}, \cd{ignore}, or \cd{type}
  as {\it key} names.)

  The \cd{define-declaration} macro does not have any special compile-time
...
parse-macro name lambda-list body &optional env
  ...
  as \cd{defmacro} and \cd{macrolet}.  It returns a lambda-expression that accepts
  two arguments, a form and an environment.  The {\it name}, {\it lambda-list},
  and {\it body} arguments correspond to the parts of a \cd{defmacro} or \cd{macrolet}
  definition.

  The {\it lambda-list} argument may include \cd{\&environment} and \cd{\&whole}
  and may include destructuring.
  The {\it name}
  argument is used to enclose the {\it body} in an implicit \cd{block} and might also
...
enclose lambda-expression &optional env
  ...
  This function returns an object of type \cd{function} that is equivalent to what
  ...
  lexically visible in {\it env}, any \cd{go} to a tag that is lexically visible in 
  {\it env}, or any \cd{return-from} mentioning a block name that is lexically 
  visible in {\it env}.
\end{defun}  
\end{newer}
