...
\item[\cd{:symbol-macro}]
 The argument is a list of symbol macro definitions, each of the form
                ...
                \emph{cadr} of a \cdf{symbol-macrolet} special operator.  The new environment
...
\item[\cd{:macro}]
        ...
        Note that the argument is \emph{not}
                in the same format as the
                \emph{cadr} of a \cdf{macrolet} special operator.
...
Например, допускается для оператора \cdf{defun} раскрытие всех
...
определённые с помощью \cdf{defun}, даже в режиме <<интерпретации>>.)
...
или это оператор, но не макрос, тогда возвращается {\false}. 
Наилучший способ вызвать функцию раскрытия использовать
\cdf{macroexpand} или \cdf{macroexpand-1}.

Возможно такое, что и \cdf{macro-function}, и \cdf{special-operator-p} обе возвращают
...
Форма обычно используется на верхнем уровне, но её также можно
использовать на неверхнем уровне.
Таким образом, \cdf{defmacro} должна определять функцию раскрытия макроса в
некотором лексическом, а не глобальном окружении.

Тело функции раскрытия, определяемое с помощью \cdf{defmacro}, неявно
оборачивается в конструкцию \cdf{block}, имя которой совпадает с именем
\cdf{name}, определяемого макроса.
...
Переопределение оператора (смотрите таблицу~\ref{SPECIAL-FORM-TABLE})
не допускается.
...
Таким же образом действует и \cdf{if} опуская ветку \emph{else} в случае
истинности условия.
\begin{lisp}
(defmacro arithmetic-if (test neg-form \\*
~~~~~~~~~~~~~~~~~~~~~~~~~\cd{\&optional} zero-form pos-form) \\*
...
\cdf{*macroexpand-hook*}, как функция трёх аргументов с параметрами:
функция раскрытия,
форма \emph{form} и
окружение \emph{env}.
...
\cdf{macroexpand} похожа на \cdf{macroexpand-1}, однако циклично раскрывает форму
