...
\cd{string}, в качестве аргумента может принимать символ, при
...
функцию \cd{string} применительно ко всем аргументам, тип которых не известен заранее.
...
\cd{string-trim} returns a substring of \emph{string}, with all characters in
...
\cd{string-upcase} returns a string just like \emph{string} with all lowercase
...
\cd{string-capitalize} produces a copy of \emph{string} such that,
...
\begin{obsolete}
If \emph{x} is a string character (a character of type \cd{string-char}),
