...
Если \emph{result-type} является строкой (\cdf{string}), тогда под ним
понимается \cd{(vector character)}, и под \cdf{simple-string} понимается
\cd{(simple-array character (*))}.
