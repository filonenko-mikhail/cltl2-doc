...
this type if and only if it is \cdf{eql} to \textit{object}.  While
...
только тогда, когда он \cdf{eql} для первого \textit{объекта}. Несмотря на то, что
...
\cdf{short-float}.  This does not \textit{require} \cdf{make-array} to create
...
\cdf{short-float}. Это \textit{не потребует} от \cdf{make-array} создать объект типа
...
\textit{declaration} and \textit{discrimination}.  Declaring to \cdf{make-array}
...
The \textit{value-type} may be a \cdf{values} type specifier
...
\cd{\&key}. \textit{тип-значения} может быть спецификатором типа \cdf{values} в
...
as the \textit{value-type} in a \cdf{function} type specifier or in
...
для спецификатора типа \textit{функции} и в специальной форме \cdf{the}. Данный
...
Тип \cdf{fixnum} является именем для \cd{(integer \textit{наименьшее}
...
This is very similar to a \cdf{defmacro} form: \textit{name} is the
...
Данный макрос весьма схож с формой \cdf{defmacro}: \textit{name} является
...
\textit{lambda-list}. Затем будут выполнены формы тела, как неявный \cdf{progn},
...
\cdf{deftype} differs from \cdf{defmacro} in that if no \textit{initform}
...
\begin{newer}
X3J13 voted in March 1989 \issue{CHARACTER-PROPOSAL}
to specify that if the \textit{result-type} is \cdf{string}
...
is specified, and \textit{object} is not already a \cdf{float} of some kind, then
...
A list \textit{x} whose \textit{car} is the symbol \cdf{lambda}
...
ошибка.
Список \textit{x}, чей \textit{car} является символом \cdf{lambda} приводится к
...
where the \cdf{defstruct} had the name \textit{name} and no \cd{:type} option,
\cdf{type-of} will return \textit{name}.
...
функции-конструктора, и \cdf{defstruct} имело имя \textit{name} и не имело опции
\cd{:type}, \cdf{type-of} вернет \textit{name}.
