...
Common Lisp предоставляет несколько различных представлений для чисел.
...
\cdf{//} or \cdf{quotient} operator does.  In most Lisp systems,
...
$ \emph{x}=\cis {2 \pi \over 3}= -{1 \over 2} + {\sqrt{3} \over 2}\emph{i} $.
Then $ \sqrt{\emph{x}^3} = \sqrt{1} = 1 $, but
$ \emph{x}^{3 / 2} = \emph{e}^{(3/2) \log {x}}
   = \emph{e}^{(3/2) (2\pi/3) i} = \emph{e}^{\pi i} = -1 $.
...
\begin{tabbing}
\hskip 10pc\=\kill
Arc tangent\>$ -\emph{i}\log \left((1+\emph{i}\emph{y}) \sqrt{1/(1+\emph{y}^2)}\right) $
\end{tabbing}
\end{obsolete}
...
Hyperbolic arc cosine\>$ \log \left(\emph{z}+(\emph{z}+1)\sqrt{(\emph{z}-1)/(\emph{z}+1)}\right) $ \\[2pt]
Hyperbolic arc tangent\>$ \log \left((1+\emph{z})\sqrt{1-1/\emph{z}^2}\right) $\`{\bf WRONG!}
\end{tabbing}
\end{obsolete}
...
\begin{tabbing}
$ \arctan \emph{z}=-\emph{i} \log \left((1+\emph{i} \emph{z}) \sqrt{1/(1+\emph{z}^2)}\right)$
\end{tabbing}
}
...
axis below $-\emph{i}$ (exclusive), continuous with quadrant IV.  
...
\noindent\leavevmode\vtop{
\begin{tabbing}
$ \arctanh \emph{z}=\log \left((1+\emph{z})\sqrt{1-1/\emph{z}^2}\right) $\`{\bf WRONG!}
\end{tabbing}
}
\end{obsolete}
...
$-\pi \emph{i}/2$ and $\pi \emph{i}/2$ excluded.
\end{obsolete}
