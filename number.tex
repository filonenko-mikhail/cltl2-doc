...
\section{Precision, Contagion, and Coercion Точность, неявное приведение и явное
  приведение}
...
\section{Irrational and Transcendental Functions Иррациональные и
...
Если все переданные в функцию аргументы приндлежат типу \cd{(or~rational
...
не указано иное, реализация может вернуть или точный результат типа \cd{(or
  rational (complex rational))} или приближененный типа с плавающей точкой с
...
все аргументы становятся с плавающей точкой или комлексным числом с плавающей
...
корнем является апроксимированное комплексное число \cd{\#C(1.0 1.73205)}.
\end{defun}
...
(sqrt -9.0) \EV\ \#c(0.0 3.0)
\end{lisp}
В зависмости от реализации результат \cd{(sqrt 9)} может быть как \cd{3}, так и
...
(signum \emph{x}) \EQ\ (if (zerop \emph{x}) \emph{x} (/ \emph{x} (abs \emph{x})))
\end{lisp}
Для рационального числа, \cdf{signum} будет возвращаеть один из вариантов:
\cd{-1}, \cd{0} или \cd{1}, в соответсвии с тем, является ли число
...
комплесных функции в соотвествие с предложениями для комплексных функций в APL
...
\cdf{floor} преобразовывает аргумент путем отсечения к отрицательной
...
\cdf{ceiling} преобразовывает аргумент путем отсечения к положительной
...
\cdf{ceiling} преобразовывает аргумент путем отсечения к нулю, то есть,
...
\cdf{round} преобразовывает аргумент округлением к ближайшему целому числу. Если
...
число с плавающей точкой. Это примерно, как если бы \cdf{ffoor} передала
аругменты в \cdf{floor}, а затем применила к первому результату \cdf{float} и
...
\cdf{float-precision} будут такими же. Но в случае с ненормализованными числами
...
Третье зачение \cdf{integer-decode-float} будет \cd{1} или \cd{-1}.
...
\cdf{logtest} является предикатом, который истинен, если любой бит определенный
...
\section{Byte Manipulation Functions Функции для манипуляции с байтами}
...
(logbitp \emph{j} (ldb (byte \emph{s} \emph{p}) \emph{n})) \EQ\ (and (< \emph{j} \emph{s}) (logbitp (+ \emph{j} \emph{p}) \emph{n}))
\end{lisp}
Имя функции \cdf{ldf} озаначает <<load byte (загрузить байт)>>.
