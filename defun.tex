...
%  \noindent\leavevmode     % Need this to trigger the \everypar for margin rules.
  ...
  \catcode`&=11
  \def\@lbracehack{{$\,\lbrace$}\=\+\@backslashsetup\it}\relax
  \def\@rbracehack{\-
       \@ifnextchar*{\@rbracesuper}{\@ifnextchar+{\@rbracesuper}{\/$\rbrace\,$\@backslashsetup\it}}}\relax
  \def[{{$\,\lbrack$}\=\+\@backslashsetup\it}\relax
  \def]{\-$\/\rbrack\,$\@backslashsetup\it}\relax
  \def<{{$\,\dlbrack$}\=\+\@backslashsetup\it}\relax
  \def>{\-$\/\drbrack\,$\@backslashsetup\it}\relax
  \def({{\cf \char40}\=\+\@backslashsetup\it}\relax
  \def){\-{\cf \char41}\@backslashsetup\it}\relax
  \def|{{$\char124$}}\relax
  \def?{{$\downarrow$}}\relax
  \def\@finishdefmac{\-\end{tabbing}\endgroup\everypar{}
       \addvspace{6pt}\noindent}\relax
  \expandafter\def\@carret{\expandafter\@ifnextchar\@carret
     {\@finishdefmac\@gobblecr}{\@tabcr*\@backslashsetup\it}}\relax
  \@defunhackbraces
  \def\@backslashsetup{\def\\{\-\@tabcr*\@backslashsetup
...
\def\@defmacname#1#2 {%\typeout{#1: #2 }\relax
  ...
  \leavevmode\hbox to 0pt{\hbox to \textwidth{\hss\box0}\hss}{\cf #2\ }\=\+\@backslashsetup
  \@defunindex#2:\par{#1}
  \it}
