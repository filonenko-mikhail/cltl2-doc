...
\begin{newer}
Теперь есть много реализаций Common Lisp'а, некоторые были запрограммированны
...
(+ 4 5) \EV\ 9
\end{lisp}
означает <<результат вычисления кода \cd{(+ 4 5)} является (или будет, или был)
\cd{9}>>.
...
объектой системы Common Lisp'а (CLOS) использована в разделе~\ref{CLOS}.
Это дает представление о том, как ошибочные ситуации будут описаны,
...
показывают документирование специальных форм и макросов, предназначения котороых
тесно связаны.
...
В Common Lisp'е некоторые строковые символы используется в определенных целях. Полное
...
form} является сокращением для \cd{(quote \emph{form})}. 
...
для использования в качестве \emph{макросимволов} для пользовательских расширений
синтаксиса (см. раздел~\ref{MACRO-CHARACTERS-SECTION}).

\begin{newer}
X3J13 проголосова в Июне 1989 \issue{READ-CASE-SENSITIVITY}
ввести \cdf{readtable-case}. Данные настройки позволяют именам символов быть
