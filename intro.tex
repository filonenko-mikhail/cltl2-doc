...
переопределить значение последовательности букв \cd{nil}, которое будет 
...
Test function for testing label
\end{defun}
...
\begingroup\normalsize
\begin{defun}*[Function]
sample-function arg1 arg2 &optional arg3 arg4 &key (as 123) bv &rest tt ee
gg

The function \cd{sample-function} adds together \textit{arg1} and \textit{arg2}, and
...
\textit{arg4} (which defaults to the symbol \cd{foo}).
For example: 

Функция \cd{sample-function} складывает вместе \textit{arg1} и \textit{arg2} и
...
the last iteration.
The name \textit{name}, if supplied, may be used in a \cd{return-from} form
...
Имя \textit{name}, если задано, может быть использовано в \cd{return-from} форме
...
\\
Вычисляет выражения, как тело \cd{prog} с переменной \textit{var} связанной со
значением \cd{43}.
\begin{lisp}
