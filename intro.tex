...
ﬁxnum. Тем не менее, для концептуальной целостности языка важно то, что
...
значение <<ложь>>. Запись \cd{'nil} (явный знак кавычки) используется для
обозначения символа.
Например:
\begin{lisp}
~~~~~~~~~~~~~~~~~~~~~~~~~~~\=\kill
...
(push x v) \EX\ (setf v (cons x v))
\end{lisp}
означает <<результатом раскрытия формы с макросом \cd{(push x v)}
...
Однако, это не говорит о том, что некоторые реализации не могут определять
...
форма \cdf{sample-special-form} должна выполняться. Значение должно быть всего
...
\cd{\&optional}/\cd{\&rest}/\cd{\&key} синтаксис задаёт то, как функция
...
именно к Lisp процедуре, которая называется \cdf{read}. Она читает символы из
...
влияющий на синтаксическую конструкцию. Например, \cd{A{\Xbackslash}(B} означает
...
принадлежит символ. Например, \cd{network:reset} показывает, что символ с
...
Была введена переменная \cdf{readtable-case}. Данные настройки позволяют именам символов быть
