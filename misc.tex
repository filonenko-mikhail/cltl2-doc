...
the compiler, the \cd{documentation}
function, debugging aids, environment inquiries (including facilities
for calculating and measuring time), and the \cd{identity} function.
...
and \cd{\#,} becomes important, and the \cd{eval-when} construct
...
Unlike most other Lisp dialects, Common Lisp recognizes \cd{special}
...
the \cd{symbol-function} of the symbol is extracted and compiled.
...
the behavior of \cd{compile} to compile such functions properly, but
portable programs may not depend on this capability.)  The consequences
of calling \cd{compile} on a function that is already compiled
are unspecified.
\end{newer}
...
\>;{\rm Now \cd{foo} runs faster (maybe)} \\[4pt]
(compile {\false} \\
...
\cd{compile} returns two additional values
indicating whether the compiler issued any diagnostics
(see section~\ref{COMPILER-DIAGNOSTICS-SECTION}).
\end{newer}

\begin{newer}
X3J13 voted in March 1989 \issue{FUNCTION-NAME} to extend \cd{compile}
to accept as a {\it name} any function-name (a symbol or a list
whose car is \cd{setf}---see section~\ref{FUNCTION-NAME-SECTION}).
One may write \cd{(compile '(setf cadr))} to compile the \cd{setf}
...
to \cd{compile-file} by analogy with \cd{load}.
...
\cd{*compile-verbose*}), if true, permits \cd{compile-file} to print a message
...
as the {\it input-pathname} argument to \cd{compile-file} has implementation-dependent consequences;
\cd{compile-file} might signal an error, for example,
or might compile all files that match the wild pathname.
\end{newer}

\begin{newer}
X3J13 voted in June 1989 \issue{PATHNAME-LOGICAL} to require \cd{compile-file}
...
\begin{newer}
X3J13 voted in June 1989 \issue{COMPILER-DIAGNOSTICS} to specify that
\cd{compile-file} returns three values: the \cd{truename} of the output
file (or \cd{nil} if the file could not be created) and two values
indicating whether the compiler issued any diagnostics
(see section~\ref{COMPILER-DIAGNOSTICS-SECTION}).
\end{newer}
...
\cd{compile-file}, like \cd{load}, rebinds \cd{*package*} to its current value.  If
some form in the file changes the value of \cd{*package*},
...
\begin{itemize}
\item The value of \cd{*package*} when a top-level form in the file is processed
      by \cd{compile-file} must be the same as the value of \cd{*package*} when the
      code corresponding to that top-level form in the compiled file is
      executed by the loader.  In particular,
      any top-level form in a file that alters the value of \cd{*package*}
          ...
          \cd{in-package}, then the value of \cd{*package*} at the time \cd{load} is
          called must be a package with the same name as the package that
          was the value of \cd{*package*} at the time \cd{compile-file} was called.

\item For every symbol appearing lexically within a top-level form that
      was accessible in the package that was the value of \cd{*package*}
      ...
      load-time \cd{*package*} and in the package with the same name as the
...
\item For every symbol in the compiled file that was an external symbol in
      ...
      at load time.
\end{itemize}
  If any of these conditions do not hold, the package in which \cd{load} looks
...
to specify that \cd{compile-file} must bind \cd{*readtable*} to its current value
at the time \cd{compile-file} is called; the dynamic extent of the binding
...
to require that \cd{compile-file} bind two new variables
...
to \cd{compile-file}.  Its initial value is implementation-dependent.
...
it is initially \cd{nil} but \cd{compile-file} binds it to a pathname that
represents the file name given as the first argument to \cd{compile-file} merged
...
it is initially \cd{nil} but \cd{compile-file} binds it to the ``true name'' of
the pathname of the file being compiled.  See \cd{truename}.
\end{defun}
\end{newer}
...
X3J13 voted in March 1989 \issue{LOAD-TIME-EVAL} to add
   ...
   If a \cd{load-time-value} expression is seen by \cd{compile-file}, the compiler
   ...
   If a \cd{load-time-value} expression appears within a function compiled
   with \cd{compile}, the {\it form} is evaluated at compile time in a null lexical
   environment.  The result of this compile-time evaluation is treated as 
   an immediate quantity in the compiled code.  

   In interpreted code, {\it form} is evaluated (by \cd{eval}) in a null
   ...
   \cd{eval} may evaluate the {\it form} only once, at the time this
  ...
  If the same (as determined by \cd{eq}) list \cd{(load-time-value {\it form})} is
  ...
  substructure and when the same expression is passed to \cd{eval} or to
  \cd{compile} multiple times.  Since a \cd{load-time-value} expression may be
  ...
  If two lists \cd{(load-time-value {\it form})} are \cd{equal} but not \cd{eq}, their
  ...
  of these forms is not permitted.
   ...
   may be considered a
   read-only constant. If \cd{nil} (the default), the result must be considered
   ordinary, modifiable data. If \cd{t}, the result is a read-only quantity
   ...
   not evaluated and only the literal symbols \cd{t} and \cd{nil} are permitted.
   ...
   whereas a \cd{load-time-value} form must appear outside quoted structure in a
   for-evaluation position.

   See \cd{make-load-form}.
\end{defspec}
\end{newer}
...
to clarify that when \cd{disassemble} compiles a function, it never
installs the resulting compiled-function object in the
\cd{symbol-function} of a symbol.
\end{newer}

\begin{newer}
X3J13 voted in March 1989 \issue{FUNCTION-NAME} to extend \cd{disassemble}
to accept as a {\it name} any function-name (a symbol or a list
whose car is \cd{setf}---see section~\ref{FUNCTION-NAME-SECTION}).
Thus one may write \cd{(disassemble '(setf cadr))} to disassemble the \cd{setf}
expansion function for \cd{cadr}.
\end{newer}
\end{defun}
...
to the function \cd{compile} or for use in the \cd{function} special form.
...
in a \cd{defun} or \cd{function} special form, for example).

\beforenoterule
\begin{implementation}
An implementation is always free to return the values
{\false}, \cd{t}, {\false} from this function but is encouraged to
...
functions produced by ``in-core'' calls to \cd{eval},
\cd{compile}, or \cd{defun} to retain the defining lambda-expression
for debugging purposes.  The function \cd{function-lambda-expression}
...
to add \cd{with-compilation-unit}, which
   executes the body forms as an implicit \cd{progn}. Within the dynamic context
   ...
   to \cd{with-compilation-unit}. The results are the same as those of
   the last of the forms (or \cd{nil} if there is no {\it form}).
   ...
   the default value for this option is \cd{nil}.
   If \cd{with-compilation-unit} forms are nested dynamically, only the outermost
   such call has any effect unless the \cd{:override} value of an
   inner call is true.

  The function \cd{compile-file} should
  ...
  as an expansion into a simple \cd{progn}.
\end{defmac}
\end{newer}
...
that \cd{compile} and \cd{compile-file}
...
during compilation.
First, note that
\cd{error} and \cd{warning} conditions may be signaled either by the compiler itself
or by code being processed by the compiler (for example, arbitrary errors may 
    occur during compile-time macro expansion or processing of \cd{eval-when}
    forms).
Considering only those conditions signaled {\it by the compiler} (as
    opposed to {\it during compilation}):
\begin{itemize}

\item   Conditions of type \cd{error} may be signaled by the compiler in
...
\item  Conditions of type \cd{warning} may be signaled by the compiler in 
        ...
        Examples of such situations may include
            ...
            altering or rebinding a constant defined with \cd{defconstant},
            calls to built-in Lisp functions with too few or too many arguments
                or with malformed keyword argument lists,
            referring to a variable declared \cd{ignore}, or
            unrecognized declaration specifiers.

\item  The compiler is permitted to signal diagnostics about matters of
        programming style as conditions of type \cd{style-warning}, a subtype
    of \cd{warning}.  Although 
        a \cd{style-warning} condition {\it may} be signaled in these situations, no 
        implementation is {\it required} to do so.  However, if an 
        implementation does choose to signal a condition, that condition 
        will be of type \cd{style-warning} and will be signaled by a call to 
        the function \cd{warn}.
        Examples of such situations may include
            ...
            calls to functions (other than built-in functions)
                ...
                or with malformed keyword argument lists,
            unreferenced local variables not declared \cd{ignore}, or
            standard declaration specifiers that are ignored by 
                the particular compiler in question.
\end{itemize}

Both \cd{compile} and \cd{compile-file} are permitted (but not
    required) to establish a handler for conditions of type \cd{error}.
    Such a handler
...
The functions \cd{compile} and \cd{compile-file} each return three values.
See the definitions of these functions for descriptions of the first value.
    The second value is \cd{nil} if no compiler diagnostics were issued, and
    true otherwise.
    The third value is \cd{nil} if no compiler diagnostics other than style
    warnings were issued; a non-\cd{nil} value indicates that there were 
    ``serious'' compiler diagnostics issued or that other conditions of
    type \cd{error} or \cd{warning} (but not \cd{style-warning}) were signaled during
    compilation.
\end{newer}
...
If a function is of type \cd{compiled-function}, then
all macro calls appearing lexically within the function have 
        ...
        compilation effectively turns every \cd{macrolet} or \cd{symbol-macrolet}
        construct into a \cd{progn} (or a \cd{locally}) with all
        instances of the local macros in the body fully expanded.

If a function is of type \cd{compiled-function}, then
all \cd{load-time-value} forms appearing lexically within the function have
...
Implementations are free to classify every function as 
   a \cd{compiled-function} provided that all functions
satisfy the preceding requirements.
Conversely, it is permissible for a function that is
      not a \cd{compiled-function} to satisfy the preceding requirements.
  
If one or more functions are defined in a file that is compiled
      with \cd{compile-file} and the compiled file is subsequently loaded
by the function \cd{load},
the resulting loaded function definitions must be of
    type \cd{compiled-function}.
  
The function \cd{compile} must produce an object of type
      \cd{compiled-function} as the value that is either returned
or stored into the \cd{symbol-function} of a symbol argument.
...
system with respect to certain actions taken by \cd{compile}, \cd{compile-file}, and
\cd{load}.
\end{newer}
...
\item In conforming code, macros referenced in the code being compiled
  ...
  (This implies that \cd{setf} methods must also be available at
	compile time.)

\item In conforming code, proclamations for \cd{special} variables must
...
\item The compiler may assume that functions that are defined and
	declared \cd{inline} in the compile-time environment will retain the
...
\item The compiler may assume that, within a named function, a
	...
	same function, unless that function has been declared \cd{notinline}.
...
\item In the absence of \cd{notinline}
	declarations to the contrary,
 \cd{compile-file} may assume that a call within the file being compiled to a named
...
\item The compiler may assume that the signature (or ``interface contract'') of
	...
	they had been proclaimed \cd{inline}.

\item The compiler may assume that the signature (or ``interface contract'') of
	functions with \cd{ftype} information available will not change.

\item The compiler may ``wire in'' (that is, open-code or inline)
the values of symbolic constants
	that have been defined with \cd{defconstant} in the compile-time
	environment.

\item The compiler may assume that any type definition made with \cd{defstruct} 
        or \cd{deftype} in the compile-time environment will retain the same 
        definition in the run-time environment.  It may also assume that
        a class defined by \cd{defclass} in the compile-time environment will
...
Calls to defining macros such as \cd{defmacro} or \cd{defvar} appearing
    within a file being processed by \cd{compile-file} normally have
    ...
    more \cd{eval-when} forms and that compile-time
...
The user must ensure that the body of a \cd{deftype} form is
    ...
    \cd{satisfies} of a named function that isn't defined in the compile-time
...
\item[\cd{defun}]
No required compile-time side effects are associated with \cd{defun} forms.
    In particular, \cd{defun} does not make the function definition available
    ...
    \cd{inline} expansion of the function.

\item[\cd{defvar} and \cd{defparameter}]
The compiler must recognize that the variables
    named by these forms have been proclaimed \cd{special}.  However, it must
    not evaluate the {\it initial-value} form or \cd{set} the variable at compile
    time.
...
The compiler must make \cd{setf} methods
    available so that they may be used to expand calls to \cd{setf} later on in
    the file.  Users must ensure that the body of a call
    to \cd{define-setf-method} or
    the complex form of \cd{defsetf} is evaluable at compile time if the
    corresponding place is referred to in a subsequent \cd{setf} in the same
    file.  The compiler must make these \cd{setf} methods available to 
    compile-time calls to \cd{get-setf-method} when its environment argument is
...
The compiler must make the structure type name recognized
    ...
    for \cd{deftype}) and
    make the structure slot accessors known to \cd{setf}.
    In addition, the
    compiler must save enough information so that
    further \cd{defstruct} definitions can include (with the \cd{:include}
    option) a structure type defined
    earlier in the file being compiled.  The functions that \cd{defstruct}
...
The rules are essentially the same as those for
    \cd{defstruct}. The compiler must make the condition type recognizable as a
    ...
    \cd{define-condition} form in the file being compiled.

\item[\cd{defpackage}]
 All of the actions normally performed by the \cd{defpackage} macro at load
...
information about definitions
    ...
    during or after compilation) or during subsequent calls to \cd{compile} or
    \cd{compile-file}.  For example, the following code is not portable because
    it assumes that the compiler stores the macro definition of \cd{foo} where
...
\end{lisp}
    ...
    definition within the \cd{eval-when} form:
\begin{lisp}  
...
to add a new macro \cd{declaim} for making proclamations recognizable
at compile time.  The declaration specifiers in the \cd{declaim} form
are effectively proclaimed at compile time so as to affect
compilation of subsequent forms.  (Note that compiler processing
of a call to \cd{proclaim}
does not have any compile-time side effects, for \cd{proclaim}
is a function.)
\end{flushdesc}
...
all of the actions normally performed by the \cd{in-package} macro at load
time must also be performed at compile time.
\end{flushdesc}
...
can be used as a specializer in subsequent \cd{defmethod} forms.
Thus the compile-time behavior of \cd{defclass} is similar to that of
\cd{deftype} or \cd{defstruct}.

\item[\cd{defgeneric}]
The generic function can be referenced in subsequent \cd{defmethod} forms,
...
The compiler does not arrange for the method to be callable at compile
    ...
    compile time, compiling a \cd{defmethod} form does not add the method to that 
    generic function; the method is added to the generic
    function only when the \cd{defmethod} form is actually executed.

    The error-signaling behavior described in the specification of
    \cd{defmethod} in chapter~\ref{CLOS} (if the function isn't a generic function
    or if the lambda-list is not congruent) occurs only when the defining
    form is executed, not at compile time.

    The forms in \cd{eql} parameter specializers are evaluated when the \cd{defmethod}
    form is executed.  The compiler is permitted to build in knowledge
    about what the form in an \cd{eql} specializer will evaluate to in cases
    where the ultimate result can be syntactically inferred without
    actually evaluating it.

\item[\cd{define-method-combination}]
The method combination can be used in subsequent \cd{defgeneric} forms.  

    The body of a \cd{define-method-combination} form is evaluated no earlier 
...
An object may be used as a quoted constant processed by \cd{compile-file}
...
as constants in code processed with \cd{compile-file}.  Conforming
...
The following terms are used throughout this section.
  ...
  not a named constant defined by \cd{defconstant}.
  The term {\it source code} is used to refer to the objects constructed
  when \cd{compile-file} calls \cd{read} (or the equivalent) and to
...
\item[\cd{character}]
  ...
  how \cd{eql} is defined on characters.)
...
\begin{itemize}
\item  ${\it S}$ is accessible in \cd{*package*} at compile time and ${\it S}'$ is accessible in
       \cd{*package*} at load time.
...
\end{itemize}
  ...
  to do with \cd{*readtable*} or how the function \cd{read} would parse the 
...
\item[\cd{package}]
  ...
  \cd{find-package} with the package name as an argument.  An error is
  signaled if no package of that name exists at load time.

\item[\cd{random-state}]

 We say that two \cd{random-state} objects are {\it functionally equivalent} if 
  applying \cd{random} to them repeatedly always produces the same 
  pseudo-random numbers in the same order.  
  
  Two random-states are similar as constants if and only if copies of
  them made via \cd{make-random-state} are functionally equivalent.
  (Note that a constant \cd{random-state} object cannot be used as the {\it state}
  argument to the function \cd{random} because \cd{random} performs
...
\item[\cd{array}]
  ...
  for vectors (one-dimensional arrays), the \cd{length} and \cd{element-type}
  and the result of \cd{elt} for all valid indices;
  for all other arrays, the \cd{array-rank}, the result of \cd{array-dimension}
  for all valid axis numbers, the \cd{array-element-type},
  and the result of \cd{aref} for all valid indices.  (The point of
distinguishing vectors is to take any fill pointers into account.)

  If the array in the source code is a \cd{simple-array}, then
  the corresponding array in the compiled code must also be a
  \cd{simple-array}, but if the array in the source code is displaced, has a
...
\item[\cd{hash-table}]
  ...
  three requirements.
  First, they must have the same test (for example, both are \cd{eql} hash tables
  or both are \cd{equal} hash tables).
...
\item[\cd{function}]
   ...
   to specify that objects of type \cd{function}
...
are \cd{eql} but ${\it A}$ and ${\it B}$ were not \cd{eql}, then we say that ${\it A}$ and ${\it B}$ have been
...
constants, except that objects of type \cd{symbol}, \cd{package},
\cd{structure}, or \cd{standard-object} obey their own rules
and may not be coalesced by a separate mechanism.

\beforenoterule
\begin{rationale}
Objects of type \cd{symbol} and \cd{package} cannot be coalesced because the fact
...
predicate.  Currently \cd{make-load-form} provides a protocol by which
\cd{compile-file} and \cd{load} work together to construct an object in the
...
destructively modify constants \issue{CONSTANT-MODIFICATION} (see \cd{quote}).
...
required to preserve \cd{eql}-ness of substructures within a file compiled
by \cd{compile-file}.
\end{newer}
...
a separate function \cd{documentation} is provided
...
\end{tabular}
\end{flushleft}
In addition, names of special forms may also have \cd{function} documentation.
...
\cd{setf} may be used with \cd{documentation} to update
documentation information.
\end{defun}
\end{obsolete}

\begin{newer}
X3J13 voted in June 1988 \issue{CLOS} to make \cd{documentation}
a CLOS generic function (see chapter~\ref{CLOS}).
\end{newer}

\begin{newer}
X3J13 voted in March 1989 \issue{FUNCTION-NAME} to extend \cd{documentation}
...
to determine whether there is any documentation for a \cd{setf}
expansion function for \cd{cadr}.
\end{newer}
...
\begin{obsolete}\noindent
Invoking \cd{trace} with one or more function-names (symbols) causes
...
\end{lisp}
If a function call is open-coded (possibly as a result of an \cd{inline}
declaration), then such a call may not produce trace output.

Invoking \cd{untrace} with one or more function names will cause those
...
Calling \cd{trace} with no argument forms will return a list of functions
currently being traced.

Calling \cd{untrace} with no argument forms will cause all currently
traced functions to be no longer traced.

\begin{newer}
X3J13 voted in March 1989 \issue{FUNCTION-NAME} to extend \cd{trace}
and \cd{untrace}
to accept any function-name (a symbol or a list
whose {\it car} is \cd{setf}---see section~\ref{FUNCTION-NAME-SECTION}).
Thus one may write \cd{(trace (setf cadr))} to trace the \cd{setf}
expansion function for \cd{cadr}.
\end{newer}
...
to specify that the values returned by \cd{trace} and \cd{untrace} when
given argument forms are implementation-dependent.
\end{new}

\cd{trace} and \cd{untrace} may also accept additional
...
to clarify that \cd{step} evaluates its argument {\it form}
in the current lexical environment (not simply a null environment),
and that calls to \cd{step} may be compiled, in which case
...
facility of the same name.  Note that the MacLisp/Lisp Machine Lisp function \cd{time}
...
X3J13 voted in January 1989
\issue{STEP-ENVIRONMENT}
to clarify that \cd{time} evaluates its argument {\it form}
in the current lexical environment (not simply a null environment),
and that calls to \cd{time} may be compiled.
\end{new}
\end{defmac}
...
\cd{describe} prints, to the stream in the variable \cd{*standard-output*},
...
\cd{describe} of a symbol will exhibit the symbol's value,
its definition, and each of its properties.  \cd{describe} of a
...
\cd{describe} returns no values (that is, it returns what the expression
...
to let \cd{describe} take an optional second argument:
...
The output is sent to the specified {\it stream}, which
 ...
 the {\it stream} may also be \cd{nil} (meaning \cd{*standard-output*})
 or \cd{t} (meaning \cd{*terminal-io*}).

The behavior of \cd{describe} depends on the generic function
\cd{describe-object} (see below).
\end{defun}
\end{newer}
...
X3J13 voted in March 1989 \issue{DESCRIBE-UNDERSPECIFIED} to add
  the generic function \cd{describe-object}, which writes a description of an object to a
  stream.  The function \cd{describe-object} is called by the \cd{describe} function; it
  should not be called by the user.

  Each implementation must provide a method on the class
  \cd{standard-object} and methods on enough other classes to ensure that
  there is always an applicable method.  Implementations are free to add
  methods for other classes.  Users can write methods for \cd{describe-object} for
  ...
  method.
   ...
   cannot be \cd{t} or \cd{nil}.
   The values returned by \cd{describe-object} are unspecified.

   Methods on \cd{describe-object} may recursively call \cd{describe}.  Indentation,
   ...
   \cd{describe} recursively if there are more structural levels.
   If this rule is not obeyed, the results are undefined.

   In some implementations the {\it stream} argument passed to a \cd{describe-object}
   method is not the original stream but is an intermediate stream that
   implements parts of \cd{describe}.  Methods should therefore not depend on the
   identity of this stream.

\beforenoterule
\begin{rationale}
 This proposal was closely modeled on the CLOS description of \cd{print-object},
 ...
 \cd{print-object} are applicable to \cd{describe-object}.

 The reason for making the return values for \cd{describe-object} unspecified is to
 avoid forcing users to write \cd{(values)} explicitly in all their methods;
 \cd{describe} should take care of that.
\end{rationale}
\afternoterule
\end{defun}
\end{newer}
...
\cd{inspect} is an interactive version of \cd{describe}.
The nature of the interaction is implementation-dependent,
but the purpose of \cd{inspect} is to make it easy to wander
...
X3J13 voted in January 1989
\issue{RETURN-VALUES-UNSPECIFIED}
to specify that the values returned by \cd{inspect}
...
\cd{room} prints, to the stream in the variable \cd{*standard-output*},
...
\begin{newer}
X3J13 voted in June 1989 \issue{PATHNAME-LOGICAL} to require \cd{ed}
to accept logical pathnames (see section~\ref{LOGICAL-PATHNAMES-SECTION}).
\end{newer}
...
to accept as a {\it name} any function-name (a symbol or a list
whose {\it car} is \cd{setf}---see section~\ref{FUNCTION-NAME-SECTION}).
Thus one may write \cd{(ed '(setf cadr))} to edit the \cd{setf}
expansion function for \cd{cadr}.
\end{newer}
\end{defun}
...
\begin{newer}
X3J13 voted in June 1989 \issue{PATHNAME-LOGICAL} to require \cd{dribble}
to accept logical pathnames (see section~\ref{LOGICAL-PATHNAMES-SECTION}).
\end{newer}
...
to clarify that \cd{dribble} is intended primarily
...
techniques for implementing \cd{dribble}.  All are reasonable interpretations
of the original specification, and all behave in approximately the same
way if \cd{dribble} is called only from the interactive top level.
However, they may have quite different behaviors if \cd{dribble} is
called from within compound forms.

Consider two models of the operation of \cd{dribble}.  In the ``redirecting''
model, a call to \cd{dribble} with a pathname argument
...
may be affected as well.  A call to \cd{dribble} with no arguments
undoes these side effects.

In the ``recursive'' model, by contrast, a call to \cd{dribble} with a
...
the dribble file.  A call to \cd{dribble} with no arguments
does a \cd{throw} that exits the recursive command loop and returns
to the original caller of \cd{dribble} with an argument.
...
a newline (provided by the function \cd{print}) and the words
...
\cd{Bird} to a file named \cd{basketball}.
...
\cd{basketball} will be closed, and then execution of the
\cd{progn} form will be resumed.  A newline and ``\cd{Larry~}'' (note the trailing space)
will be printed to the standard output, and then the call
\cd{(dribble)} may complain that there is no active dribble file.
Once this error is resolved, the word \cd{Bird} may be printed
...
\cd{and cream of rice} will be sent to a file named \cd{baby-food}.

The recursive model will direct exactly the same output to the file
named \cd{baby-food} but will never print the words
...
not to decide which model to use.  Users are advised to call \cd{dribble}
...
Whenever \cd{apropos} finds a symbol, it prints
...
otherwise ``all'' packages are searched, as if by \cd{do-all-symbols}.
Because a symbol may be available by way of more than one inheritance
path, \cd{apropos} may print information about the same symbol more than once.
The information is printed to the stream that is the value
of \cd{*standard-output*}.
\cd{apropos} returns no values (that is, it returns what the expression
\cd{(values)} returns: zero values).

\cd{apropos-list} performs the same search that \cd{apropos} does but
...
fraction of a second, as specified by \cd{internal-time-units-per-second}.
...
to specify that \cd{decode-universal-time},
like \cd{encode-universal-time}, ignores daylight saving time information
if a {\it time-zone} is explicitly specified; in this case
the returned {\it daylight-saving-time-p} value will necessarily be
\cd{nil} even if daylight saving time happens to be in effect in that
...
\cd{get-internal-run-time} and \cd{get-internal-real-time}
...
to \cd{get-internal-time}, rendering that function less useful for accurate
timing measurements.
\end{rationale}
\afternoterule
\end{defun}
...
measured in the units defined by \cd{internal-time-units-per-second}.
\end{defun}
...
\noindent
See also \cd{user-homedir-pathname}.
...
\begin{obsolete}
One standard feature name is \cd{ieee-floating-point}, which should
...
are read in the \cd{keyword} package unless an explicit prefix
designating some other package appears.  The standard
feature name \cd{ieee-floating-point} is therefore actually the
...
The {\it object} is returned as the value of \cd{identity}.

\begin{new}
The \cd{identity} function is the default value for the \cd{:key}
...
of the
\cd{identity} function regarded as a function of a complex numerical argument.

Many other constructs in Common Lisp have the behavior of \cd{identity}
when given a single argument.  For example, one might well use \cd{values}
in place of \cd{identity}.  However, writing \cd{values} of a single
...
\beforenoterule
\begin{incompatibility}
In Maclisp, \cd{progn} was a function of any number of arguments that
returned its last argument,
so \cd{progn} could be used as an identity function.
In Common Lisp, \cd{progn} is a special form and therefore cannot be used for
that purpose.
\end{incompatibility}
\afternoterule
\end{new}
