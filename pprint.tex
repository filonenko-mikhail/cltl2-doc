...
\cdf{*print-circle*} (and possibly also \cd{*print-shared*})
...
\cdf{*print-circle*}, and \cd{*print-\discretionary{}{}{}shared*}.  An error message is issued if it is
...
n} specifies the indentation in ems.  If \emph{relative-to} is \cd{:block}, the
...
arguments to printing functions.  The arguments \emph{colnum} and {\it
...
\cdf{*print-circle*} (and possibly also \cd{*print-shared*}) is not \cdf{nil} and
...
divided into segments \cd{{\Xtilde}<\emph{prefix\/}{\Xtilde};\emph{body\/}{\Xtilde};{\it
...
\emph{Table} (which defaults to \cd{*print-pprint-dispatch*}) must be a
...
should obey \cd{*print-readably*}.  Any values returned by the \emph{function}
are ignored.
...
The \emph{table} (which defaults to the value of \cd{*print-pprint-dispatch*}) must be a pprint
dispatch table.  The specified entry is placed in this table.

It is permissible for \emph{function} to be \cdf{nil}.  In this situation,
...
\cdf{*print-circle*}, \cd{*print-shared*}, and \cdf{*print-escape*}, and can tolerate
several different kinds of malformity in the data structure.  The output below
shows what is printed out with a right margin of 25, \cdf{*print-pretty*}
\cdf{t}, \cdf{*print-escape*} \cdf{nil}, and a malformed \cdf{kids} list.
\begin{lisp}
